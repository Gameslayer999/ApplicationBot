"""Editing an answer in the real Review panel is saved and used (decision 153).
...
store — the same store `run_apply` loads before it fills. Also asserts the pre-submit auto-save:
clicking "Watch it fill" with an unsaved edit saves it before the browser is opened.
"""
...
from applicationbot import answer_overrides, web
...
def ui(monkeypatch, tmp_path):
    """The real UI on a free port with one ready application, its overrides in a temp dir."""
    ...
    yield f"http://127.0.0.1:{srv.server_address[1]}/", watched
    srv.shutdown()
...
def test_edited_answers_are_saved_and_used(ui):
    ...
    with sync_playwright() as pw:
        ...
        assert len(boxes) == 2, "every answer — filled and unanswered — must be editable"
