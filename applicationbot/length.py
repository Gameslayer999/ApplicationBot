...
class LengthBudget:
    ...
    def prompt(self) -> str:
        ...
        multi = round(one * 1.5)
        return (
            f"Length target: aim to slightly OVERFILL {self.pages:g} page(s) — produce a bit "
            "MORE strong, relevant content than fits, never less. The app then measures the "
            "rendered PDF and removes the least-relevant lines one at a time until it fits "
            f"exactly {self.pages:g} page(s), so overshooting guarantees a full page while "
            "undershooting leaves it half-empty and cannot be fixed afterward. Overshoot on "
            f"purpose. Include the FULL allowance — {self.max_experience} experience entries, "
            f"{self.max_projects} projects, {self.max_activities} leadership/activity entries "
            f"(the most relevant), each with the full {self.max_bullets_per_entry} bullets — "
            "and keep education. Go below the allowance ONLY when you genuinely lack relevant, "
            "truthful content for a slot; never pad with filler or repeat yourself to reach "
            "it. When in doubt, include the extra entry or bullet — the app will trim it if it "
            "doesn't fit.\n"
