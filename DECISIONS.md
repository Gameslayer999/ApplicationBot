...
|---|------|----------|--------|
| 188 | 2026-08-19 | **A profile is exportable as one dated `.zip` that mirrors `profile/`, so backing up or moving computers is a download and an unzip — and the archive is an ALLOWLIST, so a config file added next month cannot ride along.** User: *"lets get started on the ability to export a profile"*; the scope was put to them before any code (portable setup · everything-including-history · the apply profile alone · a human-readable summary) and they chose the **portable setup bundle**. **What a profile is, for this purpose.** The work a user does once: `application_profile.yaml` (identity, eligibility, EEO, the 41-entry answer bank and dropdown aliases), `discovery.yaml` (the filters), **every** `profile/*.yaml` that validates as a Résumé — not just `resume.yaml`, because the Profile page lets the user keep several and pick between them, and an export that silently took one would hand back a backup missing work they'd notice only on the new machine — and `uploads/` (decision 152's kept PDFs, already content-addressed, so they carry no absolute path). **Allowlist, not folder-minus-denylist.** The denylist is the tempting shape and it fails open: the next config file lands in `profile/` and ships itself. The allowlist fails closed — a new file is left out until someone decides it belongs. Three files are excluded on purpose and each carries its reason into the manifest: `mailbox.yaml` (an inbox credential), `safety.yaml` (the arming switch and submission cap — Guideline #3 says arming is a deliberate per-machine act, and an armed `safety.yaml` travelling inside a backup is exactly how a machine submits before its owner meant it to), and `notifications.yaml` (an ntfy topic is a *readable address* — whoever holds the zip could subscribe to the applicant's job alerts). History, tailored PDFs and caches are left behind as this machine's record or as regenerable. **The layout is the restore mechanism.** Members sit exactly where they sit under `profile/` — résumés at the top level, uploads under `uploads/` — so `unzip` into `profile/` IS the restore, with no importer to write and no import path to get wrong. A tidier `resumes/` subfolder was rejected for precisely that reason: it would have made the UI's one-line restore instruction false. A matching **Import** button is deliberately NOT in this change (it must decide merge-vs-replace per file, which is its own decision); the export is useful without it, and the UI promises only what the app can do. **Surfaces:** `GET /profile/export` (zip attachment, dated filename; **JSON on failure** so a blocker lands inline by the button instead of saving a file the user discovers is broken later — UI Principle #3), the Profile tab's *⬇ Download profile (.zip)* under a heading that says what it is for, with the restore step and an explicit **"Left out on purpose"** list beside it (UI Principle #5 — a backup that quietly omits things is worse than no backup), and `python -m applicationbot.profile_export [DEST]` so it can be cron'd (Guideline #8). An empty profile raises rather than zipping nothing, naming the missing file and the fix. **One defect found and fixed while testing:** `export_to` decided file-vs-directory with `.is_dir()`, so `export_to("~/Backups")` on a directory that didn't exist yet wrote a file literally named `Backups`; it now decides on the `.zip` suffix. **Verified end to end, not just unit-tested:** the live server returns the zip with `Content-Type: application/zip` and the dated `Content-Disposition`, `unzip -t` clean; unzipping it into a **fresh `APPLICATIONBOT_DATA`** reloads the whole setup on that clean root (41 saved answers, 5 dropdown aliases, the filters, the résumé's 4 roles / 8 projects, both kept PDFs); a real Chromium drive of the Profile tab shows the section, the nav pill, downloads `applicationbot-profile-2026-08-19.zip`, ends on *Downloaded ✓* with the button restored, and logs **zero console errors**; and the no-profile-yet path returns HTTP 400 with the exact fix. **Tests:** new `tests/test_profile_export.py` (14) — the exact member list, three parametrised leak guards that assert the excluded files' *contents* appear nowhere in the archive, every résumé exported with config YAML not mistaken for one, the mirror layout the restore instruction depends on, the manifest, the empty-profile error, both `export_to` destination forms, and the button/route/nav wiring. Full suite **942 passed**. | Adopted |
| 187 | 2026-08-18 | **No question is called unanswerable until Claude has looked for its answer in everything the applicant has already given us — a fourth BATCHED stage in the page fill, plus two shadowing bugs it exposed that were losing answers the user had actually written.** User: *"make sure that unanswered form questions are fed through ai first to figure out if there is an answer anywhere in available data before being marked as unanswerable"*. **What existed.** Decision 041's two-pass fill already spent ≤3 batched calls per page: classify a novel question onto a structured profile field, match it against the answer bank as a rewording, and pick dropdown options. What it never did was ASK whether the answer was derivable from the applicant's data as a whole — so a question that is neither a known field nor a reworded bank entry went straight to "no saved answer" and parked the application. **Choice: a `derive` stage, extraction not drafting.** `answer_bank.derive_answers` sends the still-unanswered questions in ONE call with `AnswerResolver.data_facts()` — résumé JSON, the profile's stated facts, and the answers already given — and one instruction: answer only what that data settles, empty string otherwise, never treat absence as evidence. It runs at the SAME model tier as its sibling batch calls, not the cheap drafting model (this one decides whether a factual claim goes onto a real application, and an over-cautious cheap model just hands the field back unanswered — measured: the weak model returned 0 of 5 on one run and 3 of 7 on another; the batch tier returns the same 1 of 6 on repeat runs). Guards: demographic questions are never derived (only the applicant may declare those), company-specific ones stay with `generate_answer`, placeholder replies ("N/A", "unknown", "TBD") are dropped, and a reply over 400 chars is refused because an extraction that runs that long is drafting. **Precedence is unchanged and deliberate:** user edit → structured rules → answer bank → derived. `resolve()` was split into a `_resolve_rules` chain plus a thin wrapper, because most rules `return p.field or None` the moment their profile field is empty and so never reach the end of the chain — the fallback had to sit outside them. **Two defects this exposed, both fixed (they were losing the USER'S OWN answers, no AI involved).** (1) A question whose structured field is empty never reached the bank lookup at the end of the chain: "What is your anticipated start date?" was banked as *September 2026* and still reported "no saved answer" on a live Palantir form, because `earliest_start_date` was unset. The wrapper now consults the bank in that case — the user's own answer must be used before anything is called unanswerable, and it costs no Claude call. (2) Every injection site appended beside the BLANK placeholder that a previous run captured for that same question, and `resolve()` reads the first match — so the freshly-decided answer was silently ignored and the field still reported "no saved answer". One `_inject_answer` helper now drops the in-memory placeholder first (the on-disk one is untouched, so the user is still asked to answer it once) and all three stages use it; the bank-match stage had the same bug. **Visibility (this is why it is not silent):** a derived answer is labelled `derived`, not `resolver`, and every report carries "Re-read your résumé and profile for N question(s) … answered M, left K for you" — without that line "no saved answer" cannot be told apart from "nobody looked", and the first is a fact about the profile the user can fix. `_mark_derived` had to move ABOVE `print(report.summary())` and the `on_filled` callback, which is what the web UI and the review panel read — relabelling after them would have shown the user a report that hid which answers were inferred. **Cost:** one extra batched call per form page, only when questions remain unanswered. **Verified:** 32 new tests, and five live dry-run fills of a real Palantir form — 9 unanswered → 6, with the recovered answers traced back to the user's own résumé/bank in every case. | Adopted |
| 186 | 2026-08-18 | **A night can run WITHOUT tailoring — `--no-tailor` sends the user's own résumé verbatim — and, unflagged, the night now follows the same saved résumé policy the in-app loop uses; the policy helpers move out of `web.py` so there is one implementation, not two.** User: *"we should be able to run the night loop without tailoring as well (saves tokens, uses the resume i personally tailored vs the one the agent tailored that i have never seen)"* — two distinct reasons, cost and **trust**, and the second is the stronger one: an unattended run that submits a résumé its owner has never read is doing something they did not sanction. **What existed.** Decision 174 already gave `run_testing_mode(tailor=False)` (no Claude call, no reuse scan, sends the user's best uploaded résumé document or their base résumé rendered verbatim, via `untailored_pdf`), and decisions 178/180 gave the loop a saved résumé POLICY — `smart` / `always` / `under N` / `never` — decided by `_loop_policy` + `_tailor_choice`. Both helpers lived inside `web.py`, so the CLI night (decision 185) could not reach them and defaulted to always-tailor. **Choice.** The two helpers move verbatim into `pipeline.py` as public `loop_policy` / `tailor_choice`; `web.py` keeps the old private names as import aliases, so every existing call site and both existing test modules are unchanged (Guideline #7 — a pure move, no behaviour change). `night.py` then reads the same saved policy and applies it per posting (`tailor_choice(policy, m.fit_score)` → `tailor` / `force_retailor`, plus the saved `reuse_threshold`), so **⚙ Loop settings now governs the CLI night exactly as it governs the app** — one place decides which résumé goes out, which is decision 180's rule extended to the night. `--no-tailor` forces `never` for that run only and writes nothing back to `discovery.yaml`. **Rejected:** a night-only tailoring switch stored separately (a second source of truth for the same decision — precisely what decision 180 removed), and quietly defaulting the night to no-tailor (cheaper, but it would silently change what a user's armed run submits). **Because it changes what is SENT, the choice is now on the record everywhere:** the console line at start, `summary.json` (`resume_policy` + `resume_note`), `report.md` ("Résumé sent: …"), and a `resume_policy` journal event — an agent reporting a night must say which résumé went out. Token note for the runbook: `--no-tailor` removes the largest per-application call but discovery's fit judge and per-question answer generation still cost tokens, so such a night is much cheaper, not free. **Verified:** 6 new tests (the flag, the saved-policy path, the override, the report line, and the plain-words description of every mode) with the run-queue fake now really calling `apply_one` so the résumé decision is exercised end-to-end, plus a real `--no-tailor` dry-run night. | Adopted |
| 185 | 2026-08-18 | **An agent can run the whole night alone: `python -m applicationbot.night --goal 100 --until 07:00 --arm` drives a submission GOAL across cycles, records every application as JSON, audits its own submissions in the morning, and turns what failed into a ranked backlog.** User: *"lets add stuff to this repo so that an agent(hermes, claude, etc) would be able to run the loop overnight (ex: i want it to send 100 applications tonight), without ever needing to come back to the user for feedback, confirmation, etc. The agent would drive the applications, document them, and review its own work, as well as continually improving itself"*. **What was already there.** The unattended machinery mostly existed — `runner.py` (`--continuous`) applies to every cleared match with failure isolation, a Claude usage-limit pause/retry and the kill switch; `pipeline.run_testing_mode` is the ONE submit path both it and the web loop use; `parking.py` classifies a block; `tracker.py` records everything. **What was missing was agent-shaped, not pipeline-shaped:** (a) `--max` caps ONE cycle, so "100 tonight" had no representation — and the default cap is 10/run, so a 100-goal night would have quietly stopped at 10; (b) every result was prose on stdout, so an agent had nothing to branch on; (c) no wall-clock deadline; (d) no audit — the printed number is how many times the bot clicked submit, which is NOT how many applications landed; (e) nothing turned a night's failures into the next night's fixes; (f) no written contract telling an agent what it may decide alone at 3am. **Choice: two new modules and a runbook, no second submit path.** `night.py` — a session driver that loops the EXISTING discover→judge→apply cycle until the goal, the `--until` deadline, `profile/KILL`, a circuit breaker, or a fatal stop; writes `nights/<ts>/events.jsonl` + `summary.json` + `report.md`; and exits 0 goal · 3 deadline · 4 kill · 5 breaker · 6 preflight · 7 fatal so an agent never parses prose. `night_review.py` — the morning audit the user asked for, answering exactly the three things they picked: **were the submissions real** (every claimed submit matched to its tracker row's status and `date_applied`; `unconfirmed` called out as a possible false success, and the review exits 2 when the night's own count cannot be trusted), **what failed and is it getting worse** (failure kinds and per-ATS grouping, trended against earlier nights' `summary.json`, with NEW-tonight flagged as a regression), and **is the fit judge calibrated** (decision 043's outcome calibration). Output is `findings.json`, a ranked backlog where each item names its evidence and the file that fixes it. [docs/AGENT_NIGHT_RUN.md](docs/AGENT_NIGHT_RUN.md) is the contract: never ask the user; the three tiers an agent may improve on its own (data always; code fixes on `development` ONLY behind a failing-first repro test and a fully green suite; anything touching safety, module boundaries, scraping strategy or schema is proposed, not done); and what to report and in which order. `scripts/night.sh` runs preflight → night → review as one idempotent command a cron line can call. **Arming (the user's call, asked before any code was written).** The agent may arm — `--arm` writes `armed: true` AND sets `max_submissions_per_run` to the goal in one write (`safety.save_arming`, which preserves every other key) — but only when the user asked for a real run, and it **restores `safety.yaml` exactly as it found it** when the night ends, on Ctrl-C, and on a crash, so an armed night cannot bleed into tomorrow. `--dangerously-unlimited` is the user's explicit ask for a no-ceiling run ("*a dangerously skip mode just like claude*") — no cap, applying until the deadline, the kill switch, the breaker, or the Claude usage limit. **`safety.py`'s gate itself is untouched**: `may_submit` still checks armed + KILL + cap immediately before every click, and the runbook forbids an agent from editing it. Email verification (the user's *"readonly access to my email"*) needed no new work — the linked read-only bot inbox (decisions 108/182) already serves account walls; preflight now names it so a night never starts without it. **Blocked applications** (the user's third choice) are recorded and skipped, never waited on; the breaker stops the night only when failure is systemic — 5 in a row or 10 of one kind — so 100 attempts are never burned on one broken selector. **Alternatives rejected:** driving the web loop over HTTP (needs a server, a browser session, and its own request queue — the CLI path calls the same `run_testing_mode`); extending `runner.py --continuous` in place (would have changed a shipped command's behaviour, against Guideline #7); and Claude-sampling filled answers for quality, which the user declined for now. **Verified:** 61 new unit tests (goal/deadline/kill/breaker/backoff/preflight/arming round-trip; the real cycle wiring; verification, taxonomy, trend, ranking) plus two real dry-run nights driven end-to-end. Those live runs earned their keep by exposing two defects, both fixed before this entry was written: **(1)** only the FIRST cycle may `revisit` — decision 149 deliberately re-surfaces prepared-but-unreviewed applications so they aren't buried, but overnight nobody reviews anything, so every application the night prepared stayed "unreviewed" and came back next cycle (cycles 1 and 2 both prepared the same Palantir posting; with the fix cycle 2's cleared queue dropped 5→2 and moved to a different company). It was never a duplicate-SUBMIT risk — an `applied` row is excluded from the revisit set — but it would have spent a night's tokens re-tailoring the same postings instead of applying to new ones. **(2)** A discovery-time exception (a board 503, a parser error) used to kill the whole night; it now backs off and re-searches, giving up only after 5 consecutive failures, and an unexpected crash still writes `summary.json` + `report.md` so an unattended run never ends with no record. | Adopted |
| 184 | 2026-08-12 | **Review opens in a popup over the page instead of expanding inside its card — one panel, one place, and the lists stay slim one-line rows.** User: *"make the review a popup instead so it makes the site less cluttered"* (following decision 183, which put 13 ready applications in Discover). **The clutter.** Every card carried its own hidden `.review` panel and expanded in place (decision 125), so an open review pushed the rest of the list — and the search breakdown under it — hundreds of pixels down, and each of the three card types (loop ready, Notifications, parked) held a *separate* panel instance that had to be found again after scrolling. With the durable list now 13 cards long in Discover **and** the same 13 in Notifications, in-place expansion stopped scaling. **The change (front-end only — no endpoint, no request, no submission path touched, Guideline #7).** One `#review-modal` reusing the existing `.modal-scrim`/`.modal` chrome already behind ⚙ Loop settings and Discovery settings (`.modal-wide`, 1020px, because a review carries a full answer table), with `#review-panel` as its body. `toggleReview(id, panel, btn, …)` → `openReview(id, title, signoff)`: the card no longer owns a panel, the button is a plain **Review** (no ▾/▴ state to keep in sync), and `renderReview`/`rescanReview`/`saveAnswers`/both `signoff` builders are **unchanged** — they paint into the modal's panel exactly as they painted into the card's. Closes four ways (UI Principle #1): the header ✕, `Esc`, a click on the scrim, and the panel's own bottom button, renamed *Close review* from decision 179's *▴ Collapse this review*. **Reloaded on every open, not cached.** The old panel rendered once and re-showed (`dataset.loaded`); a popup that reopened stale after a rescan, a re-tailor or an edit would misreport what is about to be submitted, so each open re-fetches `/track/review`. **One panel means late responses must be policed:** a module-level `REVIEW_OPEN` holds the id the popup is showing, and every asynchronous paint checks it — the initial load drops its response if the user closed or switched cards while it was in flight, and a rescan that lands minutes later re-renders **only** if its own application is still on screen (its result stays in `RESCAN_MSG`, shown when that application is reopened). **Acting on a review closes it** (`watchReady`, `applyReady`, `reapplyParked`) — but only *after* the pre-fill answer save succeeds, so a save error keeps the popup open where the Save-answers error is. That is not cosmetic: those actions report progress and errors into Discover's own panels (`#loop-msg`, `#test-progress`), which a popup left open would cover — silence the user would read as nothing happening (UI Principle #5). Rejected: keeping the panel in the card and merely capping its height (the clutter is the expansion itself, and it still can't be read while the 2s `/loop/status` poll rebuilds the list); a full-page review route (loses the list you came from, and the back-navigation is a second thing to get right); pinning Watch/Apply into a `.modal-foot` (decision 125's rule is that the submit sits *past* the answers — a footer would put it one click from the top of an unread review). **Verified in the real app** (Playwright over the real server, GET-only against the live tracker): from Discover, a card's **Review** opens the popup titled *"Verkada — Technical Support Engineer - University Graduate 2026"* with the whole panel — posting meta, both artifact buttons, the 17-answer table with required marks and the amber *Check this* flags, *Needs attention — unanswered (1 of 1)*, Save answers, Rescan questions, Tailor this résumé, the job description toggle — ending in `[Watch it fill, Watch it apply ▶, Apply ▶, Close review]`, and **nothing renders inside the card** (`#loop-ready .pkcard .rv-h` is null); from Notifications, all 17 cards are one-line rows and a parked card's popup carries `[Re-apply (dry-run) ▶, Submit for real ▶]`; `Esc` and a scrim click both close it, with **zero page errors**. Served page JS is `node --check` clean. **Tests:** `tests/test_web_loop_review.py` rewritten for the popup — it survives ~3 loop-status polls (the old in-card panel's failure mode), is titled with its application, renders nothing in the card, and closes on both `Esc` and *Close review* with the card still there; the five other review UI tests were repointed from `#loop-ready .review` to `#review-panel` (same assertions). Full suite green. | ✅ Implemented |
| 183 | 2026-08-12 | **Discover's "Ready to apply" list becomes the durable one the Notifications tab already showed — one `_ready_cards` helper feeds both — so an application prepared in an earlier run, or before a restart, is still reviewable and submittable where the loop lives.** User: *"add review functionality from the notifications into loop discover"*. **The gap.** The two lists were built from different sources. Notifications (`_build_inbox`, decision 145) unions the in-memory loop queue with the still-`dry-run` applications named by `approval_needed` log rows, so it survives a restart; `GET /loop/status` read `_LOOP_STATE["ready_ids"]` alone. That queue is cleared by a process restart **and** by `_loop_reset()` at the start of every run — so prepared-but-unsubmitted applications vanished from Discover the moment the loop stopped or the next run began, and the only way back to their review was the Notifications tab. The cards themselves were never the problem: Notifications already renders the *same* `loopReadyCard` / `renderReview` / `parkedCard` (decision 138), and every action on them (`queue_submit` / `queue_watch` / `queue_watch_submit` / `queue_rescan`) already routes to the per-click armed path when the loop is idle. **The change.** `_ready_cards(ready_ids, *, log=None, path=None)` is lifted out of `_build_inbox` verbatim (same union, same order — this run's queue first, then the restored ones, deduped, `dry-run` only) and called by both `_build_inbox` and `/loop/status`; the caller that already read the notification log passes it in, so the shared helper adds no query. `/loop/status` also returns **`ready_run`** — how many of the listed cards this run prepared — because the goal counter must keep counting *this run*: with a goal set the header still reads "Ready to apply (2 of 5 goal)" and names the rest separately ("· 3 more prepared earlier"); with the loop stopped it reads "Ready to apply (13) — prepared earlier, waiting for you", so a list appearing under an idle loop is never mistaken for a run in progress (UI Principle #4). **Nothing about submission changed** (Guideline #7): no new endpoint, no new queue, no change to the gate, the cap, or what any button does — only which applications the Discover panel lists. Rejected: a second "held over" list below the loop's own (two lists of the same thing, and the review would open in whichever half the user didn't scroll to), and clearing `ready_ids` less aggressively (a run's own counters must not inherit a previous run's queue). **Verified in the real app** (Playwright over the real server, GET-only against the live tracker): with the loop idle `/loop/status` returned **13** ready cards and `ready_run: 0` where it previously returned `[]` — byte-identical to `/inbox`'s ready list — Discover rendered all 13 under "Ready to apply (13) — prepared earlier, waiting for you", and opening one card's **Review** loaded the full panel (Posting · 17 answers with their required marks · the amber "2 answers don't match what the question asks" check · "Needs attention — unanswered (1 of 1)" · Save answers · Rescan questions · Tailor this résumé) with the loop stopped. Served page JS is `node --check` clean. **Tests:** `tests/test_notifications.py` gains `test_ready_cards_shared_by_discover_and_notifications` — this run's card first, an earlier run's restored from the log, an already-submitted one not offered again, and `_build_inbox` returning the identical list. Full suite green. | ✅ Implemented |
| 182 | 2026-08-12 | **MyGreenhouse Quick Apply becomes OPT-IN and signs in with the security code Greenhouse emails; the stored password is deleted, not migrated.** User: *"myGreenhouse seems to use an email/security code combo to log in now, not just a password"* → chose *"drop by default with opt in to the email automation"*. **Why the old path was dead:** `_greenhouse_native_autofill` (decision 017) filled a password field and clicked sign-in; against a code-only login that step can never succeed, so every Greenhouse posting would burn the 20s popup-close wait plus the 8s popup wait before falling back — for an autofill decision 017 itself measured as **unnecessary: our resolver fills a live Greenhouse form 15/15 with no account**. That asymmetry (a mailbox-dependent login for zero coverage gain) is why the default is OFF rather than "make the code flow work for everyone". **Options:** (a) delete the feature outright — simplest, but throws away Quick Apply for the user who wants it; (b) automate the emailed code — **chosen, behind an opt-in**; (c) persist a browser session from one manual login — rejected: a recurring manual re-login is exactly what Guideline #8 forbids. **Shape.** `ApplicationProfile.greenhouse_password` is **removed** (with it, decision 060's write-only field, `set/get_greenhouse_password`, `greenhouse_linked`, `greenhouse_credentials`, `POST /profile/greenhouse/unlink`, and the plaintext→keychain migration); `greenhouse_quick_apply: bool = False` replaces it. `load_profile` now runs `_drop_dead_greenhouse_password` — once per process, best-effort — which **deletes** the keychain entry and strips any legacy `greenhouse_password:` from the YAML. Deleting rather than keeping is the Guideline #12 call: a password that can no longer sign in anywhere is pure liability, and it may be reused by the user elsewhere. `greenhouse_quick_apply_problem(profile)` is the single source of truth for "can this run", returning '' or the exact blocker + its fix — off / no account email / no linked inbox / **the MyGreenhouse address isn't the linked inbox** (that last one matters: the code goes to the account address, so a mismatch means we'd poll an inbox the code never reaches). Same string feeds the Profile panel, the Autofill-accounts row, and `report.errors`. **Sign-in flow:** email → Continue → `mailbox.wait_for_verification(sender_contains="greenhouse", since_epoch=asked_at, prefer_code=True)` → `_fill_security_code` (handles both a single field and one-box-per-digit OTP widgets) → submit; failure is non-fatal and the popup is closed so no half-finished sign-in tab is left. **Two mailbox changes, both needed for a repeating login and both reusable:** `since_epoch` (a code is only valid if it arrived AFTER we asked — the newest-matching-message scan would otherwise replay last session's code and fail the sign-in *silently*; unknown-age messages count as stale, and Workday's flow passes no `since_epoch` so it is unchanged) and `prefer_code` (take the digits over a verify-ish link in the same email). Fixing the freshness guard surfaced a **latent bug in date handling**: RFC 2822 `-0000` parses to a naive datetime whose `.timestamp()` is read as local time, shifting a send time by the machine's UTC offset — now pinned to UTC. **Tests:** `tests/test_greenhouse_creds.py` rewritten (12: opt-in gating, all four blocker messages, keychain+YAML deletion, and the sign-in driven against a fake page/popup asserting the code is fetched fresh, code-preferred, Greenhouse-only, typed in, and the popup closed on timeout) + 4 in `tests/test_mailbox.py` for freshness/`prefer_code`; suite **827 passed**. **Drove the real UI**: toggle off→on saves through `/profile/update`, survives reload, and the mismatch blocker renders naming both addresses and both fixes, with an "Open inbox settings" button; no JS errors; profile restored afterward. **UNVERIFIED against a live MyGreenhouse account** — the sign-in selectors are best-effort, as they were before. | 🟡 Implemented, opt-in; live sign-in unverified |
| 181 | 2026-08-12 | **Every location field moves into one "Location" section on the Profile screen — home address *and* the four work-location preferences — so "I have no preference" is one place to look, not five.** User: *"where does the profile list location preferences? I am trying to set that i have no preference but I dont see it"*, then *"lets consolidate all those options into one location tab so its easier for other people."* Scope and placement were put to the user before any code: they chose **work preferences + home address**, as a **section on the Profile screen** (not a sixth left-rail tab). **The problem.** Nothing was missing — `work_arrangement` already offers *"No preference"* as its first option (decision 094) — it was unfindable. Applicant details ran identity → address → links → eligibility → **relocate** → **remote** → **arrangement + commute** → **office locations** → salary → EEO, so the answer to one question ("where will I work?") was spread across four rows separated by unrelated ones, and the two that express *no preference* (`willing_to_relocate`, `open_to_remote` at "—") sat in a row with `us_citizen`, reading as eligibility rather than preference. **The change (`web.py`, layout only — no endpoint, handler, model field, or stored format touched).** A new `#s-location` section, second on the page and second in the section-jump nav, holds one `#location-card` split by two `.grouphead` labels: *Where you live* (Country · State/City · Street/ZIP) and *Where you'll work* (Willing to relocate · Open to remote · Preferred work arrangement · Max commute · Preferred office locations), each group with one line of copy — the second stating outright that leaving everything at "—"/"No preference"/blank is a valid, complete answer, which is what the user was hunting for. Applicant details keeps name, contact, links, eligibility, salary, start date, experience, how-heard and EEO; `us_citizen` now stands alone where `willing_to_relocate` was its row-mate. **The save path is the risk, and it is pinned.** `collectProfile()` read `#profile-card` only, so a field rendered in a new card would have looked editable and been dropped on save — silently wiping a saved answer, the shape of decision 159's data loss. It now merges `cardData($("location-card"))` alongside it, and a new test asserts *every* `data-k` the Location card renders is read back by `collectProfile`. `parseLocation`/the `city`+`state` → `"City, ST"` recomposition are unchanged and moved with the fields. Considered and rejected: a sixth top-level tab (it would need its own load/save wiring and split one Save-profile button in two), and pulling `discovery.yaml`'s `remote_only` into the same section (it drops postings at discovery time — a different kind of switch, in a different file, on the Discover tab; the section's subhint says where it lives instead). **Verified in the real app** (Playwright over the real server): the nav reads *… Applicant details · **Location** · Languages …*, the section renders both groupheads and all ten fields in order, Applicant details holds none of them, `collectProfile()` returns the saved values unchanged (`location: "Edison, NJ"`, `country`, `work_arrangement: ""`, `preferred_locations: []`) with **zero console errors**, and the served page JS is `node --check` clean. **Tests:** new `tests/test_profile_location_section.py` (4) — the card holds exactly the ten fields in order, Applicant details holds none of them, every rendered field is collected on save, and the section + nav pill exist. Full suite **813 passed**, and `profile/application_profile.yaml` is byte-identical (shasum) before and after. | ✅ Implemented |
| 180 | 2026-08-12 | **Neither tailor checkbox survives: ⚙ Loop settings alone decides the résumé for what the loop prepares itself, and a SINGLE application carries its own tailoring choice — a second button on each judged row, and a "Re-tailor résumé" action inside its review.** User: *"since the loop settings exist, get rid of the tailor resume checkbox in the autoapply loop"*, then *"remove from both but when pressing apply (and while reviewing) for single applications, have the button in there"*. (Which of the two checkboxes was meant was genuinely ambiguous — both were named back to the user, with the other two tailor controls in the app listed, before anything was removed.) **What was wrong.** Tailoring was decided in three places at once: decision 178's saved résumé policy, decision 179's per-run "Don't tailor" switch, and decision 174's "Tailor my résumé to the posting first" checkbox in the search breakdown. The last two were *modes* — sticky state set at one moment that silently governed a click made later, which is exactly the "same button, two outcomes" failure decision 177 removed from the Apply buttons. **The rule now:** a run-wide default belongs in ⚙ Loop settings; a single application's résumé belongs to that application. **Judged rows** carry both actions instead of a mode: **Apply ▶** (tailor, then fill and submit) and **Apply as-is ▶** (your résumé exactly as it stands, no Claude call), each with its own tooltip and its own confirm wording, dispatched by a `data-jtailor` attribute through the existing delegated handler — the server side (`queue_prepare(url, tailor=…)`) is unchanged, it just has one caller shape now. Below-bar rows read **Apply anyway ▶** / **Apply anyway as-is ▶**, so the "regardless of its score" signal survives on both. `JUDGED_TAILOR` (the shared global) and its delegated `change` listener are gone. **The review panel** gains the tailoring control for one prepared application: **Re-tailor résumé** — or **Tailor this résumé** when that application is currently set to send the résumé as-is, read from its `resume_source`, so the button never hides what it is about to replace. It re-tailors from the job description **saved beside the résumé when the application was prepared** (`resume_store.read_jd` → `pipeline.tailor_and_render`, no re-scrape and no re-judge), records the new PDF and flips the row's provenance to *freshly tailored*, then re-fills the form so the panel's answers match the résumé that will actually be sent. **It rides the rescan path rather than inventing one** (decision 164): same headless dry-run, same `gate=None`, same "landed when the archived report's timestamp changes" polling, same loop-thread queue — `queue_rescan(app_id, retailor=True)` tags the id in `_LOOP_RETAILORS` so the loop re-tailors it at its next step when the loop owns the browser, and runs it directly when idle. The tag is consumed once, so a later plain rescan of the same application spends no Claude usage. Rejected: a new endpoint + a new `autoloop` drain for re-tailoring (it is the same job — refresh this one application — and a second queue could reorder against the rescan of the same row), and reusing Track's existing "Re-tailor ▶", which navigates to the dry-run panel with the posting loaded rather than acting on the application in front of you. **Failure modes are named, not swallowed:** an application whose JD was never stored (prepared before that existed) reports exactly that with the fix ("Apply to the posting again from Discover to tailor it fresh") and re-fills **nothing** rather than filling with a stale résumé; the loop reports the same and goes back to preparing. **Verified in the real app** (Playwright, throwaway `APPLICATIONBOT_DATA`): the loop panel's switch list is down to the five that remain and `#loop-no-tailor` is gone; the breakdown renders no checkbox and each row shows its pair (`Apply ▶` + `Apply as-is ▶`, `Apply anyway ▶` + `Apply anyway as-is ▶`); the as-is confirm reads "…will fill the form with your résumé exactly as it is (no tailoring) and SUBMIT it", and dismissing it fires **no request**; two seeded applications show the two labels correctly (*Tailor this résumé* for the untailored one, *Re-tailor résumé* for the tailored one), the click POSTs `{"id":2,"retailor":true}`, and the missing-PDF case surfaces its specific error in the panel. **Tests:** new `tests/test_per_app_tailoring.py` (9) — a plain rescan keeps its résumé, a tagged one re-fills with the new PDF and flips the provenance, the tag is consumed once, a missing JD fills nothing and says why, both queue routes, and two guards that the removed switches stay removed (no `no_tailor` parameter anywhere, neither checkbox in the page, both `data-jtailor` buttons present). `tests/test_required_marks.py`'s rescan stub now takes `retailor` and asserts the plain Rescan button never re-tailors. Full suite **812 passed**. | ✅ Implemented |
| 179 | 2026-08-12 | **You can watch the bot really submit — per application ("Watch it apply ▶") or for a whole run ("Show the browser while it applies"); "Don't tailor" is a per-run switch on the loop panel; and a ready application's review sits ABOVE the list of judged postings, collapsing back to its one-line row.** User: *"lets have a way to watch the bot actualy send in an application. Also, when looping for applications, lets have a toggle to be able to tell the loop to not retailor resumes before it running. Lets move the review above the list of found applications and have a way to collapse"*. Three options were put to the user before any code; they chose the per-application button **and** the loop-wide toggle, "send my résumé as-is" for the no-tailor switch, and the review-on-top + slim-row layout. **The gap this closes.** Nothing visible ever submitted. "Watch it fill" (decision 125) is a dry run — it never sends — and the loop's own submits ran headless (`_armed_submit(headed=False)`), so the one step the user most needs to trust, the actual send, was the only step they could not see. **What changed (submission path — Guideline #1/#3).** `_armed_submit` gained `headed` + `hold`: the gate, the fill, the recording and the pre-submit required-field check are **identical**; `headed` changes only whether a window is shown, and `hold` (a `threading.Event` the caller clears) whether that window stays open on the confirmation page. Two ways in: (a) **per application** — `queue_watch_submit` / `POST /loop/watch-apply` puts the id in the *same* `_LOOP_SUBMITS` queue (so the cap and the ordering see it) plus a `_LOOP_WATCH_SUBMITS` tag that makes the loop thread run that one headed and holding; with the loop idle it routes to `start_reapply(arm=True)`, which was already headed-and-pausing, so there is no second submit implementation. (b) **whole run** — `start_loop(show_browser=True)` shows every submit but holds none, because a run that stalled on each window the user has to close would not be a run. Rejected: making a watched submit its own drain in `autoloop` (it *is* a submit — a separate queue could let it slip past the cap), and holding the window in `show_browser` mode. **Stop stays honest:** `_LOOP_WATCH_HOLD` releases a held window on Stop or a manual close, and the hold is never re-armed once the stop event is set — otherwise a Stop could sit behind a window nothing would close (pinned by a test). `show_browser` is forced off in a dry run, which submits nothing: the hint then says so instead of promising a window that never opens (UI Principle #3). **"Don't tailor — send my résumé as it is"** was added here as a per-run switch (`start_loop(no_tailor=True)`) and **removed one turn later by decision 180**, which moved the tailoring choice onto the individual application instead — the plumbing described here no longer exists. **Layout.** `#loop-ready` moved above `#loop-scan`/`#loop-judged`: the review is what the user acts on, so it must not be pushed below a long list of judged postings, and collapsed it is a slim row sitting on top of that list. A review panel now also collapses **from the bottom** ("▴ Collapse this review") — after reading a full panel the "Review ▴" toggle that opened it is far above — via a shared `collapseReview(panel)` that resets the card's own toggle label and scrolls the row into view, so it works for the loop's ready cards, the Notifications cards, and the parked cards alike. **Verified in the real app** (Playwright over the real server): the panel order came back `[loop-status, loop-ready, loop-scan, loop-judged]`; ticking **Dry run** disables the show-browser box, clears it, and swaps the hint; the review panel's buttons are `[Rescan questions, Watch it fill, Watch it apply ▶, Apply ▶, ▴ Collapse this review]`; the bottom collapse takes the card from 410px back to its 73px row and restores "Review ▾"; clicking **Watch it apply ▶** raises a confirm that names the posting *and* the window ("A browser window opens so you can watch it happen"), POSTs `/loop/watch-apply`, and — against a seeded row with no résumé PDF — stops before launching any browser with the actionable "The tailored résumé PDF for this application is gone…". Driven against a throwaway `APPLICATIONBOT_DATA`, never the user's own tracker. **Tests:** new `tests/test_watch_submit.py` (11) — headless by default, one watched submit headed *and* holding while the next is back to headless, `show_browser` making every submit visible and holding none, the cap still refusing a watched submit, a Stop not being re-held, both queue routes, and the per-run `no_tailor` override leaving the saved policy intact. Two of decision 176/178's tests were updated for the new worker signature (`dry_run` is no longer the last positional arg; the `_armed_submit` stubs take `**kw`). Full suite **806 passed**. | ✅ Implemented |
| 178 | 2026-08-11 | **A "⚙ Loop settings" popup on the loop panel holds what governs a run — the résumé policy (including "only tailor when the fit is under N"), the fit cutoff, résumé-reuse strictness, and a submission cap — read once when a run starts.** User: *"we should also be able to set all settings before starting the loop (ex: don't retailor, retailor only if under some fit score, etc)"*, then *"actually, lets place it on a popup in the loop panel"*. Scope confirmed with the user: all four settings, on the loop panel. **The gap.** The loop's only résumé control was a single "Re-tailor from scratch" checkbox — every application otherwise took the same path, and the two choices that existed elsewhere (`tailor=False`, decision 174's per-click "don't tailor"; the reuse threshold, a constant in code) were unreachable for an autonomous run. The fit cutoff lived in a different panel, so setting up a run meant two dialogs. **The résumé policy** is one four-way choice applied per posting by `_tailor_choice(policy, fit)`: *smart* → tailor, letting the reuse paths skip the Claude call (today's default); *always* → `force_retailor=True`; *under N* → tailor only when the fit score is **below** N (a posting your résumé already fits is sent as-is, spending nothing; a weaker match is tailored to close the gap) — an **unscored** posting is tailored, because "we don't know" must not silently become "send it untailored"; *never* → `tailor=False`, no Claude call at all. The old checkbox is gone from the panel: it is now the *always* choice, so there is one place that decides the résumé, not two. **Storage — no new file, no new schema.** Three optional fields on the existing `DiscoveryFilters` (`tailor_mode`, `tailor_below_fit`, `reuse_threshold`, all defaulting to the pre-178 behaviour so an old `discovery.yaml` loads unchanged), `min_fit` is the field that was already there, and the cap is `safety.yaml`'s **existing** `max_submissions_per_run` — which is why the command-line runner honours the same number. New `safety.save_max_submissions` is a load-modify-save that preserves every other key, `armed` above all: editing a cap must never arm or disarm the system (pinned by a test). The Discovery form's `_discPreserve` gained the three new keys, or saving *that* panel would have silently reset them. **Read once, at start** (`_loop_worker`): a run behaves the way it was configured when it started, and the popup says so — editing mid-run cannot change a run in flight. **The cap ends the run rather than pausing it.** `_loop_submit` refuses past the cap, sets `cap_hit`, and sets the stop event, so the loop stops instead of preparing applications it is no longer allowed to send; the end-of-run message names the cap and where to raise it, and anything already prepared waits under "Ready to apply". It is inert in a dry run (nothing is sent) and does not apply to a single deliberate Apply click, which is not a run. Rejected: enforcing the cap by pausing and continuing to prepare (it would pile up work the user didn't ask for), and gating the loop on `safety.yaml`'s `armed` (decision 176 settled that). **A real bug the browser run caught:** the popup un-hid before its values arrived, so anything typed in that first moment was overwritten when the load landed — every control is now disabled until the saved values are in, and the message reads "Loading your settings…". Out-of-range values are **clamped and shown** rather than rejected (400 → 100 in the box), and an unknown mode is refused with the value named. **Verified in the real app** (Playwright over the real server, both themes, against throwaway config files): load → change mode to *under 80* → save → reopen shows exactly what was saved; the threshold box is disabled unless *under* is chosen; the clamp round-trips; Escape closes; `safety.yaml` came out `{armed: true, max_submissions_per_run: 3, nav_agentic: true}` — the cap changed and nothing else did. **Tests:** new `tests/test_loop_settings.py` (13) — the policy's decision for every mode including the unscored case, the fallback for a config that predates this, the save/load round-trip through the real files, clamping, the refused mode, that a cap edit preserves `armed`/`nav_agentic`, that a missing safety file is created **disarmed**, and that the cap stops a run instead of submitting — plus a test that drives the real `_loop_worker` and asserts the saved policy reaches `run_testing_mode` (`tailor=False`, `reuse_threshold=0.4`). Full suite **795 passed**. | ✅ Implemented |
| 177 | 2026-08-11 | **The search breakdown's Apply / Apply anyway buttons submit — a button that says "Apply" applies, whether or not the loop is running.** User, on decision 176's open question: *"yes, idle case should submit since thats what the buttons are describing"*. **The inconsistency this removes.** After 176, clicking **Apply** on a judged posting did one of two different things depending on invisible state: with the loop running it was queued to the loop thread and submitted; with the loop idle it ran `_judged_prepare` and only prepared, leaving the application in *Ready to apply* for a second click. Same button, same label, two outcomes — the exact failure UI Principle #1 exists to prevent, and decision 174's own user quote (*"prepare then submit"*) always described the submitting one. **What changed.** `_judged_prepare_worker(url, tailor, submit=True)` now finishes the job on its own thread — the thread that owns the browser when the loop is idle: prepare (still a dry-run fill, `gate=None`), then submit the resulting application. The submit body was extracted from `_loop_submit` into **`_armed_submit(app_id, note)`**, shared verbatim by the loop thread and the idle click: same per-click armed one-shot `SafetyGate` (decision 058), same `profile/KILL` check, same pre-submit required-field gate, same ready-list bookkeeping. `_loop_submit` is now a four-line wrapper that adapts the progress callback to the loop status and counts the run's submissions; there is exactly one submit implementation, not two. **A blocked fill is still never submitted** — the submit only runs when the prepared row landed in the ready list, so a fill that stopped on a login or an unanswerable question parks and waits, as before. **The dry-run loop keeps its promise.** When a loop IS running, the click is still queued to it and follows the loop's mode, so a loop in **Dry run** prepares the posting and holds it — "this loop submits nothing" stays absolute rather than being punctured by a side channel. Rejected: making the click submit even through a dry-run loop (it would make the switch's guarantee conditional on which control you touched), and adding a per-click "prepare only" option (nobody asked for it; the loop's Dry run already serves that need — Karpathy #2). **UI.** The click confirms first, naming the posting and the action ("Really apply to Acme — Backend Eng? … will tailor your résumé, fill the form and SUBMIT it"), so the irreversible step is deliberate exactly as the ready-card Apply is. The confirm is skipped only in the dry-run-loop case, where nothing is submitted and a scary dialog would be noise. The button label reads **Applying…** (not "Preparing…") while it runs, the tooltips say the click submits, the queued note says which of the two things the loop will do, and the row's end state reports the submission. **Verified in the real app** (Playwright over the real server, both themes): the confirm names the posting; **cancelling fires no request at all**; with a dry-run loop running the same click raises no dialog and reports "the loop is in dry run, so it prepares this one…". **Tests:** `tests/test_apply_anyway.py` updated to the new behaviour — the below-bar click now asserts the two ordered steps (dry-run fill, then an armed submit) and an emptied ready list — plus a new test that a **blocked** fill is never submitted. Full suite **781 passed**. | ✅ Implemented |
| 176 | 2026-08-11 | **The auto-apply loop APPLIES by default — it submits every application it prepares, with no click per application — and a "Dry run" switch restores prepare-only.** User: *"loop should by default actually apply. it should have a switch to dry run it"*. **The gap this closes.** The loop (decision 069) only ever prepared: tailor → PDF → headless fill → park it in *Ready to apply* and wait for a human to click **Apply ▶** on each one. That made the product's headline claim — five stages, no human in the loop — false at the last step, and it was the step that mattered: a run that found 20 matches still needed 20 clicks. Guideline #3 already anticipated this (*"once armed by the user, per-application confirmation is NOT required — that is the point of the product"*); what was missing was the arming step for the loop itself. **What changed.** `autoloop.auto_apply_loop` takes `apply_immediately` (default False, so the pure core keeps its old behaviour for every other caller); `prepare_one` / `prepare_requested_one` now RETURN the prepared application's id, and the loop calls `submit_one` on it immediately — inside the same batch iteration, before the next match is prepared. A `None` id means the fill came out **blocked**, and a blocked application is never submitted; it still parks and still notifies. `should_stop()` is re-checked between the prepare and the submit, so **Stop** lands before an unwanted send. The web layer passes `apply_immediately=not dry_run`; `start_loop(dry_run=False)` is the new default and `/loop/start` submits unless the payload says `dry_run: true`. **Where the safety sits now.** Nothing about the submit itself changed: it is still `_loop_submit` → the per-click armed one-shot `SafetyGate` (decision 058) → `run_apply`, so `profile/KILL` still halts the very next submit and the pre-submit required-field gate still refuses an unresolved form. What moved is *when the user arms it*: **starting the loop is the arming step**, confirmed once in a dialog that names what will happen ("tailor, fill and SUBMIT … with no further confirmation"), instead of once per application. `profile/safety.yaml`'s `armed:` still governs the CLI runner and pipeline paths — unchanged. Rejected: gating the loop's submits on `armed: true` in `safety.yaml` (the user asked for apply-by-**default**, and a YAML edit is not a default), and a per-run submission cap (the loop already has a user-set **goal**, and a silent stop at 10 would read as a bug). **Three follow-on corrections the change forced, none of them optional.** (1) *Goal counting.* `goal` counted `ready_ids`, but in apply mode an application leaves that list the moment it is sent — the count could never reach the goal and the loop would hunt forever. It now counts submitted + still-waiting, so "stop after 5" means five applications **sent**. (2) *"Keep topping up to the goal"* is a dry-run idea (top up as the user applies); in apply mode the loop applies to them itself, so `maintain` is forced off and the checkbox is disabled with the reason on it. (3) *The approval notification is suppressed in apply mode* — pushing "Ready to apply: open ApplicationBot to review and submit" for an application the loop submits two seconds later is a lie; the blocked/intervention push is untouched. **UI (Principles #1/#3).** The button reads **▶ Start applying** (**▶ Start loop (dry run)** when the switch is on), the panel blurb swaps between "applies to each one for you … real applications are sent" and the old prepare-only text as the switch is toggled, the goal reads "stop after N application(s)" with a hint saying what is counted in each mode, the status line carries an **N submitted** chip beside **N prepared**, and the end-of-run message reports what was *sent* (plus anything left waiting) instead of what is "ready for you to apply". Watch mode's copy no longer claims "it never submits on its own". **Verified in the real app** (Playwright over the real server, both themes): the panel loads in apply mode with the live blurb; toggling **Dry run** swaps the blurb, the button label, and the maintain gate; clicking **Start applying** raises the confirm dialog naming the goal, and dismissing it leaves `/loop/status` `running: false` — no loop, no submit. **Tests:** new `tests/test_loop_apply_mode.py` (13) — the core's interleaved prepare→submit ordering, that a blocked prepare is never submitted, that a Stop lands between the two, hand-picked postings in both modes, `start_loop`'s default and its maintain override, the suppressed-but-still-registered ready state, and two tests that drive the **real `_loop_worker`** with stubs to prove it submits through an armed gate in apply mode and submits nothing in a dry run. Full suite **780 passed**. | ✅ Implemented |
| 175 | 2026-08-11 | **The launchers no longer open a browser: `run.sh` (and therefore `dev.sh` / `restart.sh`) just serves the port, and the user reloads the dashboard tab they already have open.** User: *"change the start and restart scripts to not open the dashboard in chrome anymore: ill just restart the existing open page"*. Removed the backgrounded `sleep 1; open "$URL"` / `xdg-open "$URL"` block in `scripts/run.sh` (lines 113-119). Every restart was spawning a *new* tab in the default browser while the previous one sat there stale, so a day's development left a row of dead localhost tabs; the user's own workflow is to hit reload on the one tab they keep open. No flag was added to restore the old behaviour — the URL is echoed on startup and the shell's own click-through covers the one-off case, so a `--open` switch would be configurability nobody asked for (Karpathy #2). `--window` is untouched: it launches the native pywebview window, not a browser, which is a window the user cannot reload into existence. `ApplicationBot.command` / `ApplicationBot.bat` are also untouched — they run `--window`. Docs corrected in the same change (Guideline #13): `README.md`'s Option B line no longer claims it "opens your browser", and the usage comments in `run.sh` + `restart.sh` say to reload the existing tab. | ✅ Implemented |
| 174 | 2026-08-11 | **Any judged posting can be applied to by hand — including one Claude scored BELOW `min_fit` ("Apply anyway") — and the user chooses per application whether the résumé is tailored first. Every URL in the UI becomes a labelled button.** User: *"there are some applications that the system flags as not being close enough to my resume, but I would like to apply anyway. We should add a apply anyway button, as well as an apply button for ones that passed the fit check. Also, try to never show a plain url: lets try to wrap everything in a button"* — then, asked what the button should do: *"prepare then submit, but with the option to not tailor the resume"*. **The gap.** The fit cutoff was absolute: `pick_top` (pipeline.py) and `cleared_queue` (runner.py) drop everything under `min_fit` before anything is tailored, so a below-bar posting had no path to an application at all — the search breakdown showed it, named exactly why it was denied, and offered nothing to do about it. Claude's judgement is a filter for the AUTOMATIC queue, not a veto over the user. **The choice: manual prepare, not a bypass of the cutoff.** `min_fit` is untouched — it still governs what the loop applies to on its own. The new `/judged/prepare` route prepares ONE user-picked posting: `run_testing_mode(..., gate=None)`, the same dry-run tailor+fill the loop's `prepare_one` runs, landing the row in the same "Ready to apply" queue via the extracted `_mark_ready` helper. **Nothing about the submit path changed** — the armed one-shot `SafetyGate` behind the ready card's Apply button is still the only thing that submits (Agent Guideline #3), so "Apply anyway" is two deliberate clicks, not one. Rejected: a one-click tailor→fill→submit from the breakdown (irreversible with no sight of the filled form), and lowering/bypassing `min_fit` for a click (it would silently retrain the outcome calibration). **`tailor=False` (the user's addition).** New `pipeline.untailored_pdf` sends the user's résumé verbatim with no Claude call: their best-covering UPLOADED document if they have one — with **no coverage threshold**, unlike decision 152's automatic path, because the user asked for their own file explicitly — else the base résumé rendered section-for-section. It outranks every other résumé path *including* `force_retailor`, since it is an explicit instruction rather than a heuristic. Provenance is a third state, not a reuse: labels start with `"Your "`, `reuse.is_untailored` badges them, and the chip reads "Untailored résumé" — claiming a tailoring pass that never ran would be a lie in Track and in the notification. **Concurrency.** A prepare owns the browser, so while the auto-apply loop runs the click is queued to the loop thread — a fourth request queue in `autoloop.auto_apply_loop` (`take_prepare_requests`/`prepare_requested_one`), drained before searching like submits/watches/rescans. **A real bug the tests caught:** `queue_prepare` held `_TEST_LOCK` while `_prepare_reset` re-acquired it — a plain `Lock`, so the first Apply click would have deadlocked the server thread. **URLs → buttons.** Raw URLs were printed as link text in three places (the chosen card, every judged row, the Track `urlCell`); ATS URLs are ~100 chars, wrap into three unreadable lines, and tell the user nothing actionable. All three now render a labelled `Open posting ↗` button with the full URL as the hover title; Track's ✎ still swaps in the raw editable value and the `javascript:`/`data:` guard is unchanged. Two settings links that showed bare domains became labelled buttons ("Create an API key ↗", "Get a free Adzuna key ↗"). The discovery-feeds textarea keeps its raw URLs — that is editable data, not prose. **Also fixed:** the dry-run breakdown re-rendered on every ~1s poll, which would have wiped the "Preparing…" state off the button the user just clicked; it is now signature-guarded like the loop's. **Verified in the real app** (Playwright over the real server, both themes): both buttons render with the right labels, the tailor toggle defaults on, clicking "Apply anyway" on a 41/100 posting shows the spinner, disables the other Apply buttons, and ends with the actionable "review it under Ready to apply" message; the breakdown survives the prepare; `run_testing_mode` received `gate=None` and `tailor=False`. **Tests:** new `tests/test_apply_anyway.py` (11) and `tests/test_untailored_apply.py` (9) — the match index and its bounded eviction, the snapshot fallback, dry-run prepare of a below-bar posting, breakdown preservation, loop queuing, autoloop drain ordering, and the untailored path's precedence over every reuse/force-retailor branch plus a section-by-section check that the verbatim render drops nothing. Full suite **767 passed**. | ✅ Implemented |
| 173 | 2026-08-11 | **Hermes is rejected as a way to route our Claude traffic; the token-cost work stays in-repo — prompt caching on the metered API path, and the tier-3 enrichment call moved to Haiku.** User: *"lets look into if using hermes to route our traffic through would allow us to more easily learn/lower our token cost"*, clarified to *"using hermes to route my claude subscription while we gain the skill/learning features of it"*. **What Hermes is:** Nous Research's Hermes Agent, whose `hermes proxy` exposes a localhost OpenAI-compatible endpoint backed by an OAuth provider session, plus an agent-loop skills system (`/usage`, `/insights`, "save what you just did as a skill"). **Four findings, each sufficient to reject:** (1) **It cannot carry the primary path.** All 21 LLM call sites funnel through two functions in `backends.py`; the primary is `run_claude_cli`, a `subprocess.run(["claude", ...])` against the Claude Code CLI on the **subscription**. A proxy can only intercept that via `ANTHROPIC_BASE_URL`, and subscription OAuth will not authenticate against a third-party endpoint — the same restriction already documented at `backends.py:15-16`. Only `run_anthropic_api` (the *fallback*, used when Claude Code is absent) is proxyable. (2) **Nous's own docs say the subscription path is not open to us:** *"Claude Pro subscribers cannot use this path; the supported alternative is an `ANTHROPIC_API_KEY`, billed pay-per-token."* Claude Max can OAuth but consumes **purchased overage credits**. Either way routing converts free-within-window subscription usage into metered tokens — cost goes **up**. (A third-party blog claims Pro works via `hermes proxy`; the official docs are treated as authoritative over it.) (3) **The skill/learning features are agent-loop features, not proxy features.** They live in interactive Hermes CLI sessions. ApplicationBot makes single-shot, schema-constrained calls with fixed prompts and no conversation state — there is nothing for a skill to accumulate. Routing through the proxy buys the transport, not the agent. (4) **We already have better cost telemetry.** `usage.py` (decision 095) captures input/output/cache tokens + `total_cost_usd` per call, tagged by activity and posting, into `usage_events`. Hermes's `/usage` is per-session and `/insights` is a 30-day rollup — strictly coarser than what the Track tab already joins per application. **Also noted:** routing subscription auth through a third-party proxy is a ToS risk against the user's own Claude account for zero capability gain (Guideline #4). **What was built instead, on the two levers the user picked.** *(a) Prompt caching on the metered API path.* `run_anthropic_api` now sends its system prompt as a `cache_control: ephemeral` block, and `_user_message` was split from one string into `(stable prefix, per-posting suffix)` — the base résumé vs the JD/length budget — so `AnthropicAPIBackend.tailor` sends two user content blocks with a cache breakpoint after the résumé. Both are byte-identical across every posting in a run, so from the second posting on they bill at cache-read (~0.1x) instead of full input price; the 5-minute TTL comfortably covers a ~40s tailor. `ClaudeCodeBackend` joins the two halves and is byte-for-byte unchanged (Guideline #7). Blocks below the model's minimum cacheable prefix silently don't cache — no error — so the short system prompts other callers pass are safe. **Verified** by driving `AnthropicAPIBackend.tailor` against a stubbed SDK and inspecting the real payload: system block 5,946 chars with `cache_control`, user block[0] = résumé with `cache_control`, block[1] = JD uncached. *(b) Per-activity model tiers.* Audited all 21 call sites: tiering was **already mostly done** — `matching.PRERANK_MODEL`/`inbox_import.MODEL`/`answer_bank.DRAFT_MODEL` are `haiku`, `matching.JUDGE_MODEL` is `sonnet`. One real gap: `enrich.claude_llm_extractor` (tier 3 of the JD cascade) passed no `model` and ran on whatever `claude` defaults to, despite being verbatim read-and-extract over a large page-text input — now `enrich.MODEL = "haiku"`, the same rationale `inbox_import` already documents. **Deliberately left alone:** `impact.score_projects` (rates project impressiveness 1-5, feeds résumé ordering — a judgement call that changes tailoring output, and it runs once per résumé, not per posting) and `resume_import` (rare, quality-critical). Downgrading either would trade output quality for pennies. **Scope note:** the caching win lands on the *fallback* engine, so it does not reduce this user's own spend — their traffic goes through the subscription — but it is a real cut for anyone cloning the repo without Claude Code, which is the product's stated audience. **Tests:** full suite **747 passed**; no README change (nothing user-facing moved — Guideline #13). | ✅ Hermes rejected; both levers implemented |
| 171 | 2026-07-30 | **Oracle Recruiting Cloud is reachable and fillable — no account and no dead end — and getting there fixed four defects in SHARED code, not in anything Oracle-specific. It stays out of `FILLABLE_ATS` until the address block completes.** User: *"lets see if we can get oracle recruiting working"*, after decision 168 measured ORC at **3,553 postings in the curated feeds — more than Greenhouse (3,026), 2.7x iCIMS, and the single largest block the pipeline drops.** **Probed first, three unrelated tenants** (Oracle CX_45001, American Express CX_1, ADT CX_1 — two host shapes, `<tenant>.fa.us2.oraclecloud.com` and `fa-<tenant>-saasfaprod1.fa.ocs.oraclecloud.com`, so `detect_ats` keys on the `oraclecloud.com` suffix, not the tenant). All three run the same flow: posting -> APPLY NOW -> `/apply/email` -> the application. Its auth screen says **"You don't need to have an account"**, which is what separates ORC from decision 168's iCIMS/Taleo/Avature: it is email-VERIFIED, not account-GATED. **Four defects, every one in code shared by every ATS.** (1) **`_count_fields` counted controls nobody can see** — an ORC posting page carries a collapsed Oracle Digital Assistant chat box, a hidden file input and a hidden copy-link box, so 2-3 "fields" made the poll declare the form found, return the posting page, and never click APPLY NOW; **2 of the 3 tenants never left the posting page**. It now counts only laid-out controls (offsetParent or a client rect), which still counts the 1px/opacity-0 react-select inputs the fill drives on purpose. (2) **Oracle's honeypot is invisible to decision 168's two signals**: it is named `honey-pot`/aria-labelled `honeypot` with **aria-hidden on the input itself**, so the label rule misses it and the aria-hidden skip (which exists for react-select mirrors) swallowed it before any honeypot check ran. It was coming back empty by luck; a tenant that laid it out would have had it filled, which is how a submission gets silently binned. `_is_honeypot` gained a name/id/aria-label tell and now runs FIRST, before both skips, so the trap is refused and NOTED. (3) **The terms checkbox is a 0x0 input behind a decorative `<span>`** — `.check()` times out and `check(force=True)` fails outright ("Element is outside of the viewport"), so a REQUIRED box stayed unticked on every tenant. Worse, the existing JS fallback set `.checked = true` and reported success while Oracle's own validation still refused with *"You need to agree to the terms and conditions"* — **a false positive the report believed**. `_check_radio` becomes **`_force_check`** (it was never radio-specific) and gains the step that actually works: click the LABEL, which the browser forwards to the input so the site's component sees a real click. Clicked at the label's top-left corner on purpose — ORC's label contains a *"terms and conditions"* LINK in the middle, and a centre click would navigate away. The JS fallback stays last, for Lever's hCaptcha overlay, with its false-positive risk documented. The two checkbox call sites now use it too. (4) **A cookie-consent overlay eats every click while it is up.** New `_dismiss_consent_banner`, called once when the form is found: least-consent first (Reject All / Only necessary / Decline), accepting only when refusing is not offered — because the alternative is leaving a form the user cannot submit — and always recorded in notes, since it is a click made on the user's behalf. A no-op on every ATS we already fill. **Verified end-to-end on a live posting** (Linamar *Product Engineer* — deliberately a low-stakes tenant, not AmEx; the user authorised using their real bot inbox for this): posting -> APPLY NOW -> email gate filled (email, terms ticked, honeypot refused, banner dismissed, **0 errors**) -> Next -> Oracle emailed a **6-digit code**, read from the bot inbox via the existing `mailbox.extract_verification` (sender `…@…oraclecloud.com`; the walk deliberately waits for a code NEWER than the last message, since a stale one is still in the inbox) -> `/apply/section/1`, the real form -> **7 fields filled by the generic engine, 0 errors**. **SUBMIT was never clicked.** Note the flow is not deterministic on first contact: an unrecognised email went straight to the form, and only a later attempt was challenged for a code. **Why `oracle` is NOT in `FILLABLE_ATS` (pinned by a test).** Six required fields remain unanswered, and they are three separate causes, only one of which is an engine gap: **(a) profile data nobody has entered** — `street_address`, `postal_code` (both added by decision 168, still blank) and `desired_salary`, which is 3 of the 6; **(b) Oracle's address block is a DEPENDENT dropdown chain** — the form states it itself (*"you may need to edit the value in their parent drop-down list"*), and City/State offer no options at all until Country and ZIP are set, so (a) blocks them and it is UNVERIFIED whether they resolve once it is filled; **(c)** the verification step lives only in the probe script — `apply.py` does not yet call `mailbox.wait_for_verification`. Letting 3,553 postings into the pipeline to stall at an address box would be worse than dropping them, so the gate stays shut and honest until (a)-(c) close. **Tests:** new `tests/test_oracle_recruiting.py` (9) over two new fixtures — `oracle_email_gate.html` (built from the live DOM: the aria-hidden honeypot and the 0x0 checkbox behind its span, both reproduced exactly, because a fixture that omitted them passed while the real page failed) and `consent_banner.html` (overlay + the collapsed widgets that broke the field count). Covers both host shapes, the visible-only field count, the gate filling with the honeypot refused and reported, `_force_check` on a 0x0 box, least-consent dismissal, that dismissal unblocks the control underneath, that a page with no banner is untouched, and that `oracle` stays out of `FILLABLE_ATS`. Full suite **747 passed**. | 🟡 Partially implemented — reachable and filling; address block open |
| 170 | 2026-07-30 | **Simplify's autofill engine is rejected as an Apply-stage dependency; the only Simplify surface we integrate stays its public GitHub listings feed (already shipped, decisions 031/073). What we take from it instead is its published portal list, as a target list for our own ATS coverage.** User: *"look into if we can integrate simplify into our workflow since they already have a robust autofill workflow"*. **What was checked.** [simplify.jobs/copilot](https://simplify.jobs/copilot), the Chrome Web Store + Firefox AMO listings, `help.simplify.jobs` (installing/using Copilot, "Autofill Not Supported?"), and their Terms. **Seven findings, each one on its own sufficient to reject:** (1) **No API** — the sole interface is an MV3 browser-extension content script; no developer docs, no programmatic access, no paid tier that would buy one (the product is free and stays free). (2) **It never submits** — their own help pages and store listings describe filling fields and advancing multi-page wizards; the final Submit is a human click. That is precisely the half we already own (`apply._attempt_submit` + `safety.SafetyGate`, decision 035), so the integration would supply the easy half and stop at the irreversible one — the opposite of Guideline #3's shape. (3) **Wrong data source** — it fills from the profile stored in *their* cloud, not `apply_profile.yaml`. Keeping the two in sync means pushing the full PII set to a third party (Guideline #5) and, with no API, scripting their own web UI to do it — the same form-filling problem relocated, with no sanctioned path. (4) **It would blank the Tailor stage** — it uploads the résumé held in their account, so the per-posting tailored PDF, the `answer_bank` answers, and the per-posting review overrides (decisions 153/155) cannot be injected. Stage 3 of the five-stage pipeline goes dark. (5) **Blind submit** — the pre-submit required-field DOM scan, `submit_probe`, and the `blocked` outcome all depend on knowing field-level fill state. Arming an irreversible click on a form a black box filled inverts the safety architecture. (6) **Terms + account risk** — their Terms prohibit bot-registered accounts and "any automated process to extract, harvest or scrape", and grant a revocable, non-transferable, no-source license. Driving the extension under Playwright on the user's real account risks that account for zero capability gain (Guideline #4). (7) **Recurring manual step** — an MV3 extension needs `launch_persistent_context`, a non-headless Chromium, a side-loaded CRX we would have to redistribute, and a Simplify session that expires and needs a human re-login (Guideline #8). **Consistent with the prior finding**, decision 024 already rejected Simplify (with Teal/Huntr) on the *tracker* side for "no public API, human clicks"; the same wall stands on the apply side. **What we keep**: `CuratedListSource` reads their public `New-Grad-Positions` + `Summer2026-Internships` `listings.json` from `raw.githubusercontent.com` — open data, no account, no ToS surface, 3,459 active postings, refreshed daily. That integration is done and unchanged. **What we take**: their published portal list (Workday, Greenhouse, Lever, SmartRecruiters, iCIMS, Taleo, Avature, BambooHR, Jobvite; "100+" total) read as a coverage checklist against ours — we fill six public-API ATSs (`ATS_SOURCES`) + Workday's deterministic adapter + the generic DOM fallback, and gate iCIMS non-fillable in `pipeline._is_fillable`. The gap (iCIMS, Taleo, Avature, BambooHR, Jobvite) is queued in NEXT_STEPS, iCIMS first because we already detect it and the curated feeds link to it. **Docs-only decision** — no code changed. | ✅ Logged (no code) |
| 169 | 2026-07-30 | **A field is identified by a KEY (its label, plus " #n" for the n-th control showing that label), separate from the QUESTION the key names. Wizard pages get a fresh done-set.** User, after decision 167 flagged the limitation: *"look into that architecture change: we might as well get it out of the way now instead of having to do a much more costly rewrite later"*. **Measured before choosing.** Across the 20 archived real applications the only repeated labels were checkbox-group rows (one per checked option, by design) and one filled row plus its own "Other" note — no genuine collisions. Live, with the fill's own visibility filter: Lever 21/21 distinct labels, Ashby 5/5. **Then the decisive measurement — is a DOM-identity key viable?** Loading each form twice: Lever 87/87 controls carry a `name`/`id` and the key list is *identical* across loads, but Ashby's radio group is keyed by a **per-render UUID** (`96b38c4a-…` → `5fe56b1a-…`), so a DOM key would silently detach that field's saved edit and required mark on every re-fill. **Options.** (a) **Chosen — occurrence-suffixed key:** the second control showing a label becomes `Date #2`. No schema change, no migration (99% of keys stay byte-identical; only colliding fields get a new one), and it composes with decision 167 — a `#2` row already carries its own *"on the form: …"* context line, so a disambiguated row explains itself. (b) **DOM-identity key** (new `field_key` carried through report/overrides/panel, question text demoted to display data): 2–3× the work, needs a migration layer for existing archives and `answers.json`, *and* a label-fallback matcher anyway because of the Ashby churn — i.e. the "costly rewrite later" is a rewrite the evidence argues against at any point, not a debt that grows. (c) defer. **The conflation this fixes.** One string was doing two jobs: *field identity* (which control on this form — `done`, `report.filled/captured/required/context`, the per-posting edits) and *question identity* (cross-posting and semantic — the answer bank, classification, drafting). They are now split by `_question(key)`, which strips the disambiguator: the resolver's rules, `banked_qa`, `option_hints`, `classify_question(s)`, `match_banked_question(s)`, `generate_answer` and `choose_required_option` all see the QUESTION (a second "Date" box is still a date question, and two fields asking the same thing cost **one** batched classification between them), while the user's edit and the field's surrounding text are looked up by the KEY, so correcting one of two same-label fields can't silently rewrite the other. `answer_bank.is_context_dependent` now also refuses any field key outright — `Date #2` names one control on one form and must never enter the shared bank. **Numbering is positional, not a running counter,** because `_fill_page` runs the same loop twice (round 1 defers, round 2 fills): `_KEY_FN_JS` counts same-label controls in document order, so a control's key is identical in both rounds and on a later re-fill. `_REQUIRED_MAP_JS` embeds the same function — otherwise the required marks could not be joined onto the fill's keys. Label derivation is memoised on the element (a JS property, not an attribute) so counting stays O(n) per page rather than O(n²). Radio/checkbox OPTIONS are deliberately excluded: they legitimately share their group's question. **The cross-page bug found while tracing this:** `done` was shared across every page of a wizard, so a label appearing on page 1 and again on page 3 was skipped on page 3 — it stayed empty and the pre-submit gate blocked the run. Each page now gets its own `done` (the two rounds within a page still dedupe), and `_merge_checkbox_groups` folds rows that are identical in label, value and control, so the same question answered on two pages is one row in review rather than two edit boxes writing to one key. **Verified:** new `fixtures/apply_forms/repeated_labels.html` (two pre-rendered education blocks) filled live — both blocks answered, keyed `School`/`School #2` and `Email`/`Email #2`, with the required sweep marking the first School and the second Email exactly as the fixture does; `tests/test_field_keys.py` pins the key/question split, the per-field edit and context, and the bank refusal. A real re-fill of the live Palantir posting produced **35 keys, all byte-identical to the previous archive** and no `#n` — the change is invisible where nothing collides. Suite **738 passed**. | ✅ Implemented |
| 168 | 2026-07-30 | **The queued ATS coverage, re-ordered by what the portals actually serve: Jobvite and BambooHR become fillable; iCIMS, Taleo and Avature are refused at the account wall instead of being half-filled.** User: *"lets implement the ATS coverage, then commit"*. **The plan was wrong and the probe said so (Guideline #0).** Decision 170 queued iCIMS first on volume. Before writing a handler I ran a **read-only** probe — real postings taken from the curated feeds, driven through our own `_open_application_form`, filling and submitting nothing — and it split the five portals in two: **Jobvite** (`jobs.jobvite.com/visionist/job/oEBdAfwM`) reveals a public `<posting>/apply` form, 16 labelled fields; **BambooHR** (`alkira.bamboohr.com/careers/232/`) renders the form on the posting page, 17 fields; while **iCIMS** (Lynker, Brookings) redirects to `<posting>/login` (email + hCaptcha), **Taleo** (AAR) to `careersection/iam/accessmanagement/login.jsf`, and **Avature** (Delta) to `careers/ApplicationMethods` — all three an account wall, every tenant sampled. So the two "cheapest, smallest" portals are the ones we can finish today, and the 1,311-posting prize is gated behind the multi-week account work, not behind a form handler. **The volume numbers** (counted across both curated feeds, 2026-07-30): iCIMS 1,311 · Taleo 152 · Jobvite 104 · Avature 26 · BambooHR 22. **Surfaced by the same count and NOT acted on: `oraclecloud` at 3,553 — more postings than Greenhouse (3,026) and the single largest block we drop.** It was not on Simplify's list, so it was not in 170's queue; queued in NEXT_STEPS rather than folded in here. **Shipped — detection and gating.** `apply.detect_ats`, `apply._ats_from_frame` and `discovery.detect_ats_from_url` learn all five. `discovery.FILLABLE_ATS` (the six board-API ATSs + Workday + Jobvite + BambooHR) and `discovery.ACCOUNT_GATED_ATS` (iCIMS/Taleo/Avature) replace `pipeline._is_fillable`'s inline literals — "can we discover it" and "can we fill it" are now separate, readable sets, and the gated three keep costing zero judge tokens. **Shipped — the account wall is refused, not filled.** A sign-in step has 2-4 boxes, so `_find_form_frame` reads it as a short application form: the run would have put the résolver over "User Name" / "Password" and **banked them as screening questions**. `_account_wall_evidence` proves the wall from a visible password box or a URL that IS the sign-in step (`/login`, `/login.jsf`, `/accessmanagement/login`, anchored at the end of the path so a `senior-login-systems-engineer` slug can never match), and `_open_application_form` refuses with the portal named and the fix stated. **Scoped to `ACCOUNT_GATED_ATS` only, on purpose (Guideline #7):** Workday creates an account mid-application by design and decision 059's adapter handles that password page, so a global refusal would break a portal that already works. The refusal already parks as `login` → "Store the login" → the credential store, which is the existing machinery, not new UI. **Shipped — two things BambooHR's real form does that no fixture covered.** (a) A **honeypot**: a genuinely CSS-visible, labelled text box (`nickname_hpcsaf`, "Please leave this field blank") hidden from people only by `aria-hidden` on its *wrapper*. A visibility check cannot see it; filling it is how a submission gets silently binned. `_is_honeypot` matches either signal — an aria-hidden ancestor, or a label that instructs you to leave it empty — and the field is skipped, **noted**, and never captured as a question for the user. (b) A **button-facade select**: the native `<select>` is `aria-hidden`, `readonly`, height 0 and carries **no options**, while the user drives `<button aria-haspopup>` → `[role=menu]` > `[role=menuitem]`. `_fill_facade_selects` is a new pass beside `_fill_radio_groups`/`_fill_checkboxes` rather than a branch inside `_fill_all_fields`, because the control is a `<button>` that loop never enumerates. Without it BambooHR's REQUIRED State and Country stay empty while the page looks answered. `_open_options` gained a `[role=menuitem]` tier (last, so a real listbox still wins). The facade's nearest-ancestor XPath is load-bearing: anchoring on the `<form>` handed every facade on the page the first toggle in document order — Country would have been set to a state. **Shipped — the split address block, which is what actually blocked both portals.** Jobvite and BambooHR both ask Address / City / State / ZIP as four separate **required** boxes and the resolver could answer none of them: `Address` → None, `State` → None (the "which state do you live in?" rule deliberately won't match a bare `State*`), `ZIP` → None, and `City` → the whole `"Edison, NJ"`. Two new profile fields — **`street_address`** and **`postal_code`** — because those two are genuinely new user data; city and state are **derived** from the existing `location` (`_city_only`, `_state_from_location`), so nothing is asked of the user twice. Profile-screen inputs and the save round-trip added. The `Address` rule is guarded against `url`/`web`/`linkedin`/`ip ` (and can never see "Email Address" — the email rule returns first). **One deliberate behaviour change, called out rather than folded in (Guideline #7):** a `City` field now answers `"Austin"`, not `"Austin, TX"`. `fixtures/resolver_corpus.yaml` pinned the old value and this flipped it; the corpus entry is updated in the same commit, with the reason — its own two neighbours are the separate `State/Province` and `Current location` fields, so that case was always a split address block and `"Austin, TX"` was never a right answer in a City box. `"Where are you based?"` and `"Location (City)"` still return the pair. **Verified against the live forms, not only fixtures** (the point of the exercise): a real dry fill of `alkira.bamboohr.com/careers/232/` fills **10/10** answerable fields including the facade State, leaves the honeypot empty and says so, and skips only Date Available / Desired Pay / Website (genuinely unanswerable); `jobs.jobvite.com/visionist/job/oEBdAfwM` fills **9**, address block included, with **no Jobvite-specific code**; Brookings (iCIMS) and AAR (Taleo) are refused with the portal named and park as `login`. That live run is also what caught the last defect — `_pick_from_open` reads only the first 30 options and "New Jersey" is #33 in a US state menu, so the fixture passed while the real form did not; `_best_option_index` was extracted from it and the facade pass reads up to 250. **Tests:** new `tests/test_ats_coverage.py` (19) over three new fixtures (`bamboohr_apply.html` with the honeypot and both facades, `jobvite_apply.html`, `account_wall.html`) — detection, the gate, the address block and its lookalike guards, the honeypot, the facade fill, the prefilled-facade native path, the refusal, its `login` parking, and that the refusal stays scoped to the gated three. Full suite **738 passed**. | ✅ Implemented |
| 167 | 2026-07-30 | **A field whose label doesn't say what it wants is answered from the text around it: the fill reads the field's neighbourhood off the page and uses it in the rules, in every model call, and in the review panel.** User: *"Also make sure that fields like those use the context around them to figure out what should be in them"* (following decision 166). **The gap.** Every answer was derived from one string — the label `_LABEL_JS` derives. That works for *"Are you legally authorized to work in the United States?"* and fails for the labels that actually go wrong: `Date`, `Name`, `Other`, `If yes, please explain`, `Please specify`. Those name a *format* or a *follow-up*, not a question; the form says what they mean in the heading above them and the field before them, and we were throwing that away. Decision 166 fixed the bare `Date` with a default (today) — correct for a signature block, wrong inside an education or employment block, and nothing in the system could tell those apart. **What is captured.** New `_CONTEXT_JS` returns a short, capped string for one control: its `aria-describedby` help text, the heading/legend of each block it sits in, the nearest text blocks before it that hold no form control of their own (a sibling *field's* text describes that field, not this one), and the label of the field it follows — *"a Date after a Signature is a signature date"*. Fragments are ≤180 chars, the whole string ≤300, and a fragment equal to the field's own label is dropped (a field's `<label>` is a previous sibling of its input, so it would otherwise lead every context string with the label we already have). **When it is read:** only for a **generic** label (`_is_generic_label`, matched on the whole normalised label — `Date` is generic, `Date of birth` is not) or for a field **nothing structured could answer**. Every other field costs exactly what it did before; a form reads ~6 extra `evaluate`s, once each (an empty read is recorded as "already looked"). **Where it is used.** (1) The signature-date rule became `_signature_date`: today *unless* the surroundings say the box belongs to the applicant (birth, graduation, employment, school, document expiry) — then it returns None and the field is captured for them, because a wrong date is worse than a blank one they are told about. (2) Every model call now carries it as a prompt block: `classify_question(s)`, `match_banked_question(s)`, `generate_answer`, `choose_required_option` — explicitly framed as *"use it to tell WHICH question is being asked; it is NOT the question, and never the answer"*, so a classifier that saw only `"Date"` now sees the block it lives in. (3) It is archived on the report and shown in the review panel under the label (*"on the form: Applicant certification · follows the field: Signature"*) — a bare `Date` row is not reviewable, that row is. **The re-poisoning loop, closed.** New `answer_bank.is_context_dependent` (delegating to `_is_generic_label`) now gates `is_reusable_answer`, `valid_mapping` and `apply_profile.remember_answers`: a generic label can never enter the shared bank again, in any code path. That is what made decision 166's bug recur across postings — the first wrong fill was *learned* as a bank entry keyed on the word "Date". **Design chosen over the alternatives.** Rejected **qualifying the label itself** (*"Date (Applicant certification)"*): the label is the join key for saved edits, captured options, required marks and the answer bank, so rewriting it invalidates existing archives and every rule that matches a label whole. Rejected **a model call per ambiguous field** to interpret it: non-deterministic, and the DOM already holds the answer. The context therefore rides *alongside* the label, in a `label -> text` map on the resolver (like `seen_options`/`decided_options`), so no function signature in the fill path changed and the review panel's preview resolver — which has no browser — behaves exactly as before with an empty map. **Known limitation, unchanged:** answers are keyed by label, so a form with two identical bare `Date` boxes still fills only the first. **Verified on the real thing:** a headless re-fill of the live Palantir posting reads *"SELF-IDENTIFICATION OF VETERAN STATUS … · follows the field: Name"* around its `Date` (correctly today), and *"LINKS · follows the field: GitHub URL"*, *"OFFER DEADLINES · …"*, *"YEAR AT PALANTIR - ROLE INTEREST · …"* around the questions it could not answer — which is exactly what the classifier and the drafting path now see. New `fixtures/apply_forms/context_dates.html` proves the veto live: the same bare `Date` label that gets today in a signature block is left for the user inside an education block. Suite **711 passed**. | ✅ Implemented |
| 166 | 2026-07-30 | **A form's signature "Date" is answered with today; a short label can no longer take a long banked question's answer; and the review panel flags any answer whose shape doesn't fit its question.** User: *"there are many fields filled out technically correctly but not the right context: date on the bottom (meant to take in the day you apply, had a string instead saying when i am free). Let's make sure that these are easy to catch in the review tab, and also fix it at the source"*. **Reproduced from the user's own archive** — three Palantir dry-runs filled the signature-line `Date` box: two with `"Yes"`, one with `"I'm available immediately, with confirmed graduation in May 2027."`, all `source: resolver`. **Root cause (one line).** `AnswerResolver.banked_qa` matched a label against a banked question by containment with the length guard on **one side only**: `len(qn) > 15 and (qn in n or n in qn)`. The 4-character label `"date"` is a substring of *any* banked question containing the word — so it took the answer to *"What is your earliest available start date?"*, and on another posting the answer to a *"…receive up**date**s…"* opt-in (`"Yes"`). The wrong answer was then re-learned into the bank as a literal `Date` entry, so it would have repeated on every future form. **Three fixes at the source:** (1) containment now requires **both** sides > 15 chars — a short label must match a banked question exactly or not at all; (2) a new `_is_todays_date_q` rule answers a form's own date-of-signature field (`Date`, `Today's date`, `Date signed`, `Date of application`, …) with `date.today().isoformat()`, placed **after** the start-date rule (so *"date available to start"* still answers availability) and **before** the answer bank, which is what was answering it wrongly. It matches the label **whole**, never as a substring, so every date *about the applicant* — birth, graduation, start — fails it and stays theirs to state; (3) a native `<input type=date>` is now filled from that structured answer (`YYYY-MM-DD` only, never a drafted sentence, which the control would reject) instead of being reported *"unsupported field type"*. `scripts/prune_answer_bank.py` gained the retroactive half — it drops a **generated** bank answer that a structured rule now answers — and clearing the poisoned `Date` entry from the user's profile was one `--apply` run. **Why the review panel needed its own fix.** This class of bug is invisible in review precisely because the field *is* filled: every row looked alike, so a plausible answer to the wrong question read as success in the table and in the screenshot. New `web._answer_flag` checks each answer's **shape** against its question — date fields answered with prose, number/GPA questions with no digits, email/phone/URL fields with the wrong shape, a Yes/No question answered with a sentence, and the inverse (a *which/why* question answered `"Yes"`). Precision over recall: each rule fires only where the expected shape is unambiguous (start-date questions are excluded — *"Immediately"* is a real answer; an option the **form itself** offered can never be off-shape; a long question that merely mentions dates is a prose question), so a flag always means *look at this*, never *the bot was unsure* — that is what the unanswered table is for. Flagged rows get an amber row, a **Check this** badge and the reason **above the edit box that fixes it** (UI Principle #3), with a count at the top of the section; model-produced answers now also carry an **AI-drafted** / **AI-picked** chip, since those are the other rows most likely to be right in form and wrong in context. **Rejected:** having Claude judge each answer against its question — a model call per field per fill, non-deterministic, and it would flag on taste rather than on a defect the user can see. **Verified on the real thing:** run against the user's four archived Palantir applications, the flagger caught all three bad `Date` fills *and* a fourth wrong-context answer they hadn't reported (a *"Which of these roles resonates the most … and why?"* question answered `"Yes"` — a pre-decision-159 archive); a live headless re-fill of the real Palantir form then put `2026-07-30` in that `Date` box, and the panel shows zero flags for it. New `tests/test_wrong_context_answers.py` (35 cases: the resolver rule, the containment bug isolated, a live fill of a new `fixtures/apply_forms/signature_date.html` proving the signature date fills and the applicant's own date fields don't, the shape-check table, and the panel highlighting headless). Suite **700 passed**. | ✅ Implemented |
| 165 | 2026-07-30 | **A dropdown question is captured and reviewed as a dropdown: answered single-choice fields record their options, and the review panel edits them with the form's own list (with a typing escape hatch).** User: *"dropdown support seems to not be working: a field is asking for a dropdown in the actual application while showing up as a text box in the review"*. **Reproduced from the user's own archive** (`palantir-software-engineer-new-grad-defense/report.json`, 2026-07-30 10:45): four `select` answers — *Year of High School Graduation*, *Which university…*, *Please include your intended graduation year*, *Please tell us how you heard about this opportunity* — are in `filled`, and **none of them appears in `captured`**. Two independent bugs, both real: **(1) Capture.** `_record_capture` ran only on the paths where we could NOT answer a field (plus checkbox groups), because its only consumer was the answer bank's "capture blank questions for the user". Once the review panel started rebuilding the real control from `captured` (decisions 153/156), every ANSWERED dropdown lost its options — the panel had nothing to build from. Now a native `<select>` records `_selectable_options` **however the fill turns out** (one `evaluate`, no menu to open — its `<option>`s ARE the complete choice set), an answered radio group records the option labels it already reads to match the answer, and the required-dropdown model-pick path records the list it chose from. Placeholders are excluded — offering *"Select …"* as an answer would commit an empty selection. **Comboboxes are deliberately narrower:** a react-select list is recorded only when the answer matched an option **shown on open** (new `AnswerResolver.seen_options`, set at the two phase-1 commits). A searchable picker's open list is an alphabetical prefix, not its choices (decision 080) — a school typeahead never matches there, so it records nothing and stays a text box, which is the correct editor for an unbounded list. Re-opening a menu just to enumerate options was rejected: expensive, flaky, and for a typeahead it would capture a misleading list. **(2) Render.** `answerRow` special-cased only check-all-that-apply groups; everything else fell through to a text/textarea box — so even the fields that DID have captured options (an unanswered `Veteran status`, every Yes/No radio card) were edited as free text, and a typed answer that matches no option fills as blank or forces the fill to guess. New `isSingleChoice` + `singleChoiceInput` render the form's list as a `<select>` mirrored into a hidden input, so `saveAnswers` reads `.value` unchanged. **Why the escape hatch:** a captured list can be short of the form's real one (`_record_capture` caps at 40, `_open_options_and_texts` at 60, and a posting can change its options), so replacing a text box with a closed list could have made a field *un*-editable — a regression. Every dropdown therefore ends with **"Type a different value…"**, which swaps in a text box, and a one-click way back to the list; a stored value the form no longer offers is kept and labelled `(not in this form)`. **Existing archives stay text boxes until rescanned** — they were written before this and carry no options; decision 164's **Rescan questions** is the fix, and the panel already points at it. **Verified:** new `tests/test_review_dropdowns.py` — the live fill of `lever_custom_cards.html` proving answered EEO `<select>`s and Yes/No radio cards now record their options (and the placeholder does not), plus headless panel tests for editing a dropdown answer as a dropdown and for the typing escape hatch round-tripping into `answer_overrides`. Suite **667 passed**. | ✅ Implemented |
| 164 | 2026-07-30 | **Required vs optional is swept off the live form once per page and archived with the fill, and the review panel gains a headless "Rescan questions" that re-reads the form on demand.** User: *"when reviewing the questions for an application, the user should be able to know which ones are required and which ones aren't so its clear what needs to be filled out to submit and which ones are optional. Also add option to rescan the posting/application to refresh the question types/answers"*. **The gap:** `apply.py` already knew about required-ness in three separate places — `_is_required(loc)` (per control, used to force a draft), `_REQUIRED_LABELS_JS` (per page, used to flag unfilled required fields) and `_UNMET_REQUIRED_JS` (the pre-submit gate) — but none of it ever reached the user. `report.filled`/`skipped` carry no such flag, so the review panel listed every answer identically and the only signal was the wording of a skipped line. A user could not tell whether the blank box in front of them blocked the submit or was a voluntary EEO field. **Options for capturing it:** (a) thread a `required` flag through every `report.filled.append` site — ~12 call sites across `_fill_all_fields`, `_fill_radio_groups`, `_fill_checkboxes`, `_upload_resume` and `workday.py`, each needing its own `_is_required` evaluate (one round-trip per field); (b) **chosen — one JS sweep per form page** (`_REQUIRED_MAP_JS` → `_record_required`) returning `{question: isRequired}` for every visible control, reusing the *existing* `_LABEL_JS`/`_GROUP_QUESTION_JS`/`_IS_REQUIRED_JS` so its keys are the same question strings the fill reports answers under, merged across wizard pages and archived as `report.required`; (c) re-derive it in the browser when the panel opens — impossible, the panel is server-rendered from the archive and the posting may be gone. (b) is one evaluate per page instead of one per field, adds no argument to any fill path, and covers questions that were never filled at all. **Honesty rule:** a question the sweep never saw is **absent** from the map, and the panel shows it with no badge — it is never called "optional" on the strength of not being marked. Reports written before this (and Workday runs, which fill through their own driver and never reach `_fill_all_pages`) carry no map at all; `_review_data` reports `required_known: false` and the panel says so and points at the rescan, rather than showing a wall of unmarked fields. **Rescan:** a HEADLESS dry-run re-fill (`headed=False, pause=False, gate=None`) of the same posting with the stored PDF — it rewrites `report.json`, which is the panel's only source, so questions, control types, required marks *and* answers all refresh in one action; the user's saved per-posting edits are loaded by `run_apply` itself, so a rescan never reverts them (and unsaved edits are flushed first, like Watch/Apply). It is queued onto the loop thread when the loop is running (new `take_rescan_requests`/`rescan_one` hooks in `autoloop`, mirroring watches) because that thread owns the browser — refusing would have made Rescan the one panel button that doesn't work while the loop runs. The panel detects completion by the archived report's **timestamp changing**, which works identically for the direct and the queued run, then re-renders itself in place. **Also fixed here:** `start_reapply` referenced an undefined `mode` (`_TEST_STATE["mode"] = mode`), so every re-apply/watch with the loop idle raised `NameError` before starting — uncommitted breakage from the session that added the dry-run modes, on the exact path a rescan reuses. **Verified:** new `tests/test_required_marks.py` — the sweep driven live against the committed `lever_custom_cards.html` (✱-marked, `required`-attribute, radio *cards* keyed by their question not "Yes"/"No", and genuinely optional EEO selects), the `_review_data` join incl. the unknown case, the panel's badges/counts headless, and a stubbed-fill rescan proving the panel re-renders from the new report; plus two `autoloop` tests for the drain order. Suite **664 passed**. | ✅ Implemented |
| 163 | 2026-07-30 | **The Review & tailor tab is deleted; tailoring becomes a choice on Discover's dry run — job source (best match found · a posting I paste) × how far to go (tailor + fill · tailor only).** User: *"we can get rid of the review tab altogether: make it a dry run option (you can choose to dry run the application or just the tailoring step)."* **Why the tab had to go somewhere, not just go.** Review was the app's original screen (tailor one résumé, read the result); as the pipeline grew it became a stage-3 island with an empty form until you fed it a posting. But it still owned three live things: the **`#resume` selector** every other screen reads (`currentResume()` → profile load/save, résumé upload, LinkedIn import), the only path to **tailor for a posting the search never found**, and Track's **Retailor / Save-to-fixtures** destination. Deleting it wholesale would have silently dropped all three, so each was rehomed: the résumé picker now sits at the top of **Profile**, labelled **"Profile you're working on"** (user's correction — "résumé file you're editing" undersold it: the selection is what the sections edit, what an import merges into, *and* what every application is tailored from, so the label names the thing it governs, not the file it writes; the hint says outright that applicant details are one shared file and do **not** switch with it), and the fixture picker + paste box + engine/quality/length knobs moved inside the dry-run panel, shown only when the job source is *a posting I paste*. **The option grid, and the one combination that can't exist.** A pasted posting has no application URL, so *paste × fill the form* is impossible; rather than fail at run time, choosing paste disables the fill option with the reason on the control ("A pasted posting has no application form to fill"), per UI Principle #3. The single button relabels itself for each state ("▶ Find & fill one (dry-run)" · "▶ Find one & tailor (dry-run)" · "▶ Tailor for this posting") so what will happen is on the button, not in a paragraph. **Server side.** `_test_worker(force_fresh, mode)` gains `mode="tailor"`: after the top match is picked it calls `pipeline.tailor_and_render` and returns — no browser, no fill, no tracker row (nothing was applied to). The PDF still lands on the posting's reusable path with its stamp, so a later apply run reuses it instead of paying for the tailoring twice. `tailor_and_render` gained an additive `on_result` callback so the UI can render the tailored résumé and its **drift warnings** (the factual-accuracy guard that was Review's real value) without changing what the function returns; the new `/test-run/resume` streams the PDF from the path in our own state. The state echoes `mode` so a tailor-only run shows four steps instead of six. **Alternative rejected:** keeping Review as a hidden route reachable only from Track — two ways to do one thing, and the empty-form problem intact. **Verified by driving the real app** (headless Chromium, live server): every option state relabels correctly and the impossible combination is disabled; a real paste-path tailor rendered the résumé, notes, drift panel and PDF button; a real discovered *tailor-only* run searched, judged, tailored *Vestmark — Associate Java Software Engineer*, showed the four-step progress and preview, and served a 3,636-byte PDF from `/test-run/resume`; no JS errors. | ✅ Implemented |
| 162 | 2026-07-30 | **The app opens on Discover, and every view change goes through one `showView()`.** User: *"lets also make the discover page the main page instead of the review and tailor."* **Why it's right.** Review & tailor was the landing view because it was the first thing built (the original "review a tailored résumé" UI); it is stage 3 of five and has nothing to show until a posting exists, so a returning user landed on an empty form and had to click away. Discover is where the pipeline actually starts — search, the auto-apply loop, parked work, and the fit chart all live there — so it is the honest home. Discover is now first in the nav rail and marked `active` in the served HTML, and `#view-discover` renders unhidden while `#view-review` ships hidden, so the correct page paints on first frame with no flash. **The mechanism.** The tab-click handler's body became `showView(v, {nudge})`; the nav clicks, the `#hash` deep-link router, the tour, and the new landing call all route through it, so a view can never be shown without the loaders it needs — landing on Discover had to fire `pollLoop`/`loadParked`/`loadSources`/`loadFitInsights`/`loadCandidates`/`loadDisc`/`pollTest`, which previously only ran on a click. `applyHash()` now returns whether the hash named a real view, and the landing call is `if (!applyHash()) showView("discover", {nudge:false})` — deep links (`/#notifications` from a push notification, decision 138) still win. **`nudge:false` on the landing call only:** a first-visit nudge belongs to *navigating into* a section, and at load `/setup/status` hasn't answered yet, so `setupOk()` reads false and an already-configured user would be nudged to "choose what jobs to find" on every launch. The tour's fallback view moved from `review` to `discover` for the same reason. **Verified by driving the real UI** (headless Chromium against a live server, per ui.md): the page lands on *Discover & apply* with `view-discover` the only visible view, all seven Discover endpoints hit on load, no JS errors, `/#notifications` still lands on Notifications, and clicking Review still shows it. | ✅ Implemented |
| 161 | 2026-07-30 | **Résumé import moves to the top of the Profile screen; LinkedIn import becomes a small button inside it, not a section of its own.** User: *"lets move the resume import to the top of the profile screen as well as linkedin import. Linkedin import can be a small button in the resume inport section since it probably wont be as used."* **The problem.** Both import boxes sat *below* the whole profile form — past Applicant details, Languages, Experience, Projects, Education, Skills, Résumé header, Screening answers, Autofill accounts and Logins. The one action that fills all of those in was the last thing a new user reached, so the intended first step read as an afterthought (UI Principle #1: the interface should present the obvious path, not make the user hunt for it); the onboarding nudge existed largely to scroll people down to it. **Change (`web.py`, layout only — no handler or endpoint touched).** `#s-upload` now renders directly under the page header, above `#profile-form`, retitled *"Start here — upload your résumé"*. `#s-linkedin` keeps its id and every control (`li-file`/`li-import`/`li-msg`) but is nested inside `#s-upload` as a collapsed `.li-panel`, opened by a small secondary `#li-toggle` button ("Import from LinkedIn instead" ↔ "Hide LinkedIn import", `aria-expanded`/`aria-controls` kept in sync) sitting beside *Upload & parse*. LinkedIn requires downloading a data archive first, so it is the rarer path and does not deserve equal weight — but hiding it behind a click keeps the primary box short. **Directional copy fixed with the move:** "reads it into the sections **above**" → "below" in the upload blurb, the LinkedIn blurb, and the post-import success message ("Review the sections below and Save"), which would otherwise point users the wrong way. The section-jump nav swaps its trailing `LinkedIn import` entry (now inside a collapsed panel — an anchor to a hidden element) for a leading **Import résumé** → `s-upload`. **Verified** by driving the running app in headless Chromium: `s-upload` precedes `profile-form` in the DOM, the panel starts hidden, the toggle opens/closes it and relabels both ways, and the page loads with zero console/page errors. | ✅ Implemented |
| 160 | 2026-07-30 | **Résumé-upload merging identifies an entry the way a person does — fuzzy employer/title match, month-precise span as a second signal, a differing year as a veto — instead of exact string equality, and reports every entry it matched instead of adding.** User: *"when parsing a resume and adding onto an already existing profile, we should take into account what already exists (ex: don't make another duplicate intern role if one already exists)."* **The defect.** `resume_import.import_resume()` deduped on `(organization.lower(), role.lower())` — an exact match. Two résumé versions of one job almost never spell it identically, so uploading a second résumé appended duplicates. Reproduced against the user's own profile: re-importing `profile/Gabriel Chan - SoftEngResume10:29:25.pdf` onto a copy of `profile/resume.yaml` added *Jaguar Technologies — Junior Software Engineer* on top of the existing *Jaguar Technologies — Software Engineer Intern*, same May 2024–Apr 2025 span. **The matcher (deterministic, no extra LLM call).** Text is reduced to token sets: punctuation dropped, runs of single letters glued back into their acronym (`M.I.T.` → `mit`, `B.S.` → `bs`), company boilerplate removed (`Inc`/`LLC`/`Corp`/`the`/…), and the title synonyms résumés actually vary on folded (`engineering`/`developer`/`swe` → `engineer`, `internship`/`coop` → `intern`, `sr` → `senior`). Two token sets name the same thing when they are equal, when one contains the other (`Intern` ⊂ `Software Engineer Intern`), or when they are ≥0.87 character-similar. An entry is a duplicate when the **employer** matches **and** (the **title** matches **or** the **month-precise span** is identical) **and** the two do not state **different years**. The span clause is what catches a re-titled job; the year veto is what preserves a genuine second stint at the same employer (2023 internship vs 2024 return offer). Both `experience` and `activities` are searched for every candidate, because the same internship lands in one section from one upload and the other from the next. **Alternative rejected:** feeding the existing profile to Claude as parse context — it biases a parser whose whole contract is verbatim extraction, costs tokens, and makes dedup non-reproducible; the merge is where identity belongs. **Two consequences beyond dedup.** A matched entry now has its *blank* fields filled from the upload (dates, location, degree, project link) — the same fill-blanks-only rule contact fields already used; nothing non-blank is ever overwritten. And the Profile page names what it matched (“Already in your résumé, so not duplicated: …”, “Filled in missing details on …”), because a fuzzy match the user cannot see reads as *it ignored half my résumé* (UI Principle #5). Skills stay on exact matching with punctuation stripped (`Node.js` = `NodeJS`) — fuzzy there would merge `React` into `React Native`. **The LinkedIn import shares the matcher (same session).** `linkedin.import_into()` had the identical exact-string dedup, and LinkedIn is the worst case for it: it stores the legal company name a résumé shortens (`Jaguar Technologies Inc.` vs `Jaguar Technologies`, `Kumon North America` vs `Kumon`), its own title wording, and ISO dates. Rather than a second copy of the rules, the matcher was made the module's public API — `find_role`, `find_education`, `same_tokens`, `same_period`, `fill_blanks`, `skill_key` — and `linkedin.py` imports it, so the two paths cannot drift on what counts as the same entry; `_month_year` learned the ISO form. The matcher stays in `resume_import` (its first caller) rather than a new shared module, to avoid a file-structure change for two callers. **Verified on real data:** re-importing the user's real PDF, *then* a LinkedIn export naming those same roles LinkedIn's way, onto a copy of their real profile adds nothing on either path and leaves the file byte-identical; suite green (657). | ✅ Implemented |
| 159 | 2026-07-30 | **A config file may never be listed as a résumé, `role_commitment` may never be mapped onto a "which/why" question, and a destroyed apply profile is rebuilt from the archives by script — not by hand.** User: *"fix 1 and commit"* (the `role_commitment` defect logged from 158's live dry-run). **Three things, one of them serious. (a) The data loss.** While verifying 158 I ran the full suite twice; `profile/application_profile.yaml` — git-ignored PII with no backup — came back holding a *"Jane Doe" résumé*. Cause: `web.list_resumes()` decides what is a résumé by **trying to load each `profile/*.yaml`**, and `tests/test_web_multi_select.py` stubs `web.load_resume` to return a valid `Resume` for **any** path. Every config file then validated, the Profile screen's résumé dropdown listed `profile/application_profile.yaml` (it sorts before `resume.yaml`), `_allowlisted` accepted it because it was in that same list, and `/resume/update` wrote a résumé over it under the **real** `REPO_ROOT` — the test redirected `apply_profile.DEFAULT_PATH` but not `REPO_ROOT`. Deterministically reproduced with a canary file, then bisected to that one test file. **Fixes:** the test now redirects `web.REPO_ROOT` to `tmp_path` and builds a whole temp `profile/` tree, so it cannot reach the user's files at all; and `list_resumes()` gained a `_NOT_RESUMES` name check. The name check is the belt to the load-check's braces *on purpose* — a listed path is not merely selectable, it is a **write target**, so "it failed to load" is too weak a guarantee for something that can overwrite PII, and any future change that makes loading succeed re-opens the hole. A prior name-based blacklist was removed for drifting out of sync; it is restored **alongside** the load check, not instead of it. New `tests/test_resume_list_excludes_config.py` pins it by reproducing the exact trigger (load_resume succeeding for every path) and asserting both that the config files are unlisted and that `_allowlisted` refuses them. **(b) The recovery.** No backup existed anywhere (no `~/Library/Application Support/ApplicationBot`, and `profile/` is git-ignored). The only surviving record was `profile/applications/*/report.json`, which logs every answer a run submitted **tagged with its source** — a `resolver`-sourced answer is a profile value verbatim. New **`scripts/recover_apply_profile.py`** (idempotent, dry-run by default, per Guideline #8 — no hand-editing) mines them and restored **16 fields**. It ranks a `resolver` answer above an `option:*` one, because the latter is the stored value after being matched onto the form's wording: raw counts alone picked `race_ethnicity = "Asian"` (3 matched options) over the true stored `"Asian (Not Hispanic or Latino)"` (1 verbatim). It refuses to guess: `desired_salary`, `earliest_start_date`, `years_experience`, `portfolio_url`, `work_arrangement`, `max_commute_miles`, `preferred_locations`, the answer bank, and `dropdown_aliases` are **reported as unrecoverable** rather than inferred — a wrong preference is submitted to a real employer. **This also corrects 158's write-up:** the six fields I reported as "blank because your profile is blank" were blank because the *first* suite run had already destroyed the profile; the Palantir dry-run's identity answers came from the résumé and Lever's own autofill, not from the profile. **(c) The logged defect.** `role_commitment` answers a flat `"Yes"`, so `valid_mapping` now refuses it for any question containing which/why/tell us/describe/explain/what kind/what type, and its `CLASSIFIABLE_TYPES` description says it fits a *whether* question only. Scoped deliberately to this one type: the other yes/no types back DESCRIPTIVE dropdowns ("Which best describes your work authorization?") where the `"Yes"` is then matched onto an offered option by `option_hints`, so the same gate there would regress a working path. Because 158 made `classify_question(s)` filter through `valid_mapping`, this blocks the mapping at classification too, not just at persistence. Suite **642 passed**, and the profile is byte-identical after a full run. | ✅ Implemented |
| 158 | 2026-07-30 | **Spoken languages become a structured apply-profile field (`languages: [{name, proficiency}]`), not another captured screening question — and a programming-language question may never be answered from it.** User: *"looking through a application review, and language skills is not a mutiple choice like in the application. Lets fix that and also consider adding language skills to the profile since many more applications will likely ask about it"*. **Diagnosis first (Guideline #0/#11):** the review panel already renders a check-all-that-apply group as checkboxes (`isMultiAnswer`/`multiCheckboxes` in `web.py`) and `apply.py:_record_capture` already records the group's `{kind, options}` — but the `captured` plumbing only landed in decision 157's commit (2026-07-30 10:12), so **all 20 archived `report.json` files predate it and carry no `captured` key at all** (newest 2026-07-29 15:44). With no options in the archive the panel falls back to a text box. So nothing in the rendering path was broken; the archives were stale, and any fresh fill already renders it correctly. The *real* defect was underneath: `"Language Skill(s) (Check all that apply)"` sits in `skipped` as **"no saved answer"** on all four Palantir runs and is absent from `custom_answers` — no résumé field and no profile field carries spoken languages, so every form that asked was captured blank and the user would answer it once per phrasing forever. **Options considered for the data shape:** (a) `languages: list[str]` of plain names — smallest change, but leaves a Workday-style "language + proficiency" grid and any "Native/Fluent/Conversational" dropdown unanswered; (b) **`list[{name, proficiency}]`, chosen by the user** — answers both the name and the level, at the cost of a repeating-row profile editor and one extra resolver rule; (c) one string per line with the level inline (`"English (Native)"`) — matches checkbox options by substring but has no structured level to feed a proficiency dropdown. **Implemented:** `apply_profile.Language{name, proficiency}` + `ApplicationProfile.languages`; `AnswerResolver._languages_answer()` returns the names `"; "`-joined — the exact multi-answer format `_fill_checkboxes` splits on to tick each box, and a string a single-select still matches (`"English"` is a substring of it), so one rule serves checkbox groups, dropdowns, and text fields; `_language_proficiency()` returns the level for whichever language the question *names*, falling back to the first-listed (most proficient) language only when the question names none — a level is **never** claimed for a language the applicant didn't list; `answer_for_type("languages")` so a Claude-classified rephrasing answers live from the profile; a `languages` entry in `CLASSIFIABLE_TYPES`. **The guard that justifies the extra code:** "Which languages are you proficient in?" is a tech-stack question at most employers, and answering it with `"English; Mandarin"` is confidently wrong — so `_CODE_LANGUAGE` (programming/coding/scripting/markup/query language/tech stack/framework/language model/…) stands the rules down, `valid_mapping` refuses to bank such a question as `languages`, and — new — `classify_question`/`classify_questions` now run their result through `valid_mapping` too, so a misclassification is never even *used* for the current run, not merely kept out of the bank. **UI:** a "Languages" section on the Profile screen (repeating name + proficiency cards, `selField` proficiency preset that preserves any other saved wording), jump-nav entry, and `collectProfile`/`replace_profile` round-trip. **Verified:** 10 new tests in `tests/test_languages.py`, including a headless-Chromium fill of `fixtures/apply_forms/multi_select.html` — the literal Palantir question and options — proving the profile field alone (no banked answer) ticks English + Spanish and leaves the group out of `skipped`; full suite **640 passed**. | ✅ Implemented |
| 157 | 2026-07-30 | **Decision 076/077's `apply.py` half is restored rather than its tests deleted: bot walls are reported as refusals and parked retryable, and unknown "reveal the form" wordings are learned once into a host-keyed nav recipe.** User: *"lets get rid of these build errors that pop up in every test: test_parking.py bot_wall failure and the test_nav_recipes.py import error"*. **What was actually wrong (Guideline #0 before the fix):** the two failures were not stale tests — they were the only surviving evidence that a shipped-and-logged feature had gone missing. `git log -S` shows `_bot_wall_evidence`, `_distil_nav`, `run_agent_nav`, `nav_agentic_enabled`, and `ApplyReport.bot_wall` **never existed in any commit**, while everything they plug into did: `nav_recipes.py` (store, `is_shareable_host`), `parking.py` (`BOT_WALL` kind, `describe`, and a `getattr(report, "bot_wall", "")` read that could never be true), the `bot_wall.html` / `unknown_reveal.html` fixtures, and 20 tests. Decision 076 records this code as implemented and verified against the real posting, so the apply-side edit was lost before the commit; five later sessions then logged the failures as "pre-existing" and shipped around them. **Options considered:** (a) delete the two test files — cheapest, but it would silently drop a documented Apply-stage capability, leave `parking.py`'s `BOT_WALL` branch dead, and make DECISIONS 076/077 a lie; (b) mark them `xfail` — same loss, plus a permanent "known broken" marker; (c) **restore the implementation, chosen** — the tests are a complete, executable spec of the missing behaviour, so this is the one option that turns a red suite into working product. **Restored, all in `apply.py`:** `ApplyReport.bot_wall` (a structured flag, because the wall's own vendor host is `captcha-delivery.com` and `parking.classify` must not read that as a solvable puzzle) + `ApplyReport.notes`; `_bot_wall_evidence` walks **every frame** (text signals + vendor hosts — the real 403 renders the wall in a DataDome iframe while the host page's body is empty); `_park_bot_wall` flags it, names the site (host, not "generic"), and the caller then **withholds** the "form did not load" timeout error, because a form that was never served did not time out; `_open_application_form` gained `replay`/`url_hint`/`recipe_path` — it consults the learned recipe for the host (URL suffix and/or extra reveal labels, always on and free), checks for a wall up front, once after the reveal click, and at timeout; `_distil_nav` distils a route by **DOM diff** (which control vanished, what the URL gained), drops page chrome via `_NAV_DECOY` so a dismissed cookie banner never becomes a replayed recipe, and returns an **empty** recipe when the route is opaque rather than a wrong one; `nav_agentic_enabled` (`nav_agentic: true` in `profile/safety.yaml`, off by default, mirroring `workday_agentic`); `nav_agent_prompt` hard-bars filling, uploading, account creation, submitting, and CAPTCHA/wall circumvention; `run_agent_nav` reuses `workday._spawn_claude_agent` (same Claude + Playwright-MCP-over-CDP worker, so there is one mechanism, not two) and saves what it learned. **Wiring:** `run_apply` allocates the CDP port for a non-Workday posting too when `nav_agentic` is armed, and on a failed open calls the fallback **only when `report.bot_wall` is empty** — the agent drives the same browser from the same IP into the identical wall, so aiming it there would spend a token to fail and is evasion (Guideline #4); when the fallback is *not* armed the error names the flag that would fix it (UI Principle #3). `_record_run` records a refused posting as **`blocked`** (it never reaches submit, so `submit_state` stays `dry-run`, and the UI advertises any dry-run row as "ready to apply") with notes reading *"Refused by …: the site blocked automated access…"* instead of "Dry-run". `detect_ats` learned `smartrecruiters` (and `_ats_from_frame` with it). **Verified beyond the tests:** drove `run_apply` headless against all three fixtures over loopback HTTP — the wall fixture reports the refusal, spends no Claude call, and `parking.classify` returns `bot_wall`/"Try again"; the SmartRecruiters posting is detected as `smartrecruiters`, the "I'm interested" reveal opens the form, and 5 fields fill; the unknown-wording posting fails deterministically and names `nav_agentic: true`. Full suite **630 passed, 0 failed** (was 610 passed + 2 failed + 1 collection error). **Discovered, not fixed:** the generic "form did not load" timeout error contains the words "require sign-in", so `parking.classify` parks such a posting as `login` ("store the login, then re-run") — pre-existing and untouched here (Guideline #7); logged in NEXT_STEPS. | ✅ Implemented |
| 156 | 2026-07-30 | **A screening question captured from a checkbox GROUP is a MULTI-answer question everywhere: the Profile screen renders every option as a checkbox, and the fill ticks each chosen one.** User: *"i see that a screening that told users to mark all that apply was not shown as a multiple answer question in the screening questions section of profile"* — the real capture in the user's bank was `Language Skill(s) (Check all that apply)` with 33 options. **Why it matters (Guideline #0):** the Apply stage cannot submit a form whose multi-select question can only ever hold one answer, and a required "check all that apply" group left half-answered is a wrong answer, not a gap. **What was wrong:** `_fill_checkboxes` already captured such groups faithfully (`kind:"checkbox"` + every option label), but `qaAnswerInput` rendered *any* question with options as a single-choice `<select>` — the widget could physically store only one of the 33 languages. **The fix, both ends of the round-trip:** (a) UI — `qaIsMulti` (kind `checkbox` **and** >1 captured option; a single agreement checkbox and every dropdown are untouched) routes to a new `qaMultiInput` that renders one checkbox per option in a scrollable grid, pre-checks what is stored, and mirrors the checked set into the hidden `data-k="answer"` field, so the save path, `cardData`, and the YAML schema are all unchanged — a stored value the form no longer offers still shows, checked and marked "(not in this form)", rather than vanishing on save (UI Principle #5); (b) fill — the answer is one `"A; B"` string, and `_fill_checkboxes` now splits the resolved value on `;`/newline into candidates, so **every** chosen option is matched and ticked instead of the whole string matching nothing. **Storage stayed a string** (rather than a new list field) so no migration, no schema change, and every existing single answer keeps working — the separator only ever splits inside the checkbox-group path. **Consistency — the Review panel edits it the same way** (user: *"lets do that: we want everything to be consistent"*): the per-posting review boxes were plain text, so the one place the user checks before an irreversible submit would have asked them to hand-type `"A; B"`. The checkbox widget is now **one shared implementation** (`multiCheckboxes`, wrapped by `qaMultiInput` for the profile's save round-trip) used by both screens. Three supporting changes make that possible: (1) `_record_capture` now runs for a checkbox group **whether or not it was answered** — the review panel needs the option list for a group the bot already filled — and the control map is persisted in the archived `report.json` (`_report_snapshot`); (2) new `web._merge_checkbox_groups` folds the report's **one-row-per-checked-option** into a single question row (before, the panel showed the same question N times and each edit box saved to the same label, so only the last survived); (3) an answer first given in Review is banked **with its control** (`upsert_answers(meta=…)` via new `web._captured_controls`), so it comes back as checkboxes in the profile instead of a text box. **Not changed:** the batched Claude adjudication still never invents an answer for a checkbox group; unanswered groups are captured for the user exactly as before. **Tests:** new `tests/test_multi_select.py` (headless fill of a new `fixtures/apply_forms/multi_select.html` group: multi-answer ticks exactly the two chosen boxes, a single answer still ticks one, an unanswered group is captured as `checkbox` + options), `tests/test_web_multi_select.py` (drives the real Profile screen: the group renders as checkboxes with the dropdown question left a dropdown, and saving two checked options writes `"English (ENG); Spanish (SPA)"` to the profile), and `tests/test_web_multi_select_review.py` (real `_review_data` folds the two filled rows into one row carrying its options; the panel renders both the answered and the unanswered group as checkboxes, and saving writes the joined override **and** banks the new answer with `input_kind: checkbox` + options). Full suite 610 passed; the 2 `test_parking.py` `bot_wall` failures and the `test_nav_recipes.py` import error are pre-existing. | ✅ Implemented |
| 155 | 2026-07-29 | **An answer the user edits in Review is also LEARNED: reusable ones are written to the shared answer bank, so the next posting that asks the same question is answered instead of coming back blank or wrong.** User: *"when users are reviewing and editing answers for an application, their edited answers should also be saved so that the program can learn from them and won't have to have it unfilled/wrong in the future"*. **Why it matters (Guideline #0):** decision 153 stored a review edit **per posting only** — deliberately, so "Why Acme?" could never be reused elsewhere — but that meant the user re-answered the same generic screening question ("How many years of Python…?") on every single posting, and a wrong banked answer stayed wrong forever. Learning from the edit is what turns Review from a per-application patch into the Apply stage's feedback loop. **What is learned, and what is not — every edit falls in exactly one bucket** (`web._learn_reviewed_answers`): (a) **learned** — a reusable question: upserted into `ApplicationProfile.custom_answers`, which fills the blank entry `capture_questions` banked when autofill got stuck, or overwrites the answer the user just rejected; (b) **profile_owned** — a structured profile rule already answers that label (Email, work authorization, start date…), and `resolve()` consults those rules **before** the bank, so banking it would be inert — the edit stays per-posting and the panel says where the answer really lives, with a link to the Profile tab (UI Principles #3/#5); (c) **posting_only** — company-specific or demographic/EEO, which `answer_bank.is_reusable_answer` refuses for the same reason the run-time bank never caches them (a "Why us?" answer is wrong at the next employer; EEO belongs to the structured fields). **Bucketing is decided by the resolver itself**, not a second copy of its rules: `AnswerResolver.banked_qa()` (extracted from the bank loop inside `resolve()`, same conservative match) distinguishes a bank-answered label from a profile-answered one. **Write semantics — `apply_profile.upsert_answers`:** deliberately overwrites, unlike `remember_answers` (which appends what a *run* learned and never touches an existing entry), because here the author is the user; it also clears `generated` and any `maps_to`, since re-deriving a mapped answer would reproduce exactly the answer the user replaced. A **blank** edit still only clears that posting's override — it never erases what the bank already learned. **Also fixed:** `load_profile`/`save_profile` resolved `DEFAULT_PATH` as an import-time default argument, so redirecting the module constant (as tests must, to keep the developer's own PII out of a run) silently read and wrote the real profile; both now resolve the path at call time. **Tests:** new `tests/test_answer_learning.py` (15) — blank-capture fill, overwrite of a rejected answer + its mapping, blank/no-op writes, the reusable/not-reusable policy table, the three-way bucketing of one save, and the end-to-end point (after the edit, a *different* posting's resolver answers the question while the company-specific edit stays behind); `tests/test_web_answer_edit.py` now drives the whole thing through the real UI headless (edit two answers → the reusable one is in the bank file, "Email" is reported as profile-owned, "Why Acme?" never lands) with the profile/résumé redirected to a temp dir. Full suite 604 passed; the two `test_parking.py` `bot_wall` failures and the `test_nav_recipes.py` import error are pre-existing and untouched. | ✅ Implemented |
| 154 | 2026-07-29 | **A dropdown that spells the applicant's school differently is matched on identity tokens; a dropdown that genuinely doesn't list it takes the form's own "Other" — and says so.** User: *"we dont know what to do if the school dropdown doesnt have an option for Penn state… fuzzy match… and then be able to put 'other' if it really isnt there."* Option matching was equality-or-substring (`_matches`), so "The Pennsylvania State University" saw nothing in a list that says "Penn State University-University Park", and when a list truly lacked the school the field was left blank — which blocks an armed submit (Guideline #3's required-field gate). **New `_fuzzy_option_index`** compares the answer and each option as **identity tokens** (filler `the/of/at/campus/main` dropped, `univ→university` expanded): an option qualifies only if **every** identity token of the answer is accounted for — by equality, a 4+ letter truncation ("penn"→"pennsylvania"), or a 5+ letter typo (difflib ≥ .88) — and it wins on **fewest leftover tokens**, which is why the main campus beats "…- Schuylkill Campus" for free. **The guards matter more than the matching:** institution TYPE words (university/college/institute…) must not conflict, so "Boston University" never matches "Boston College"; a tie between different options is ambiguous and returns None rather than guess ("Miss State" vs Mississippi/Missouri State); 2–3 letter answers stay on the old whole-word rule so "US" cannot reach "Australia", and the 5-letter floor keeps "male"/"female" (ratio .80) apart. Wired into `_fill_select` (after exact/substring), `_pick_from_open`, and `_fill_combobox` phases 1 and 2b — deterministic, so it runs **before** any Claude call and is learned like a vetted mapping. Phase 2b's per-query loop is no longer gated on generation, so a searchable picker resolves with Claude off too. **"Other" is deliberately narrow:** only for labels naming a school/university/college/institution (`_accepts_other`) — answering a demographic, work-authorisation or country question with "Other" would be a wrong answer, not graceful degradation — and it only ever clicks an option the form already offers (`_other_option_index`: "Other…", "Not listed", "None of the above"). Choosing it appends a needs-attention line *"School — 'The Pennsylvania State University' is not in the list; selected \"Other\""* (UI Principle #5: never silently substitute for what the user told us); the "please specify" box such forms reveal is then filled with the real school by the existing resolver rule (asserted in the fixture, which reveals that box on "Other" — that is what makes "Other" an honest answer rather than a dropped one). **Tests:** new `tests/test_school_fuzzy_other.py` — 5 pure cases (abbreviation, typo, main-campus preference, the three refusals, "Other" recognition) plus a headless fill of a new 4-control fixture (`fixtures/apply_forms/school_not_listed.html`: two `<select>`s, two react-selects; spelled-differently and not-listed-at-all) with generation **off**, so every fill there is deterministic and token-free. Two existing tests now report tier `option:fuzzy` where they reported `option:claude` — same committed value, same learned mapping, no model call: `test_required_input_mirror` (batched route still covered by `test_two_pass_fill`/`test_async_school_pick`) and `test_combobox_fill`, whose menu-must-be-CLOSED assertion moved to the work-auth control so that coverage survives. Full suite **589 passed**, only the pre-existing `bot_wall` failures (`test_parking` ×2, `test_nav_recipes` import) remain. | ✅ Implemented |
| 153 | 2026-07-29 | **Every answer in the Review panel is editable, and an edit is what actually gets submitted: it is stored per posting and beats every resolver rule on the next fill.** User: *"lets include the ability to edit answers that the bot will submit when user is reviewing"*. **Why it matters (Guideline #0):** Review was the last checkpoint before an irreversible submit (Guideline #3), but it was read-only — a wrong answer left the user with only bad options: submit it anyway, edit the *global* profile/answer bank for a one-off posting, or abandon the application. **The edit cannot touch the form** (the dry-run browser is long gone by the time the panel is read), so it is recorded against the posting instead and applied on the next fill — which is exactly how every submit path already works (`/loop/apply`, `/parked/reapply`, and "Watch it fill" all re-fill from scratch). **Options considered:** (a) write the edit into the global answer bank — rejected: a posting-specific answer ("Why Acme?") would then be reused everywhere; (b) re-open the browser and edit the live form — rejected: it needs the browser slot the loop owns, and the value would be lost on the next re-fill anyway; (c) **per-posting override file, chosen** — `answers.json` in that posting's archive dir (decision 043), the same company/role/url key the panel already reads `report.json` and `filled.png` from, so it is git-ignored PII with no new storage concept. **Implementation:** new `answer_overrides.py` (load/save/delete, blank value = clear); `AnswerResolver.overrides` + `override_for()` checked at the top of `resolve()`, so an edit outranks the profile, the answer bank, semantic classification, and Claude drafting for free on every path that resolves a label (text, select, combobox, radio, checkbox, free-text drafting); the native-first branch of `_fill_all_fields` also yields to an override, or an edit would silently do nothing on the fields the ATS prefills; `run_apply` loads the posting's overrides right after it derives company/role (both the generic and Workday branches), so re-apply, the loop's queued submit, and the runner all honour them with no caller changes. `_report_snapshot` now records each field's `control`. **UI:** each answer row is an input holding the value that will be sent; unanswered fields get the same rows (UI Principle #2 — the fix is where the block is shown, not a read-only warning); "Save answers" ends in "Saved ✓ — N edited answers will be submitted instead of the bot's" (Principle #5); and *Watch it fill* / *Apply* / *Submit for real* auto-save unsaved edits first and **abort with an actionable error if the save fails** — never fill with answers the user cannot see. **Verified:** `tests/test_answer_overrides.py` (7) + `tests/test_web_answer_edit.py` drives the real UI headless (edit → Save → override file written; unsaved edit → auto-saved before the fill is queued); full suite 580 passed, with 2 pre-existing failures in `test_parking.py`/`test_nav_recipes.py` for an unshipped `bot_wall` feature, untouched by this change. | ✅ Implemented |
| 152 | 2026-07-29 | **Résumé precedence: a real résumé the user uploaded is sent as-is to any posting whose demanded skills it already covers — it outranks every tailored or reused PDF.** User: *"if a application is close enough to an uploaded resume, we should use that one instead of reusing a retailored; actual submitted resumes take precedence over a tailored one."* **Why it matters (Guideline #0):** the Tailor stage's whole reuse ladder (069 exact, 142 cross-posting) only ever chose between machine-generated PDFs. The user's own résumé — proof-read, laid out how they want, and in practice already sent to employers — was strictly better evidence than anything we generate, yet `resume_import` parsed it and **threw the bytes away**, so it could never be sent. **Ambiguity resolved before coding (Framework #2):** asked whether "uploaded résumé" meant the Profile-tab upload or the frozen `archive/submitted-<date>/resume.pdf` of a real submission, and whether precedence should be rank-first or score-first. User chose **the Profile-tab upload**, **rank-first** (a qualifying uploaded résumé beats a higher-scoring tailored one; score only breaks ties inside the rank). **Storage — new `resume_docs.py`:** `profile/uploads/<name-slug>-<content-hash>.pdf` + a `.meta` sidecar (`filename`, extracted `text`, `uploaded_at`), written by `resume_import.import_resume` on top of the catalogue merge (best-effort — a storage failure never fails an import). **PDFs only**, because the apply stage feeds `pdf_path` straight into the ATS file field, so a DOCX/TXT could not be submitted verbatim; content-hashed names make a re-upload overwrite while a genuinely different résumé becomes its own candidate. The *text* is stored rather than a precomputed skill list, because the skill universe comes from the user's current résumé catalogue and changes as they edit it. **Matching — `reuse.coverage` + `pipeline.find_uploaded_match`:** deliberately **asymmetric**, unlike decision 142's Jaccard `similarity`: a real résumé lists a whole career, so Jaccard would score it near zero and it would never be used. What matters is the share of the posting's *demanded* skills the document shows — `|demanded ∩ present| / |demanded|`, at the same 0.9 bar. Token-free (the stored text scored by the `ats_requirements` extraction the tailoring loop already runs). Knockouts are not compared (their verdicts are facts about the candidate, identical whichever PDF we send). A posting demanding **nothing** we can verify — usually a JD we failed to scrape — never matches, since trivial 1.0 coverage is not evidence of fit. **Wiring:** `run_testing_mode` checks the upload first and short-circuits both reuse scans; `force_retailor` ("Re-tailor") still wins, and the branch applies on armed submits too (that is the point — a real résumé is *better*, not a token-saving compromise). The bytes are copied into the per-posting slot so Track/archive point at stable files, and `resume_store.clear_tailor_sidecars` drops the `.stamp`/`.sig` there — otherwise a stale stamp would make the next dry-run reuse those bytes as "this posting's tailored résumé" and a stale sig would offer them to other postings under the wrong skill profile. **Provenance (extends 144):** `reuse.uploaded_reuse_label(name, score)` → "Reused your uploaded résumé jane.pdf (100% of demanded skills covered)" — starts with "Reused" so every existing `is_reused` badge in Track/notifications/review works unchanged. **UI (Principles #1/#3):** the Profile upload box states the behaviour, and a **Kept résumé files** list under it shows each stored PDF with its upload date and a one-click **Remove** (`GET /resume/uploads`, `POST /resume/uploads/delete`; delete takes a bare file name and refuses paths). **Tests:** new `tests/test_uploaded_resume.py` (8) — coverage asymmetry, PDF-only storage + scoped delete, scan threshold/no-demand behaviour, thin document below bar, end-to-end "no tailor call, uploaded bytes applied, no sidecars", upload-beats-a-100%-tailored-match, and `force_retailor` override; `_Stubs` now redirects `UPLOADS_DIR` so runs never pick up the developer's own kept résumés. Full suite 574 passed (the two `bot_wall` `test_parking` failures and the `test_nav_recipes` import error are pre-existing and unrelated). Verified against the running app: seeded a kept PDF, `GET /resume/uploads` listed it, the Profile panel rendered the row (screenshot), and clicking **Remove** deleted the file from disk. | Accepted |
| 151 | 2026-07-29 | **Application emails forwarded into the linked inbox become tracker rows: a free gate → Haiku extraction → match-or-insert, written directly and undoable.** User: *"are we able to update our tracking database based on emails in the linked inbox? I want to be able to forward emails from another email into it and have it make new rows in the tracker."* **Why it matters (Guideline #0):** the Track stage only ever knew about applications *this bot* made, so everything applied to by hand — on LinkedIn, on a company site, before ApplicationBot existed — was invisible, and the funnel (`funnel_report`) plus the outcome calibration that tunes `min_fit` (decision 043) were computed from a fraction of the real history. **Options considered:** (a) heuristics only — sender domain → company, subject regex → role/outcome: free and offline, but silently mis-parses any non-standard template; (b) Claude on every inbox message — highest recall, but pays for newsletters and security alerts; (c) **cheap gate then Haiku** — chosen, the same two-stage shape as decision 124's pre-rank/judge. **Pipeline (`applicationbot/inbox_import.py`):** (1) **free gate** `classify_message` — a forwarded email's `From` is the *forwarder*, so `effective_sender` digs the true sender out of the body's forwarded header block; a message is worth a token only if it comes from a known ATS domain (`_ATS_DOMAINS`, 18 platforms) or carries application wording (`_APPLICATION_MARKERS`). Job-alert emails are recognized here and routed OUT — those are new *openings*, discovery's job via `EmailAlertSource` (decision 132) — so they never cost a token. (2) **`extract_batch`** — survivors go to `haiku` in batches of 5 (`activity="inbox-import"`, so the spend shows in the Track tab's token panel, decision 095) under a schema returning company/role/kind/date/url/confidence. (3) **`apply_extraction`** — source URL wins the match outright, else normalized company (legal suffixes stripped) + role token-overlap ≥60%; a match updates the row's status, no match inserts one. **Status moves FORWARD only** (`_RANK`), so a confirmation email read after an interview invite can't reset the row to `applied`; a rejection is allowed over anything but an offer, since being rejected after interviewing is the normal path. **Write policy (user chose "direct write, flagged + undoable"):** rows land immediately — a local DB write, nothing outward-facing — carrying `method="email-import"` and the source email quoted in `notes`. The ledger (`profile/inbox_import_seen.json`, git-ignored, Guideline #12) makes a re-scan idempotent by Message-ID and records each run, so `undo_run` reverses one import exactly (inserted rows deleted, updated statuses restored, messages released for re-import). **New mailbox reader:** `fetch_messages` returns full records (Message-ID/From/Subject/Date/body) over both auth paths — `fetch_alerts` drops the headers and filters by sender, which is exactly wrong for forwarded mail. **Alerts wiring (UI Principle #2):** when a run finds forwarded alert emails while discovery's alert source is off, the Track tab offers one button that switches on exactly the providers whose emails are in the inbox. **Verified on the real inbox** (the linked bot inbox, 52 messages/60 days): the gate spent 0 tokens and classified 21 LinkedIn + 18 Lensa alerts to the alert bucket, 13 Google/Red Hat account notices to skip, 0 false negatives — no application emails have been forwarded yet. The Claude stage was then driven for real on four realistic samples: Stripe confirmation → `applied` row (portal greenhouse, `date_applied` from the email, Greenhouse URL captured), Datadog → `rejected`, Figma recruiter → `interview`, Indeed digest → correctly not an application. **Note for decision 132's flagged live step:** Lensa alerts ARE now arriving (18 in 60 days), where NEXT_STEPS recorded 0. **Tests:** 18 in `tests/test_inbox_import.py` (gate incl. forward-reading, match/insert/forward-only-status, idempotent re-run, exact undo, a failed Claude batch leaving messages for the next run, `fetch_messages` header retention) + 5 route tests in `tests/test_web_inbox_import.py`. | Accepted |
| 150 | 2026-07-29 | **The mandatory agent-bus session ritual is removed from `CLAUDE.md`: there is no parallel Cursor agent, so agents must stop invoking or referencing one.** User: *"why do agents keep mentioning a parallel cursor loop when nothing of the sort exists? lets get rid of that behaviour."* **Cause:** `CLAUDE.md` carried a "Parallel agents (Cursor ↔ Claude VS Code)" section (decision 014) instructing every session to run `agent_bus context/read` at the start, claim paths while working, and `ack`/`release`/`status` at the end. Because it read as a standing rule, agents kept narrating coordination that had no counterpart — e.g. decisions 144/145 record "claimed `web.py` on the agent bus and handed off to cursor" for work no other agent touched. That is a Guideline #11 violation (reporting something that did not happen) and it wasted turns. **Decision (user chose "Instructions only"):** replace the ritual section with an explicit prohibition — no bus commands, no claims/handoffs, no mentions of another agent in messages, commits, `DECISIONS.md`, or `NEXT_STEPS.md`. **Kept, not deleted:** `applicationbot/agent_bus.py`, `docs/AGENT_COLLAB.md`, `scripts/agent-bus`, `.cursor/hooks*`, and the git-ignored `.agent-bus/` stay on disk as dormant tooling, usable if the user ever asks for parallel agents again — rejected the full rip-out so the capability isn't lost, since the problem was the standing instruction, not the code. No runtime code, dependency, or test is affected (the bus is dev tooling only, imported by nothing in the pipeline). Supersedes the `CLAUDE.md` half of decision 014. | Accepted |
| 149 | 2026-07-29 | **An application you never looked at is not "handled": a prepared-but-unreviewed posting is exempt from both discovery suppression filters and comes back in the next search — and the running loop now shows the same search breakdown the one-shot dry run does. Plus one-click "Add all" for scouted sources.** User: *"add an 'add all' button to the new sources list and search breakdown (similar to dry run) during the loop. Also, if an application was never reviewed, it should be brought up in the search again."* **(a) Re-surface the unreviewed.** A posting the loop prepared was buried twice: `skip_seen` dropped it because a tracker row existed, and the seen-openings ledger (decision 053/146) dropped it because it had been judged. So applications prepared while the user was away — the exact case the loop exists for — silently never came up again. New `applications.reviewed_at` column (migrated in place, blank = never seen) stamped when the user **opens the application's Review panel** (`GET /track/review`) or clicks Apply/Watch; `tracker.unreviewed_source_urls()` returns the prepared-but-unstamped ones and `pipeline._revisit_canonical_urls` subtracts them from BOTH filters. *Reviewed = opened the panel* was the user's choice over "only Apply/Watch counts" and "any unsubmitted dry-run re-surfaces forever". Submitted (`applied`) rows are unaffected — they stay suppressed. **Token guard:** `discover_and_match(revisit=…)` defaults to True, but the loop passes `revisit=False` on every hunt pass after the first, and the loop skips any posting it already prepared this run, so a multi-pass goal hunt can't re-judge or re-prepare the same unreviewed postings on a spin. The funnel reports `revisited` and the UI names it ("3 brought back (prepared but never reviewed)") so a returning posting is never a mystery. **(b) Loop search breakdown.** The loop published only a one-line message; the dry run's funnel + judged list existed but only for the one-shot test run. `_loop_worker` now publishes each search's `funnel`/`judged`/`scanned`/`matched`/`cleared`/`min_fit` into `/loop/status` (shared `_judged_rows` helper, same shape as the test run), and the Discover tab renders them with the SAME `renderScanFunnel`/`renderJudged` used by the dry run — one pattern, not a second one. Rendered only when the stats change, so the 2s poll can't re-open a funnel the user collapsed. Publishing is best-effort (wrapped): failing to *describe* a search must never stop the loop from preparing it. **(c) "Add all (N)"** heads the "New sources found" panel, driving each row's existing per-row add sequentially (same endpoints, same per-row "Added ✓") with live "n of N" progress, one config refresh at the end, and a failure summary that leaves failed rows retryable — rather than a parallel bulk endpoint that could drift from the single-add path. **Verified by driving the real app** (Playwright against the running server): "Add all (32)" renders atop the 32 staged candidates; the loop breakdown renders funnel + accepted/denied judged rows and survives a re-poll collapsed. New `tests/test_revisit_unreviewed.py` (4) covers the tracker half and both filters; 543 tests pass (2 pre-existing `bot_wall` failures in `test_parking.py` unrelated). | Accepted |
| 148 | 2026-07-29 | **"Ready to apply" cards are reused across `/loop/status` polls (keyed by application id) instead of being re-rendered, so a Review panel opened while the loop runs stays open.** User: "I cant open an application review when loop is running." **Root cause:** while the loop runs the UI polls `/loop/status` every 2s and `renderLoop` rebuilt the whole ready list with `ready.innerHTML = ""` (web.py) — the expanded `.review` panel, its `dataset.loaded` flag, and any in-flight `/track/review` fetch were destroyed on the next tick, so the review flashed open and vanished ~2s later. Idle (no polling) it stayed open, which is why the bug looked loop-specific. **Fix:** `renderLoop` keeps a `LOOP_CARDS` map of `app id → {sig, node}`; each poll reuses the existing card node when its facts (`company/role/fit/portal/resume_source`) are unchanged and rebuilds only cards whose facts actually changed. Detach/re-append preserves DOM state and listeners, so the panel, its loaded content, and the "Review ▴" state survive every poll. Backend untouched — `/track/review` was never the problem (`ThreadingHTTPServer` already serves it concurrently with the loop thread). **Rejected:** stopping the poll while a panel is open (the user would stop seeing loop progress), and re-expanding after each rebuild (a re-fetch and a visible flash every 2s). **Verified by driving the real UI headless** (Playwright against the actual server with the loop faked as running): Review opens, survives three status polls with contents intact and no page errors; with card reuse removed the same drive fails with the panel collapsed — the new `tests/test_web_loop_review.py` is that drive. 17/17 loop-web tests pass; served-page `node --check` clean. | Accepted |
| 147 | 2026-07-29 | **A linked inbox's secret is proven stored in the OS keychain before the link is recorded, and a link whose secret went missing says so instead of reading as "not connected".** User: "make sure that app passwords for linked gmail accounts are saved." **Verified first (the user's live link is fine):** `profile/mailbox.yaml` records `imap.gmail.com:993`, the macOS keychain (`applicationbot-mailbox`) holds the app password, `load_config()` returns it, and `python -m applicationbot.mailbox test` logged into Gmail over IMAP with it. **Two silent-failure paths closed:** (1) `save_link`/`save_gmail_link` wrote the secret and then wrote the link file **without checking the write landed** — a locked login keychain or a `keyring` fallback backend accepts `set_password` and stores nothing, so the inbox reported "✓ Connected" while the password was gone; the next run would fail with no explanation. Now `_verify_saved` reads the secret back and raises an actionable `RuntimeError` ("did not persist … unlock your login keychain in Keychain Access, then connect again"), and the **link file is written only after the read-back succeeds** — a link is never recorded without its password. (2) When the file records an account whose keychain secret is missing, `load_link` returns `None` and the UI said only "Not connected — add your Gmail address + app password below", hiding that the *password specifically* vanished. New `mailbox.link_problem()` names the account and the exact fix ("… is linked but its app password is missing from your OS keychain — re-enter the 16-character app password below and click Connect", or the Google-authorization wording for an OAuth link); `link_status()` returns it as a `problem` field (additive; `linked` semantics unchanged), the Settings inbox panel renders it in red and keeps **Disconnect** visible to clear the stale record (UI Principle #3), and CLI `mailbox status` prints it. **Failures now surface instead of 500ing:** `POST /mailbox/link` catches a save failure and returns `ok:false` with the real reason ("Signed in to imap.gmail.com as … but the app password did not persist …"); `connect_gmail` returns `(False, "Google approved the access but it could not be saved: …")`; CLI `link` prints "Not linked — …" and exits 1. **Rejected:** writing the password to `mailbox.yaml` as a keychain fallback — that file is git-ignored but plaintext PII on disk (Guideline #5/#12); a failed keychain write must be fixed, not worked around. **Tests:** 4 new in `test_mailbox.py` (dropping-keyring backend ⇒ `save_link`/`save_gmail_link` raise and **no** link file is written; a deleted keychain entry ⇒ `link_status` reports the missing app password / missing Google authorization by name), exact-dict status assertion updated for the new field; 29/29 mailbox, 97/97 across mailbox+web+alerts+notifications. **Driven live** against the real macOS keychain (throwaway account saved, read back, deleted, then reported as a problem) and a real Gmail IMAP login; served-page `node --check` clean. Pre-existing, unrelated: `tests/test_nav_recipes.py` fails to import (`_bot_wall_evidence` missing from `apply.py`, another agent's in-progress work). | Accepted |
| 146 | 2026-07-29 | **A goal on the auto-apply loop is a commitment: the loop keeps searching until that many applications are ready or the user stops it — and the seen-openings ledger no longer burns postings the judge never scored.** User: "auto apply loop is not looping correctly. When I set it to stop only when it has 5 applications ready to apply to, it just stops and tells me that there are no new matches. The whole point of the loop is that it should keep looking if there are no new matches, until my specified number of matches is met." **Three compounding root causes, all confirmed against the user's own data:** (1) **The loop didn't loop.** `autoloop.auto_apply_loop` returned `"caught_up"` on the first empty batch regardless of the goal — only *watch* mode (decision 143) re-searched. A goal that was still short simply ended the run. (2) **The candidate pool was burned before it was judged.** `pipeline._hide_already_shown` ran *after* `match()` and recorded **every keyword survivor** into the seen-openings ledger (decision 053) — but `match()` only Claude-scores `top_n` (the user's `top_n: 40`). A pass that surfaced 200 postings judged 40 and permanently hid all 200, so no later search could ever surface the other 160. Measured on the user's ledger: **99 of 171 entries had no judged verdict in `fit_history.jsonl`** — hidden forever without Claude ever having looked at them. (3) **Cache replay.** The non-watch loop called discovery with `force_fresh=False` and `cache_ttl_hours: 12`, so a re-search inside the window replayed the identical snapshot, which the ledger then emptied. **Options considered:** *(a) loop-only fix* — re-search on an interval and leave the ledger alone; rejected: retries could then only ever find roles posted since the last pass (a trickle), and the 99 burned postings stay invisible, so "keep looking" would spin without progress. *(b) never record unjudged postings but keep filtering post-match* — rejected: the judge would then re-score the same top 40 every pass, spending tokens to produce the same verdicts. *(c) chosen: progressive deepening.* The ledger is applied **before** the judge on a live search (`pipeline._drop_already_shown`, after bridging so canonical URLs match), and only **judged** matches are recorded (`_record_shown`). Each pass therefore spends its scarce `top_n` judge slots on the next-best postings no pass has scored — pass 1 judges the best 40, pass 2 the next 40, and so on — with zero re-judging, preserving the "we don't run through tokens" constraint from decision 053/056. **The loop** (`autoloop`, new `hunt_wait(n)` injection): with a goal set and unmet, an empty batch fires a `hunting` event and idles a stop-responsive backoff instead of ending — **60s → 2m → 5m → 15m → 30m cap** (`web._HUNT_BACKOFF_SECONDS`), short at first because a fresh board search often turns something up, capped so a long hunt doesn't hammer the boards (Agent Guideline #4). Every pass after the first uses `force_fresh=True`, and a first pass served from cache with nothing new immediately escalates to a live search rather than reporting "no new matches" off a stale snapshot. It ends only on **Stop** or on reaching the goal; with `goal=None` the pre-existing "boards exhausted ⇒ `caught_up`" behaviour is untouched. **Status honesty (UI Principles #3/#5):** while hunting the UI no longer says "Caught up — no new matches"; it says how far along the goal is, what the pass actually did, and when the next one runs — e.g. *"2 of 5 ready — pass 3 found no new matches (judged 40 posting(s), best fit 54 vs your min_fit of 60). Searching again in 5 min. Stop anytime, or lower min_fit / add boards in Discovery settings to widen the net."* **Behaviour change to maintain mode:** a maintain-mode loop whose pool has dropped below the goal now hunts instead of returning `caught_up` — consistent with what "keep topping up to the goal" promises. **Ledger repair:** `scripts/prune_seen_ledger.py` (idempotent, dry-run by default, `--apply` to write) drops ledger entries with no judged verdict in `fit_history.jsonl`; run on the user's machine, it took the ledger from 171 → 72 entries, making 99 previously-burned postings eligible again. | Accepted |
| 145 | 2026-07-29 | **Every push notification the loop fires is now persisted to a durable log and shown in the Notifications tab, so a push is never lost when its underlying application changes state.** User: "I'm getting a lot of push notifs that are not in the notifications tab — track push notifications in the notifications tab so I can do things with them." **Root cause:** the Notifications tab (`/inbox`, decision 138) was a *live snapshot* — it recomputed `ready` (from the in-memory `_LOOP_STATE["ready_ids"]`) + `parked` (from `tracker.parked_applications()`) on every fetch. So a push fired for an app that was then submitted, or that dropped out when the loop reset / the server restarted, left NO trace in the tab even though the desktop/phone push had gone out. The push transports (decision 135) were fire-and-forget with no record. **Fix — treat the in-app tab as a durable channel, not a live mirror:** (1) **New `notifications` table** in the tracker DB (`tracker.py`, migrated by `_connect` like decision 043/144 columns; WAL already handles loop-thread-writes / UI-reads): one row per fired push — `event`/`title`/`body`/`link`/`application_id`/`urgent`/`channels` (which transports it went out on) + per-item `read`/`dismissed` + `created_at`. Store fns: `add_notification`, `list_notifications` (newest-first, hides dismissed), `unread_notification_count`, `mark_notifications_read(ids|None)`, `dismiss_notifications(ids|None)` (ids=None = all; `[]` = none, a no-op — not "all"). Dismissed rows stay in the DB (audit), just drop out of the tab. (2) **Central `web._record_and_push(notifier, note, app_id)`** replaces the two bare `notifier.notify()` calls in `_loop_worker.prepare_one`: it logs the notification (best-effort — a DB hiccup never breaks the loop) THEN pushes via the transports, gated by the same event toggle (a disabled event neither logs nor pushes), tagging the row with `app_id` (the tracker row) so the feed ties back to the application. The in-app log records even when desktop+ntfy are both off — the tab is itself a channel. (3) **`/inbox` extended** with `notifications` (the log, each enriched with the concerned app's current `app_status` + an `actionable` flag) + `unread`; the **nav badge keeps its decision-138 meaning — the live count of things needing action (ready + parked)** — so it falls as the user submits/resolves and does NOT clear just from opening the tab. New POSTs `/notifications/read` (mark the log seen so the "new" dots clear) and `/notifications/dismiss` (remove; ids=null clears all). **Durable ready (the fix for "can't review/do anything"):** the `ready` action cards were sourced only from the in-memory `_LOOP_STATE["ready_ids"]`, so after a server restart a still-`dry-run` application showed a notification but had NO card to act on. `/inbox` now unions `ready_ids` with the `dry-run` apps referenced by `approval_needed` log rows, so a ready application stays reviewable/submittable across restarts. (4) **UI:** the live action cards (unchanged — the exact Discover `parkedCard`/`loopReadyCard`, so Review/Apply/Resolve work identically), each tagged `id="inbox-app-<id>"`; below them a **"Recent notifications" feed showing EVERY push** (a push that says "open Notifications to review and submit" must actually appear there — the user's follow-up: they were being filtered out). Each feed row: urgency dot (unread/urgent tinted), title, relative time, body, a status tag when handled (applied/submitted), Dismiss ✕, plus **Clear all**; an actionable row (its app is still ready/blocked, via the `actionable` flag) also gets an inline **"Review & submit →"** / **"Resolve →"** button that scrolls to that application's card and briefly flashes it — the review/submit the push promised, reachable from the notification itself. *(An earlier pass filtered actionable notifications OUT of the feed to avoid card/row duplication; reverted after the user found their pushes "just missing" — showing the notification is what they wanted, and the inline jump ties it to its card.)* Test notifications (`/notifications/test`) are deliberately NOT logged (they're a channel probe, not a real event). **Layout fix (found by screenshotting the tab):** the feed's Dismiss (`.nf-x`) and Clear-all (`.nf-clear`) buttons inherited the app's global `button { width:100% }`, blowing the ✕ to full width and crushing the text column to 0 (every word wrapped to its own line); added `width:auto; margin:0` to both (the same reset every other button component uses). Verified with a Playwright screenshot — `.nf-main` fills (718px), `.nf-x` is 18px, rows read cleanly. **Verified live** (temp data dir): seeded two notifications → `/inbox` returns them with `unread:2` → `/notifications/read` → `unread:0` → `/notifications/dismiss` → feed empty; served-page JS `node --check` clean; `import applicationbot.web`/`tracker` clean. **Tests:** +9 in `test_notifications.py` (add/list newest-first/unread; mark-read one-vs-all; dismiss hides-but-keeps-row + audit via include_dismissed; empty-`[]`-is-noop), 30 in the file (incl. the durable-ready inbox regression tests), tracker/inbox/notif selection all green. **Coordination:** claimed `web.py`/`tracker.py`/`notifications.py` on the agent bus and handed off to cursor (owner of decision 138) that `/inbox` gained the notification log + a durable-ready union (badge meaning unchanged — still the live action count). **Follow-up fixes (same day, from user testing):** (a) the feed's Dismiss/Clear buttons inherited the global `button{width:100%}` and crushed the text to one-word-per-line → added the standard `width:auto;margin:0` reset (verified by screenshot); (b) restored the badge to the live action count after a first pass wrongly switched it to unread-notifications; (c) the durable-ready union above, after the user reported ready apps weren't reviewable post-restart; (d) **test-pollution fix** — the user saw repeated Acme/Bolt pushes: `test_autoloop_web.py` drives the REAL `_loop_worker`, whose `prepare_one` now calls `_record_and_push`, which used the default (real) tracker DB AND built a real notifier — so every run of that file fired real desktop notifications and wrote 13 orphan notifications (referencing fixture app-ids 1/2 that don't exist) into the developer's `applications.db`. Fixed by stubbing `web._record_and_push` in the `fake_pipeline` fixture (loop-mechanics tests only — notification behaviour is covered in `test_notifications.py`); verified the suite now leaves the notifications table unchanged, and purged the 13 orphan rows. Re-verified end-to-end with a Playwright screenshot: badge shows the action count, the Blocked + Ready cards render with working Review/Answer buttons, and an already-applied posting drops to the dismissible Earlier-notifications feed. | Accepted |
| 144 | 2026-07-29 | **Surface résumé provenance — freshly tailored vs reused — everywhere a submission is shown: Track, notifications, and the review panel.** User (follow-up to decision 142): "show in tracking, notifications, review, etc, if a submission plans to reuse a previous resume or a tailored one." Cross-posting reuse (142) and same-posting exact reuse (069) both skip the Claude tailor and reuse a PDF, but that was only ever a transient progress line — after the fact nothing recorded WHICH résumé a submission used, so "reused" could be discovered only by noticing tokens weren't spent. **Data model:** one human-readable provenance string threaded end-to-end. `reuse.py` gained the labels (`FRESH`, `exact_reuse_label`, `similar_reuse_label(source, score)` — names the source posting + match %, `stored_reuse_label`, `is_reused` — reused strings all start with "Reused" so a UI badges without parsing). `run_testing_mode` sets it per branch (fresh / exact-reuse / cross-reuse) and passes it in `meta["resume_source"]`; the re-apply/loop-submit/loop-watch paths carry the row's stored provenance (or fresh/stored for the Track re-tailor toggle). **Persistence (Framework #2 — data storage):** a new `applications.resume_source` column (migrated in `_connect` like the decision-043 columns; added to `EDITABLE`), written by `apply._record_run` as runner-owned truth (never clobbered by a user edit; only set when known), plus the same string appended to each `application_runs.detail` so the run-history log reads standalone. `ApplyReport.resume_source` carries it live (set from `meta` in `run_apply`) so surfaces can show it BEFORE it's persisted. **Surfaces (UI Principles #1/#5 — never let a reuse be a silent surprise):** (a) **Notifications** — the "Ready to apply" push body appends "Résumé: <provenance>." (b) **Review panel** (`renderReview`, the card shown before the per-click armed submit) — a provenance chip + full sentence in the résumé section. (c) **Track** — a green "Tailored résumé" / amber "Reused résumé" chip (`resumeSrcChip`, hover = full sentence) on each feed card and in the context drawer; `/track` already returns the column (SELECT *). (d) ready-card list (`/loop/status` + `/inbox`) carries `resume_source` so `loopReadyCard` chips it. Theme-aware CSS reuses the existing `--ok`/`--warn` vars. **Tests:** `test_reuse.py` +4 (label helpers + `is_reused`; `run_testing_mode` emits FRESH then a "Reused …" naming the source in `meta`; `_record_run` writes `resume_source` to the row AND the run detail with tracker stubbed; the new column round-trips via `add_application`/`get_application` on a temp DB) — 19 in the file, all green; the tracker/apply suites (`test_manual_submit`/`test_submit`/`test_runner`/`test_autoloop`/`test_calibration`/`test_required_draft`, 61) green with the new column; full suite green except the two pre-existing `test_parking`/`test_nav_recipes` failures from another agent's in-progress `bot_wall` work (unrelated — `apply.py` has no `bot_wall`). `web.py` `py_compile` + served-page `node --check` clean. | Accepted |
| 143 | 2026-07-28 | **A "keep watching" toggle turns the web auto-apply loop into a continuous review-watch: re-check the boards on an interval, autofill each new match, hold it for human approval, never submit on its own.** User wanted to *test* continuous mode and proposed "a switch that autofills but doesn't apply until a human looks it over." **Finding surfaced first (Guideline #2):** that capability already existed in two half-forms — the CLI `runner --continuous` polls forever but filled headless with no per-app approval step, and the web auto-apply loop (decision 069) autofills + holds each match as a "Ready to apply" card the user submits with one deliberate click, but **stops when caught up** (`autoloop.auto_apply_loop` returns `"caught_up"` on an empty batch), so it won't catch a seasonal role posted next week. The real gap = make that human-gated review loop keep polling. **User chose (AskUserQuestion) "both":** run the headed dry-run CLI watch now (no build) AND build the continuous web review-loop. **What shipped (surgical, no new submit path):** (1) `autoloop.auto_apply_loop` gained `watch`/`watch_wait` — on an empty batch in watch mode it fires `caught_up` (for the UI), idles via a long stop-responsive `watch_wait`, then re-searches instead of returning; ends ONLY on stop. Zero change when `watch=False`. (2) `web._loop_worker`/`start_loop` gained `watch`+`watch_interval` (default 30 min, clamped ≥1); watch mode passes `force_fresh=True` to discovery so each poll actually re-searches the boards (`only_new=True` still bounds judging to genuinely-new hits — no re-judge spend), and `watch_wait=_LOOP_STOP.wait(interval*60)` (wakes at once on Stop). A one-shot `rescan` forces `watch` off. A new `"watching"` phase message says "no new matches now; re-checking every N min; K ready to review." (3) UI: a **"Keep watching — re-check the boards on a schedule"** toggle + interval input in the Discover loop card, wired into `/loop/start`, disabled-while-running; the interval enables only when the toggle is on. **Safety unchanged (Guideline #3):** the loop itself never submits — a real submit still requires the user's per-card Apply click (the decision-058 one-shot arm behind a confirm), so watch mode is inherently "autofill every new role, hold for review, forever." Sibling to decision 140 (the CLI `runner --continuous` notify-on-ready watch); this is its web, human-approves-each-submit counterpart. **Also handed the user the no-build CLI test path:** `runner --continuous --headed --fresh --dry-run` (visible autofill, never submits, notifies each cycle). **Tests:** 2 in `test_autoloop.py` (watch idles+re-searches past caught_up, ends only on stop; `watch=False` still stops at caught_up) + 2 in `test_autoloop_web.py` (watch enters the "watching" phase and stays alive, re-searches ≥2×, stops cleanly on `stop_loop`; rescan forces watch off) — 26 loop tests + 64 across all touched areas green; full suite 516 passed (2 pre-existing `test_parking` bot_wall failures unrelated); `web.py` `py_compile` + served-page JS `node --check` clean. **Not done:** persistence across an app restart (loop state is in-process — a watch is restarted after a restart, matching the existing loop); a "next re-check in N min" countdown (the phase message states the interval). | Accepted |
| 142 | 2026-07-28 | **Cross-posting résumé reuse: when a new job demands essentially the same skills as one we already tailored a résumé for, reuse that PDF verbatim and skip the Claude tailor call.** User: "harden the tailoring logic — when a job's description is close enough for an already-existing résumé, skip the tailoring and reuse it to save tokens." **Framing (Agent Decision Framework #2 — affects *how résumés are tailored*, so options were presented before code):** the existing reuse (decision 069/029) only reuses a PDF for the **same posting** when its stamp (résumé + links + tailoring-logic + **exact** JD) matches, and is dry-run only. This adds *cross*-posting reuse keyed on a **fuzzy** JD match. **Two user decisions (AskUserQuestion):** (1) **similarity metric = demanded-skill overlap** — a tailored résumé is a function of *which of the candidate's skills the JD demands* (`ats_requirements.extract` keywords) + the hard knockout gates, NOT the prose; comparing those sets is **token-free** (reuses the extraction the tailoring loop already runs), so deciding to skip a Claude call never itself costs one — chosen over raw-text similarity (noisy: benefits/EEO boilerplate inflates it) and embeddings (adds an API call/$ per JD, defeating the goal). (2) **applies on armed submits too**, not just dry-run (user accepted the tradeoff that a real submit may ride a résumé tailored to a *similar* posting; this relaxes the decision-069 armed invariant, but only for a **different** posting — see below). **What "close enough" means:** the reuse key splits into an **exact** part and a **fuzzy** part. Exact = `tailor_base_stamp` (résumé + header links + tailoring-logic fingerprint, JD-independent) must be **identical** — a different base résumé / links / tailoring code never reuses across the change. Fuzzy = demanded-skill **Jaccard ≥ 0.9** (`reuse.DEFAULT_THRESHOLD`, deliberately strict; 0 disables) **AND an identical knockout profile** (a résumé that clears one posting's hard gates may fail another's, so differing knockouts score 0 regardless of skill overlap). **The reused PDF is company-independent** — it carries the applicant's own info + profile links, never the target company's — so a match is served by **copying the source PDF** to the new posting's per-posting path (no re-render, no Claude call). **What shipped:** (1) new pure module `applicationbot/reuse.py` — `JdSignature` (demanded-skill frozenset + knockout tuple, JSON round-trip), `signature(resume, jd)`, `similarity(a, b)` (knockout-gated Jaccard). (2) `resume_store` `.sig` sidecar (`write_sig`/`read_sig`/`all_sigs`) holding `{base_stamp, signature, label, source_url}`, cascade-deleted + pruned with its PDF like `.stamp`/`.jd`; git-ignored under `profile/tailored/` (no new PII — skill keywords + hashes + a company/role label). (3) `pipeline.tailor_base_stamp` (the JD-independent half of `tailor_stamp`) + `find_reusable(resume, profile, jd, *, threshold, exclude_path)` — scans the sig corpus for the best same-base, ≥-threshold hit. (4) `tailor_and_render` writes the sig on every fresh tailor (seeds the corpus). (5) `run_testing_mode` new branch order: `force_retailor` (fresh) → exact same-posting reuse (dry-run, unchanged) → **cross-posting reuse** (copies the hit's PDF, writes this posting's stamp + the SOURCE's signature so reuse chains don't drift, surfaces an honest note naming the source + match %, UI Principle #5) → fresh tailor. **Self-exclusion (preserves decision 069):** `find_reusable` skips the posting's **own** artifact (`exclude_path`), so same-posting reuse stays owned by the exact-stamp mechanism (dry-run) and the armed re-tailor — the decision-069 "armed submit of the same posting always re-tailors" invariant is intact; only a *genuinely different* posting is reused on armed. `force_retailor` is the escape hatch on both. **Tests:** `test_reuse.py` (8) — signature = demanded-skill set; identical/Jaccard/knockout-gated/empty similarity; dict round-trip; `find_reusable` hits + respects threshold/base-stamp/exclude; end-to-end second-posting-reuses-and-skips-tailor + dissimilar-posting-re-tailors (tailor/PDF/apply edges stubbed, no tokens/browser). All prior `test_rescan_reuse` (7) still green (self-exclusion keeps the armed-re-tailor test passing). | Accepted |
| 141 | 2026-07-28 | **Résumé-document upload: the Profile page now accepts a PDF/DOCX/TXT résumé, Claude parses it, and new entries are MERGED into `profile/resume.yaml`.** User: "I don't see a button to upload a new/updated resume anywhere" — correct: the only ways to populate the résumé were typing fields by hand or importing a LinkedIn data export (`.zip`/`.csv`); there was no path to upload the résumé document the user already has. This is a real gap for a résumé-driven pipeline, so a third import path was added (**user-chosen via AskUserQuestion: Claude parse + merge**). **New module `resume_import.py`:** (1) `extract_text(filename, data)` — PDFs via the already-bundled `pypdf` (no new dep), DOCX via **stdlib `zipfile`** (a .docx is a zip; text lives in `word/document.xml` — regex out `<w:t>` runs, break lines on `</w:p>`, so **no `python-docx` dependency**), plain `.txt`/`.md` decoded directly; scanned/image-only PDFs (no text layer) raise an actionable error pointing at exporting a text PDF (UI Principle #3). (2) `_parse_with_claude` — reuses the existing tailoring engines (`backends.run_claude_cli` on the subscription when Claude Code is available, else `backends.run_anthropic_api` with the keychain API key), forcing schema-valid JSON via a `_ParsedResume` model (same shape as `Resume` but with an **all-optional contact**, so a résumé missing an email still validates); `think=False` (a parse is extraction, not reasoning); no CLI and no key → a clear "needs Claude / or fill fields by hand" error, since parsing has no rules fallback. Hard "use ONLY what's in the text, never invent" system prompt (same guardrail family as tailoring). (3) `import_resume(path, filename, data)` — **merge semantics mirroring `linkedin.import_into`:** experience/activities dedupe on (org, role), projects on name, education on (school, degree), skills merged into matching categories deduped across all items, certifications on the string; **existing entries and non-blank contact/summary fields are NEVER overwritten** (only blanks are filled). **First-résumé case:** when `profile/resume.yaml` doesn't exist yet, the parsed document *becomes* it (so upload also onboards a brand-new user, not just updates). **Web wiring:** `POST /resume/import-file` mirrors `/resume/import-linkedin`; **target-path safety** — imports into the selected résumé only if it's an already-existing `profile/` one, else the fixed `profile/resume.yaml` constant, so a browser-supplied path can never write a shipped `examples/` file or traverse. **UI:** an "Upload your résumé" section on the Profile page (above the LinkedIn import) with a PDF/DOCX file input + "Upload & parse" button, reusing the shared busy-spinner pattern (UI Principle #5) and reporting exactly what was added/created/left-untouched; the "Import my résumé →" onboarding nudge now points here (uploading a résumé is the more natural first step than a LinkedIn export). **Safety unchanged:** résumé data is git-ignored PII; nothing is submitted. **Tests:** `test_resume_import.py` (11) — real PDF/DOCX/TXT extraction, unsupported/empty-file errors, first-upload-creates, re-upload-adds-nothing (dedupe), merge-adds-new-only, existing-fields-never-overwritten, and no-Claude error; parse step monkeypatched so no live model call. | Accepted |
| 140 | 2026-07-28 | **FAANG+ target companies are staged as validated ATS boards (not a name list), curated to early-career fit; the continuous runner becomes a notifying "watch."** User: "compile a list of FAANG+ and similar reputable companies we'll wire agents to search periodically," then "add the seasonal grad hirers back — I want the app to check them and notify me when they're ready to apply." **Correction surfaced before compiling (Guideline #2/#11):** a company is "wired to be searched" only by an `{ats, token}` entry on one of the six fetchable ATSs (`discovery.ATS_SOURCES`: greenhouse/lever/ashby/smartrecruiters/recruitee/workable). **FAANG-proper doesn't fit** — Google/Meta/Apple/Amazon/Netflix/Microsoft/Nvidia/Tesla/Salesforce/Adobe/Oracle/IBM run custom portals or Workday (an *apply* adapter with no per-company fetch source), so a famous-name list would mostly fail to wire. So the deliverable is **live-validated `ats:token` boards**, not names: 64 reputable-large-cap boards probed against the real ATS API (each returned ≥1 posting) and staged into the committed non-PII `data/source_candidates.json` (the decision-134 source-scout transport → one-click Accept, human-gated). **(a) A `GreenhouseSource` `job-boards.greenhouse.io` fallback was investigated and deliberately NOT built** (Guideline #2, simplicity-first): 5 well-known names (Grammarly, Unity, Retool, Applied Intuition, Rippling) 404 on `boards-api.greenhouse.io/v1/boards/{token}`, but every endpoint 404s for them — classic API, job-boards/embed hosts, the opaque board token from the vanity URL, AND the live board pages themselves (404, not 403 → "no such board", not blocked); no slug variant resolves. `boards-api` is Greenhouse's only public JSON API and already serves newer job-boards-hosted boards (Glean's `gleanwork` validated at 103), so a fallback would fetch zero boards — dead code. Those five have no public JSON board (Grammarly folded into Superhuman). Mistral recovered via `ashby:mistral.ai` (169). **(b) Curated by real data, not opinion:** each board's live postings were title-scanned for early-career technical roles (intern/new-grad/junior/associate/grad-year AND technical, minus senior markers); 25 with ≥1 such role today were kept, then the 15 dropped-but-known-seasonal grad hirers (Airbnb/Datadog/Reddit/Coinbase/Robinhood/Toast/Pinterest/Instacart/Anthropic/Brex/Affirm/Discord/Duolingo/Vercel/Gusto) were restaged (provenance-tagged "seasonal grad watch") → 40 candidates, because a snapshot of 0 open ≠ never-hires (UI Principle #5 — the drop was surfaced, not silent). **The watch (user-approved via AskUserQuestion — CLI continuous runner + notify-when-prepared):** wired the existing notification channels (decision 135) into `runner --continuous`, which already polls boards forever but pushed nothing. New `runner._notify_cycle(notifier, result)` fires **one summary notification per cycle** (not per-match, so a bursty first cycle can't spam) — `approval_needed` for the roles prepared + dry-run-filled ("N ready to apply: …"), `intervention_needed` (urgent) for any blocked — hooked in `run_cycle` after `run_queue`, guarded so it's built only for `--continuous` (a manual one-shot run stays quiet). `build_notifier()` respects the user's config (desktop on by default; ntfy for phone); a broken notifier is swallowed so it never breaks the watch. Zero change to `run_queue` (it already returns every outcome). Dry-run/safety unchanged — the watch only *notifies*, it never submits. **Tests:** 3 in `test_runner.py` (per-cycle summary of ready+blocked with correct events/urgency/titles; silent when nothing ready; swallows a raising notifier); 9/9 runner + 24/24 notifications green. **Still user-owned:** Accept the staged boards (nothing is searched until you do), then run `python -m applicationbot.runner --continuous`. | Accepted |
| 139 | 2026-07-28 | **Escape hatch for source discovery: bespoke `Source` adapters live as drop-in plugin files added via a reviewed PR — the loader auto-discovers them, so a PR touches zero core code.** Completes the source-discovery ladder: Phase 1 (decision 134) adds companies on supported ATSs as config; Phase 2 (decision 136) adds public-JSON aggregators as declarative specs; this handles the long tail — platforms a spec **can't** express (HTML-only listings, custom auth headers, non-standard pagination), which genuinely need hand-written Python. **Key design choice (Guideline #1, module boundaries):** rather than each new adapter editing `discovery.py` + `build_sources` + the config schema (the original decision-134 framing — a big, conflict-prone review surface), adapters are **self-contained modules in a new `applicationbot/sources_contrib/` package** that a loader auto-discovers. A PR that adds a platform is therefore exactly ONE new file (+ optional test), touching no existing code. Because it adds *code*, it is the only tier that goes through a **PR a human reviews and merges** — never auto-merged (contrast Phases 1–2, which stage data). **What shipped:** (1) `sources_contrib/__init__.py` with `load_contrib_sources(package=None) -> {NAME: module}` — iterates submodules, keeps those exposing a string `NAME` + callable `build(keywords) -> Source`, skips modules named with a leading `_` (templates/helpers) and any that fail to import or are incomplete (a broken adapter never breaks discovery). (2) `sources_contrib/_example.py.txt` — the contract + a filled reference adapter (custom auth headers + total-pages pagination), a `.txt` so it's never imported/run; the package ships **no live adapters** by default (a fresh clone runs nothing until a PR adds one). (3) `filters.DiscoveryFilters.contrib_sources: list[str]` (enable-by-name, off by default) wired into `build_sources` (looks up each name in the registry, `build(kws)` with profile-derived keywords; a missing name or an adapter that raises on build is skipped). (4) `source_scout.enable_contrib_source` (idempotent, gated on the name being a loaded adapter). (5) **Web/UI:** `GET /candidates` also returns loaded-but-not-enabled adapters; the "New sources found" panel lists them as `name (custom adapter) — description` with one-click **Add** → `POST /candidates/accept-contrib`; `/sources` shows enabled custom adapters. **Tests:** `test_sources_contrib.py` (loader discovers valid / skips underscore+broken+incomplete via a temp fixture package; `build_sources` runs an enabled adapter + skips unknown; enable idempotency; shipped package loads zero adapters) + `test_web_candidates.py` +2 (adapters listed/enabled). Served JS `node --check` clean. **Verified against the real shipped code:** dropped a real adapter file into `sources_contrib/`, confirmed the loader discovered it, `build_sources` instantiated it, its `fetch()` returned a posting, and a live `GET /candidates` listed it — then removed it so the package ships clean. **Routine (`docs/SOURCE_SCOUT_ROUTINE.md`):** a Phase 3 mode — read the template, write ONE `sources_contrib/<name>.py`, smoke-test it loads+fetches, `gh pr create` on a branch (never edit core, never merge), report the PR URL + any ToS caveat. **Boundary (Guideline #4):** adapters must respect ToS/robots and do no bot-detection evasion; a platform that can't be read within those rules is reported, not built. | Accepted |
| 138 | 2026-07-28 | **Notifications action-center tab + honest notification click-through (mobile is informational, desktop opens the app on this Mac).** Two bugs + a feature the user reported: (1) tapping a **phone** (ntfy) notification opened `http://127.0.0.1:…`, which the phone can't reach — the loop runs on the *Mac* and the server is bound to localhost; (2) clicking the **desktop** notification opened a file-open dialog (Script Editor); (3) they wanted a **Notifications tab** to see and act on everything needing them from inside the app. **User-approved scope (AskUserQuestion):** the tab is a **live action center** (not a history log), and mobile **stays localhost-only + informational** (not LAN-exposed) — its security model assumes loopback (origin guards, "bound to 127.0.0.1"). **Root causes:** ntfy set a `Click` header to the localhost `link_base` (unreachable off-device); `osascript display notification` (the from-source poster) can't carry a click action, so clicking is attributed to the posting app = Script Editor; and the `/#discover` deep-link in notification bodies never worked because **no hash router existed** — the fragment was decorative. **What shipped — notifications.py:** (a) `NtfyChannel` no longer sets `Click` (dropped `link_base`); the phone push is informational, its body says to open ApplicationBot on the Mac. (b) `DesktopChannel` gained `link_base` + an optional **terminal-notifier** path: off-frozen, if `terminal-notifier` is on PATH it posts a *clickable* notification (`-title/-message/-open <localhost-url>`) that opens the app on this Mac (localhost IS reachable here); otherwise it falls back to the existing native (frozen, app-icon, click-activates-app) / osascript (informational) poster — `which`/`runner` injected for tests, no new dependency. (c) `build_notifier` passes the localhost `link_base` to desktop, not ntfy. **web.py — the tab:** a new **Notifications** nav tab (bell icon) with a **count badge** (`#notif-badge`, hidden at zero) fed by a new `GET /inbox` endpoint that returns `{ready, parked, count}` resolved live from the tracker (a submitted/resolved item drops out on the next fetch — no stale entries, no new store). `loadInbox()` reuses the **exact** Discover cards (`parkedCard` / `loopReadyCard`) so a fix here is identical to one there, blocked-first then ready-to-submit, with an all-caught-up empty state. **Silent-submit guard (UI Principle #5):** the submit/watch/resolve actions render their progress into Discover's shared panels (`loop-msg`, `test-progress`), so `watchReady`/`applyReady`/`reapplyParked` now call `ensureDiscoverVisible()` (hop to Discover if hidden) before running — an action taken from the Notifications tab always shows its live progress instead of writing into a hidden panel. **Hash router (new):** `applyHash()` maps `/#<view>` → tab, on load + `hashchange`, so notification links (now `/#notifications`) actually navigate; a background 30 s `refreshBadge()` keeps the count live. **TDZ fix found by driving the app:** the initial `applyHash()` fired a tab click during synchronous script init → `maybeShowNudge` read `TOUR_ACTIVE` (a later `let`) in its temporal dead zone → `setTimeout(applyHash, 0)` defers it past init. Notification bodies + the test notification now link `/#notifications`. **Verified by driving the running app (Playwright, real data — 2 real blocked SmartRecruiters postings):** `/inbox` returns count 2; nav badge shows "2"; `/#notifications` deep-link selects the tab; both blocked cards render with Review; inline Review loads its submit panel; `ensureDiscoverVisible()` reveals Discover; badge/re-render correct; **zero console/page errors** on both hash and plain loads. Served JS `node --check` + `py_compile` clean. **Settings hint (user-approved follow-up, same session):** `notifications.desktop_click_status()` reports whether clicking a desktop notification can open the app (frozen→native yes; from-source→only if terminal-notifier on PATH; off-macOS→n/a), surfaced via `GET /notifications` (`desktop_click`). The Settings → Notifications panel shows a one-line tip **only when click-through is unavailable** (`applicable && !clickable`) — "install terminal-notifier: `brew install terminal-notifier`" with a Copy button — hidden in the packaged app and once installed. **Tests:** `test_notifications.py` updated (ntfy asserts NO `Click` even with a link) + desktop-channel terminal-notifier tests (clickable `-open` URL; fallback to poster when absent) + 4 `desktop_click_status` tests (from-source-missing / terminal-notifier-present / frozen-native / off-mac); 31 pass in the file, all suites green. **Verified by driving the app:** with terminal-notifier absent the hint renders with the right command and Copy writes it to the clipboard; 0 console errors. **Not done:** the from-source desktop notification is still click-inert *until the user installs* terminal-notifier (osascript can't deep-link — the in-app Notifications tab is the always-available fix); LAN-reachable mobile deep-link intentionally declined (security). | Accepted |
| 137 | 2026-07-28 | **A dedicated Settings tab is the home for set-once, cross-cutting config; operational controls stay where they're used.** Push-notification setup already shipped (decision 135) as a collapsible "Notify me…" panel inside the Discover tab — the user asked whether that belongs in its own settings tab and, if so, what else should fold in. **Framing (Guideline #1 — affects nav/UI structure, so options presented before code):** the useful line is *set-once preference/config* vs *operate-the-pipeline control*. The Discover tab was carrying 7 panels (loop, notify, dry-run, parked, sources, insights, candidates) — the app's most crowded surface — and "settings" were scattered across three places: notifications in a Discover `<details>`, the Claude connection behind a sidebar-footer modal, and the linked inbox (`mailboxPanel`) buried as a section inside the Profile form. **Options weighed:** (A) new 5th Settings tab holding the set-once config; (B) Settings tab but notifications only; (C) leave notifications in Discover, no tab. **User chose A**, and to fold in **Claude connection + Inbox linking (IMAP) + Theme** — but explicitly **NOT discovery filters** (they're core to Discover's workflow; the auto-apply loop likewise stays in Discover as an operational control). **What shipped (`web.py`, HTML+JS relocation, all element IDs preserved so existing handlers/endpoints are untouched — Guideline #7):** (1) a 5th nav tab `settings` (sliders icon) + `#view-settings` with four `.editor` cards — **Claude connection** (the former modal's `#claude-active/#claude-sub/#claude-key` render targets, now inline; `renderClaudeModal` reused as-is and painted once on load), **Notifications** (the moved panel, un-collapsed, same `ntf-*` IDs + `loadNotifications`), **Linked inbox** (`mailboxPanel()` re-mounted into `#set-mailbox-mount` once per session, then `loadMailbox()`), **Appearance** (a `.viewtog` System/Light/Dark segment reusing existing CSS). (2) The `#claude-modal` scrim + open/close/esc logic **deleted**; the sidebar-footer connection chip now deep-links to Settings → Claude via a new `goSettings(anchorId)` (UI Principle #2 — the fix, one click), and a live status chip stays in the footer. (3) Discover's notify panel replaced by a one-line "Set up notifications in Settings →" link (`goSettings('set-notify')`) so it stays discoverable in the loop's context. (4) Inbox panel removed from `renderProfileForm` + its section-jump nav + the `loadMailbox()` call in `loadProfile` (its store is the OS keychain, independent of the profile YAML — safe to move). (5) Theme IIFE refactored to expose `window.setThemeMode`/`reflectTheme`, driving both the footer toggle and the new segment. (6) Tour updated: "four sections" → "five", new Settings step. **Verified by driving the running app (Playwright, headless Chromium):** Settings tab renders all four sections with **zero console/page errors**; the linked Gmail shows live "✓ Connected" status; theme segment sets `data-theme` + highlights the active choice; the footer chip and the Discover link both navigate to Settings; Profile still renders (10 sections, no "Bot email" jump entry, no duplicate `mb-email` ID). Served-page JS `node --check` clean; `python -m py_compile applicationbot/web.py` clean. | Accepted |
| 136 | 2026-07-28 | **Phase 2 of auto source discovery: a new aggregator platform is declarative DATA (a validated `AggregatorSpec`), not a bespoke `Source` subclass.** Decision 134 deferred "new source *types*" to a Phase 2 that would open a PR with a hand-written adapter. **Reversed that framing (Guideline #2, user-approved):** the keyless JSON aggregators already in the tree (Himalayas/RemoteOK) are each a ~40-line class that GETs a JSON endpoint, walks to the jobs array, and maps fields → `Posting` — pure boilerplate that varies only in endpoint / list-path / field-map / pagination. So instead of the routine writing Python per platform (bespoke class + config field + `build_sources` branch + review + merge), **one generic `JsonApiSource` is driven by an `AggregatorSpec`**, and a "new aggregator" becomes a validated spec staged exactly like a Phase 1 board — no routine-written code, no unattended merge, one-click enable. **Options weighed:** (A) generic declarative source + spec; (B) per-platform generated adapter class + PR. **Chose A** (covers the common case — Remotive/Arbeitnow/The Muse/USAJobs/Findwork/JSearch are all public JSON APIs); a genuinely unmappable platform (HTML-only, odd auth) remains the escape hatch: a hand-written adapter via PR. Keyless only, by design (Guideline #4 / decision 026). **What shipped:** (1) `discovery.JsonApiSource` + `AggregatorSpec` (endpoint template with `{q}`/`{limit}`/`{offset}`/`{page}`, dotted `list_path`, dotted `field_map`, offset/page pagination, `remote` flag) — emits `ats="jsonapi"` so ONE new `_AGGREGATOR_ATS` entry rides every declarative source through the existing aggregator→ATS bridge + `_is_fillable` path (real platform name kept in `Source.name` + `extra['platform']`); a job missing a title or apply URL is skipped, results deduped by canonical URL. (2) Committed **non-PII registry** `data/aggregator_specs.json` (`discovery.load_aggregator_specs`) — shareable spec data, like `_BUILTIN_FEEDS`; the routine appends here. (3) `filters.DiscoveryFilters.json_aggregators: list[str]` (enable-by-name, off by default) wired into `build_sources` (looks up each enabled name in the registry, instantiates `JsonApiSource` with profile-derived keywords; a query-based spec self-skips without a résumé, like the other aggregators). (4) `source_scout.validate_spec` (field_map must name title/company/apply AND a live fetch must return ≥1 usable posting — else the exact reason), `stage_spec` (validate → upsert into the registry, stamping the sample so the UI needn't re-probe), `enable_json_aggregator` (idempotent append to `json_aggregators`); CLI `--add-spec '<json>'` / `--keyword` / `--list-specs`. (5) **Web/UI:** `GET /candidates` now also returns not-yet-enabled registry specs; the "New sources found" panel lists them (`name (aggregator) — N roles, e.g. …`) with one-click **Add** → `POST /candidates/accept-spec` → `enable_json_aggregator`; the `/sources` overview shows enabled JSON aggregators. **Bug fixed in passing (Guideline #7):** `collectDisc()` rebuilds the whole filters doc on save but had no `json_aggregators` control, so saving Discovery settings would silently wipe an enabled aggregator — now round-tripped via `_discPreserve`. **Tests:** 18 new (`test_jsonapi_source.py` 10: mapping/dotted-paths/root-array/`{q}`-per-keyword/offset-pagination/skip-missing/dedup/max-results/bridge-eligibility/build_sources wiring; `test_source_scout_specs.py` 5: field-map validation, 0-postings, live pass, registry upsert-only-when-valid, enable idempotency; `test_web_candidates.py` +3: specs surfaced/enabled/validated). Served JS `node --check` clean. **Verified against reality:** staged the real **Remotive** keyless API (`remotive.com/api/remote-jobs`) end-to-end — validated 15 live postings, upserted to the registry, surfaced in a live `GET /candidates`. **Routine (`docs/SOURCE_SCOUT_ROUTINE.md`):** a Phase 2 mode added — research a platform's public JSON API, draft the spec, `--add-spec` to validate+stage, commit. | Accepted |
| 135 | 2026-07-28 | **Push notifications for the auto-apply loop's human-in-the-loop moments — pluggable channels (native desktop + ntfy now, Telegram/others later), on both the built app and localhost.** User: "when running the auto-apply loop, look into push notifications for human approval before applying / if the bot needs human intervention (both built app and localhost)." **Problem:** the loop (`autoloop.auto_apply_loop`, decision 069/121) already prepares each match then *waits* for the user to click Apply, and parks blocked applications (`parking.py`), but both signals were visible ONLY while a browser tab / the app window was open and being watched — `_LOOP_STATE` is polled by the page every 2s (`/loop/status`); nothing pushed. The loop could sit "ready for your approval" indefinitely with no way to know. **Key simplification (Karpathy simplicity-first):** the web server runs on the user's own Mac for BOTH surfaces, so ONE server-side native macOS notification (`osascript`) covers the packaged app *and* a localhost browser session — no per-surface notification code, no browser Web-Notifications/service-worker/VAPID path. **Chosen delivery (user-approved):** ntfy + desktop now, designed pluggable so Telegram/other chat apps drop in later. New `applicationbot/notifications.py`: a `Notification` (event/title/body/link/urgent), a `Channel` protocol, two built-in channels — `DesktopChannel` (macOS; non-Darwin degrades to a no-op, never errors) and `NtfyChannel` (one `urllib` POST to an ntfy.sh topic — no new dep; a topic is treated as a shared secret and the user picks an unguessable one) — and a `Notifier` that fans out with **per-channel isolation** (a channel raising is swallowed, never breaks the loop or the other channels). Adding Telegram/Pushover/Slack later = one class implementing `Channel.send` + one branch in `build_notifier`. **SSL hardening:** `NtfyChannel` verifies against certifi's CA bundle (shipped via the `anthropic` dep) so the classic macOS "CERTIFICATE_VERIFY_FAILED" (system-Python / frozen-app trust store) doesn't silently break phone push — found and fixed by a live test. **App icon (user follow-up):** the macOS notification shows the *posting process's* icon, and `osascript` always posts as Script Editor — so `DesktopChannel` posts **in-process via pyobjc `NSUserNotification`** in the packaged app (which serves in-thread inside the pywebview process, decision 103), inheriting the `com.applicationbot.macos` bundle icon; a from-source/localhost run has no bundle to attach to, so it falls back to `osascript` (generic icon, always works). `_default_poster` gates native on `sys.frozen`, with an osascript fallback on any pyobjc error, and delivery is dispatched to the main thread (the loop posts from a background server thread). **Flagged live step:** confirm the ApplicationBot icon actually renders on a real notification from the built `.app` — can't be checked from the dev shell (not frozen → osascript path). **Two events, fired from `web.py _loop_worker.prepare_one`:** `approval_needed` when an app becomes "ready" (deduped by the existing `ready_ids` guard so each app pings once), `intervention_needed` (urgent) when a prepared app parks as `blocked`; each carries a `/#discover` deep-link (UI Principle #3). Config at git-ignored `profile/notifications.yaml` (`examples/notifications.example.yaml`); defaults desktop-on/ntfy-off/both-events-on. **UI (both surfaces, same served page):** a "Notify me when an application is ready or needs me" panel inside the Discover-tab loop card (right where the loop is started) — desktop toggle, ntfy toggle + topic, per-event toggles, Save, and a **Send test** button (`GET /notifications`, `POST /notifications/update`, `POST /notifications/test`) that fires a real notification through the posted (unsaved) config and reports per-channel success/exact-error (UI Principles #1/#3/#5). **Verified live:** drove the running server — GET/update round-trip persists; `/notifications/test` fired a real macOS notification AND a real ntfy push (`Sent via desktop, ntfy`); served-page JS `node --check` clean; `import applicationbot.web` clean. **Tests:** 15 in `test_notifications.py` (desktop osascript + AppleScript escaping + off-Mac no-op; ntfy topic/headers/body/Click/priority; notifier fan-out + failing-channel isolation + event toggles; config default/roundtrip/unreadable-fallback; build_notifier channel selection), all injected — no real shell/network. **Not done (deferred):** Telegram/Pushover channels (interface ready); Windows/Linux desktop notifications (currently no-op off macOS); browser-closed Web Push. | Accepted |
| 134 | 2026-07-28 | **Auto-discover new job sources via a periodic Claude cloud routine — deterministic wiring in-repo, fuzzy web search in the routine, human gate before anything runs.** User asked to "use a Claude cloud routine to periodically scrape the web for new aggregators/job sources and wire them into ApplicationBot automatically." **Two corrections surfaced before coding (Guideline #2 / Karpathy "push back"):** (a) **Source count is not the bottleneck — throughput is** (decision 114 / NEXT_STEPS 2026-07-22 research: the qualified pool is ~50× oversubscribed against `max_resolve`). So this does NOT chase volume; it grows *quality* boards modestly (~15/run cap) and rides the funnel work already queued. (b) **"Wire in automatically" splits into two opposite automation stories:** a *config-only* source (a new company on an ATS we already support — pure `{ats, token}` append, zero code) is safe to auto-stage; a *new source type* (a brand-new aggregator platform) needs a bespoke `Source.fetch()` adapter + config model + tests and CANNOT be safely written/merged unattended. **Decision: ship in two phases — Phase 1 (config-only, built now); Phase 2 (new platforms → a reviewed PR, deferred).** **Options weighed for the cloud→local handoff:** (1) committed non-PII candidates file + one-click app approval; (2) PR appending to a shared defaults file; (3) fully-automatic direct append. **Chose (1)** (user-approved): the routine's only output is a commit to `data/source_candidates.json` (public company ATS slugs + coarse provenance + validation result — no résumé/profile PII, so it's git-safe and git IS the cloud→local transport); the user reviews and adds with one click (or `--accept`), keeping a human gate between "found on the web" and "runs in my pipeline," and keeping the git-ignored `profile/` out of the cloud. **What shipped (Phase 1):** `applicationbot/source_scout.py` — the deterministic half: `extract_board(url)` (classify ATS + pull token across every supported URL shape incl. Greenhouse `?for=` widget, path-style and subdomain-style boards), `validate_board` (probe the live public ATS API; a token counts only if it returns ≥1 posting — dead/misspelled slugs kept unvalidated with the exact error), dedup vs `filters.boards`, the committed candidates store (load/save/merge), `merge_into_filters` (the one-click accept → append to `profile/discovery.yaml` boards, idempotent), and a CLI (`--url`/`--from-discovery`/`--accept`/`--list`). **Scope guard (NEXT_STEPS "don't overload `ATS_SOURCES`; separate can-discover from can-autofill"):** the allowlist is exactly `discovery.ATS_SOURCES`, so a candidate is only ever "another company on a board we already read AND already know how to apply to" — it asserts no new autofill adapter and touches neither `ATS_SOURCES` nor `_is_fillable`. **Allowlist-only (Guideline #4):** the routine searches only the six supported ATS domains; LinkedIn/Indeed and other excluded consumer boards are never queried or proposed. **Routine spec:** `docs/SOURCE_SCOUT_ROUTINE.md` (weekly cadence — the company→ATS landscape moves slowly and a faster cadence just grows an unreviewed backlog; the exact `/schedule` prompt + domain allowlist + commands). Not auto-registered — creating a billed recurring cloud agent is the user's call (Framework #4). **Tests:** 20 in `test_source_scout.py` (extraction across all ATS URL shapes + rejects LinkedIn/Indeed/unknown; case-insensitive dedup; skip-already-configured; store round-trip + merge keeps original provenance; non-PII header; accept idempotency). Live-probe path smoke-verified: dead token → graceful `HTTP 404` with actionable error; real board (`greenhouse:gitlab`, 187 postings) → validated. **Settings UI (shipped, same session):** a "New sources found" panel in the Discover tab's `.disc-grid` (`web.py`) — `GET /candidates` surfaces only validated, not-already-configured candidates; each row's one-click **Add** POSTs `/candidates/accept` → `merge_into_filters`, then refreshes the boards form + sources overview. Panel stays hidden when nothing is staged (mirrors `fit-insights`). 3 route tests (`test_web_candidates.py`, live-server harness) + live GET verified against the real app (staged `greenhouse:gitlab` → surfaced; unvalidated filtered). **Deferred (Phase 2):** the new-aggregator-*platform* PR-proposing routine. | Accepted (Phase 1) |
| 133 | 2026-07-23 | **A redirect is not a reason to drop a posting: unresolved `email_alert` leads are deferred to apply-time browser click-through, not stamped non-fillable.** User feedback correcting decision 132's framing: *"I don't mind bots having to jump through a couple hoops to get to an application; I only care that the user has a smooth experience. A lot of job applications use a redirect and we should not be counting them out just because of that."* Decision 132 shipped `email_alert` leads riding the aggregator→ATS bridge, but the bridge's `else` branch (redirect not resolvable server-side) stamped `auto_applyable=False`, which `pipeline._is_fillable` drops **pre-judge** — so a Lensa/Aflac lead whose link is tracking-wrapped, hits a JS interstitial, or lands on an ATS we don't detect server-side (e.g. Aflac's SuccessFactors `careers.aflac.com`, which `detect_ats_from_url` returns `"other"` for) was silently counted out, exactly the behavior the user objects to. **Fix (`discovery.py`, targeted):** new `_BROWSER_GATED_FALLBACK = {"email_alert"}` — aggregators we still TRY to resolve server-side (keeping decision 132's free upgrade of Lensa reposts that DO 30x-redirect to Greenhouse/Lever), but whose unresolved hits are tagged `extra['browser_gated']=True` and **left in the funnel** (auto_applyable unset → `_is_fillable` keeps them via `ats in _AGGREGATOR_ATS`) instead of dropped. At apply time the real browser opens the link and `apply._open_application_form` clicks through the redirect (`\bapply\b` reveal → re-derive the ATS from the resulting frame) — the same proven path Adzuna's `_BROWSER_GATED_ATS` uses (decision 120). Net effect: Aflac postings and tracking-wrapped Lensa leads now reach the judge + Apply instead of being hidden. **Distinction preserved (Guideline #4, unchanged):** this is about *following an apply redirect in the real browser* (normal navigation the bot may do), NOT about scraping a discovery-listing endpoint that blocks automated reads — Lensa's `lensa.com/jobs` 402 wall is still not scraped; those postings come via the forwarded emails. **Scope:** applied only to `email_alert` (Guideline #7 — not silently changing jooble/himalayas/remoteok, though jooble is the same redirect-aggregator class and is flagged in NEXT_STEPS as the candidate to extend this to). **Per-provider boundary (added after validating against the real inbox, 2026-07-23):** "the browser may follow the redirect" is true only when the link IS a redirect to an external apply form. Driving the linked bot inbox surfaced **79 real LinkedIn job-alert leads** — and those links are `linkedin.com/comm/jobs/view/<id>`, LinkedIn's OWN login-walled, robots-disallowed pages, not a redirect to an ATS. Auto-driving them would cross LinkedIn's ToS (Guideline #4, the exact line decision 072 drew). So `AlertProvider` gained an `auto_apply` flag: Lensa/Aflac `True` (browser-gated fallback applies), **LinkedIn `False`** — its leads set `extra['auto_applyable']=False` + `manual_only=True` at emit, are excluded from the bridge (`to_bridge` now skips `auto_applyable is False`, so we never resolve-hit a robots-disallowed site), and surface as manual-apply leads (`_is_fillable` False) instead of being auto-driven. Also fixed via the real markup: LinkedIn `url_contains` tightened to `/comm/jobs/view` (was `/comm/jobs`, which also caught the alert's own `/jobs/search-results/` link), and `email_alert` dedup now strips the query string (LinkedIn recurs the same posting across sends with a unique `trackingId` that `canonical_url` keeps). **Verified on real data:** 79 distinct LinkedIn postings parsed, deduped, all `manual_only`, bridge made zero network calls against them. **Tests:** `test_source_ats_is_bridge_eligible` replaced by two bridge tests (resolved→`auto_applyable`; unresolved→`browser_gated`+kept), plus two LinkedIn tests (`/view/`-only + manual-only + cross-send dedup; bridge skips manual-only). 13 `test_email_alerts.py` pass. | Accepted |
| 132 | 2026-07-23 | **Lensa + Aflac support via a generalized forwarded-email alert source (finally builds decision 072's LinkedIn pattern); Lensa direct-search rejected as a bot-walled redirect aggregator.** User is subscribed to Lensa and "Aflac" and asked to add both "through forwarded emails or direct site search." **Two corrections surfaced before coding (Guideline #2/Karpathy "surface confusion"):** (a) **Aflac is not an aggregator** — `careers.aflac.com` is one insurance company's own SuccessFactors careers site posting mostly commission-based 1099 sales-agent roles; user chose to keep it **email-only** (no scraping). (b) **Lensa direct-search is not viable** — `lensa.com/jobs` returns HTTP 402 (bot/payment wall) to this environment, the same class of gate as Adzuna's CloudFront WAF (decision 120); its apply links are redirect-only leads (the noise decisions 122-124 down-rank). Building a server-side scraper for it would mean evading bot detection (Guideline #4) for low-value un-appliable leads (Guideline #0), so the "Both" answer collapses to **email-only for both sites** — the direct half is reported, not built. **What shipped (the ToS-safe path settled for LinkedIn in decision 072, now generalized to any provider):** (1) `mailbox.fetch_alerts`/`_gmail_fetch_alerts` — a sibling of `fetch_verification` that returns whole HTML/plaintext bodies (newest-first, sender-filtered) over both IMAP and Gmail-OAuth, plus `_body_html` (prefers the HTML part so `<a href>` links survive, unlike `_body_text`). (2) `discovery.EmailAlertSource` + `AlertProvider` — parses each forwarded alert's `<a>` links into **leads** via a **structure-agnostic** parser (`_AnchorExtractor` + `_extract_job_links`) that keys on the DESTINATION domain (`url_contains`), NOT the email's card layout — which varies per sender and can't be guessed without a real sample (decision 072's rule); `_ALERT_LINK_STOPWORDS` drops unsubscribe/footer/social links. Built-in providers: `lensa`/`aflac`/`linkedin`. Emits `ats="email_alert"`, `extra={snippet_only, alert_provider}`. (3) **Reuse over new code:** `email_alert` is added to `_AGGREGATOR_ATS`, so the existing decision-032 aggregator→ATS bridge resolves each lead's redirect — a Lensa repost that lands on Greenhouse/Lever becomes auto-applyable for free; anything that doesn't resolve stays a tracked lead (`auto_applyable=False`, dropped pre-judge by `_is_fillable`, no Claude tokens). **[Superseded by decision 133: unresolved leads are now deferred to apply-time browser click-through, not dropped — a redirect is not a reason to count a posting out.]** No new apply path, no new fillability logic. (4) Config `filters.EmailAlertsConfig` (`enabled`/`providers`/`limit_per_provider`, off by default, provider-name validated) wired into `build_sources` behind a linked-inbox check (`mailbox.load_config()` — self-skips if the inbox isn't linked, so a keyless clone is unaffected). (5) Discover-tab settings: an enable toggle + provider checkboxes + limit, a `/sources` overview row (warns when enabled-but-inbox-unlinked → Profile tab), collectDisc/round-trip. **Tests:** 10 new (`test_email_alerts.py`: link parser keeps only provider job links + drops footer/off-domain/empty-text; `fetch_alerts` newest-first/sender-filtered/limit/error-swallow over IMAP + Gmail-OAuth; source emits deduped leads, multi-provider, max_links cap, bridge-eligibility of `email_alert`). Full suite 417 passed (pre-existing `test_parking` bot_wall + `test_nav_recipes` collection errors unrelated); served JS `node --check` clean; config round-trips through save/load. **Flagged live step (only the user can do it — same shape as decision 072):** set up the Gmail forward (Lensa/Aflac/LinkedIn alert sender → bot inbox), forward a real alert, and confirm the `url_contains` defaults match the real link markup (they may be mailer/tracking-wrapped) — tuning is a one-line change per provider. Until then the parser is verified against the CONTRACT (a plausible alert fixture), not a real Lensa/Aflac email. | Accepted |
| 131 | 2026-07-22 | **Workday create-account: robust custom-widget checkbox + report the DISABLED button gate (no more timing out on a dead Create Account click).** Next live Hartford run got past decision 130's form-render fix but failed with *"the Create Account button wasn't found or couldn't be clicked"* — email + password filled, but the final click failed. Diagnosis: the generated password already meets Workday's complexity (16 chars, upper/lower/digit/symbol — [`generate_password`](applicationbot/workday.py)), so the button was almost certainly **disabled**, and `_click`'s `.click()` was waiting the full timeout on a disabled control. Most likely gate: Workday hides the real `<input type=checkbox>` behind a custom styled box, so [`_check`](applicationbot/workday.py)'s `.check()` on the hidden input silently failed → terms unticked → button never enabled. **Changes (adapter-only):** (1) `_check` is now robust to the custom widget — tries the native `.check()`, and on failure **clicks the visible wrapper/label** and confirms the input's `checked` state (short-circuits if already checked). (2) New `_create_submit_button` (prefers an inner `<button>` when the automation-id sits on a wrapper), `_is_disabled` (native `disabled` OR `aria-disabled=true`), and `_create_disabled_reason`. `create_account` now, after filling + ticking consent, **polls up to 2 s for the button to enable**, and if it's still disabled returns a specific reason — naming the consent checkbox when that's the unmet gate, else "stayed disabled … this tenant enforces an extra requirement (stricter password policy / additional required field / different consent control)" — instead of timing out on a dead click. A genuine click exception now reports the exception type. **This directly extends decision 130's descriptive-error goal** (Guideline #11): a disabled button is now distinguished from a missing one. **Tests:** 1 new (`test_create_account_reports_disabled_button_with_gate`: a disabled submit + a hidden-input custom checkbox → reason names the disabled button + checkbox); the existing success-path test still green (24/24 `test_workday.py`). **Still not verifiable here** (no live tenant/account) — if the next live run STILL reports the button disabled *after* the checkbox reason is ruled out, the gate is a stricter Hartford password policy or an extra required field, and the reason string will say so. | Accepted |
| 130 | 2026-07-22 | **Workday create-account: wait for the async form + report the exact step that failed (no more bare "could not be completed").** After decision 129 got past the Apply gate, a live run against `thehartford.wd5.myworkdayjobs.com` failed at account creation with *"Workday account-creation form could not be completed for <tenant>."* — a bare boolean with no clue which step broke. Same root-cause class as 129: [`create_account`](applicationbot/workday.py) clicked the "Create Account" toggle and then filled the email/password fields with **instant** `.count()` checks, but Workday swaps the sign-in form for the create form asynchronously — an unrendered-yet field read as "not found." **Changes:** (1) new `_visible`/`_wait_visible` helpers + `_reveal_create_form` — switches the account screen to the create form by automation-id toggle (`createAccountLink`), falling back to clicking a control named "Create Account", then **polls up to ~6 s** for `createAccountSubmitButton` before filling. (2) `create_account`'s return contract changed from `bool` → **reason string** (`""` = success, else a one-phrase reason naming the exact failed step: create form never appeared / email field not found / password field not found / confirm-password couldn't be filled / Create button not clickable). The confirm-password fill is now gated on the field actually being visible (was an unconditional best-effort call). (3) `ensure_account` surfaces that reason in an actionable error (**per the user's explicit ask for more descriptive messages**, Guideline #11 / UI Principle #3): it names the failed step, says to re-run once (a slow render is now waited out, so transient failures clear), and — if it repeats — points the user to create the account manually at the tenant URL. **Honesty check:** the credentials CLI has only `list`/`get`/`delete` (no `set`), so the message does NOT promise the bot will sign in with a manually-created account (it couldn't — the password wouldn't be stored); it frames manual creation as unblocking *this* application, not restoring automation. **No behavior change to sign-in / verification / wizard.** **Tests:** the 3 existing `create_account`/`ensure_account` tests updated to the `""`=success contract; 2 new (`create_account` on a page with no create form returns a reason naming the missing form; `ensure_account` surfaces the reason + tenant URL). 23/23 `test_workday.py` green. **Still not verifiable here** (dev never submits, no live tenant/account) — if Hartford's screen is a two-step "enter email → then password" layout rather than a single form, the new descriptive reason (e.g. "the password field wasn't found") will pinpoint it on the next live run for a targeted follow-up. | Accepted |
| 129 | 2026-07-22 | **Workday `start_application` now waits for each async SPA transition instead of checking once — fixes "the 'Apply' / 'Apply Manually' step wasn't found" on live postings.** User hit that error auto-applying to a real Workday posting; the manual flow works (click **Apply** → autofill popup: *Autofill with Resume* / *Apply Manually* / *Apply With LinkedIn* → account/wizard). **Root cause (timing, not selectors):** `apply.py` navigates with `wait_until="domcontentloaded"`, which returns *before* Workday's React app mounts the Apply button, and after the Apply click the autofill popup mounts asynchronously too. The old [`start_application`](applicationbot/workday.py) did only instant `.count()`/`.is_visible()` checks with a flat 600 ms `wait_for_timeout` between clicks — so if a control hadn't rendered *yet* it saw nothing, set `clicked=False`, broke the loop, and reported the step "wasn't found." Secondary gap: it matched only `button`/`link` roles with `exact=True` (case-sensitive), but the popup options are frequently `menuitem`s and casing varies by tenant. **Change (adapter-only, best-effort, never raises — the live-tuning surface per the module docstring):** added `_find_apply_control` (matches `_APPLY_TEXTS` in order — manual path still preferred over autofill — across `button`/`link`/`menuitem`, anchored case-insensitive regex so `^Apply$` won't hit "Apply Manually"/"Apply With LinkedIn") and `_wait_for_apply_step` (polls up to ~9 s, 300 ms cadence, returning `"form"` once `_on_account_or_form` is true or the control once it's visible). `start_application` now loops `max_clicks=4` (was 3): each iteration waits for the next state to settle, clicks, and repeats — so a slow initial render or a slow popup mount is waited out, not misread as a missing step. `_APPLY_TEXTS`, `_on_account_or_form`, `ensure_account`, and the rest of the flow are unchanged; no callers/tests depend on the old signature (`grep` clean). **Not verifiable from this environment** (dev never submits against a real posting, Guideline #3; account creation + a live tenant are required) — the poll/click logic parses and is behavior-preserving on the already-rendered case; effect to be confirmed on the user's next live Workday run. **Possible follow-up if it still misses:** Workday embedded in an iframe on a company careers site — `page.get_by_role` searches only the main frame; would need a frame sweep like `_find_form_frame`. | Accepted |
| 128 | 2026-07-22 | **Save any tracked application's posting as a reusable tailoring fixture, and add a Track-tab "Retailor résumé →" button that (optionally) saves it then jumps to the Review tab pre-loaded.** User wanted to (a) add any application to the Review tab's fixture picker and (b) retailor an application's résumé from Track with a one-click hop to the tailor tab. **Data layout (the one architectural choice):** user-saved fixtures live in a NEW `USER_FIXTURES_DIR = DATA_ROOT/profile/job_fixtures/`, NOT the shipped read-only `fixtures/job_descriptions/` bundle — because `profile/*` is already git-ignored (so saved postings are never committed) and `DATA_ROOT` is writable in both dev (repo root) and the packaged app (`~/Library/Application Support/ApplicationBot`), where the bundle is read-only. A saved fixture is a `load_job_description`-compatible Markdown file (YAML front matter `company`/`title`/`source_url`/`saved_from`/`date_captured` + body); its filename keys on `company-title-<sha1(source_url|body)[:8]>` so re-saving the same posting **overwrites in place** (idempotent, no dupes). **Backend (`web.py`):** `list_fixtures()` now returns saved fixtures (labeled "Company — Title · saved" via `_saved_label`, read from front matter; listed first) followed by the shipped ones; a saved fixture's `path` token is relative to `DATA_ROOT`, a shipped one's to `REPO_ROOT`. New `_fixture_path(token)` resolves a token to its file in the correct root **only if it's a member of `list_fixtures()`** (path-traversal-safe — a `../../etc/passwd` token is rejected), replacing the old `_allowlisted(...)` call in `do_tailor` (résumé resolution still uses `_allowlisted`, unchanged). New `save_job_fixture(...)` writes the file; `add_fixture_from_application(payload)` pulls the JD/company/role/url from the application's archived `posting.md` (decision 043) by id via the existing `_review_data`, or from a passed title/company/body, and returns `{fixture, fixtures}` (the refreshed list) so the picker updates without a reload; an application with no captured JD returns an actionable error ("Re-run it as a dry-run first…", Guideline #11). Wired at `POST /fixtures/add`. **Frontend (Track drawer):** two buttons on every application — **"Save to fixtures"** (standalone add → latches to "Saved to fixtures ✓") and **"Retailor résumé →"** (fetches the JD via `/track/review`, offers to save via a confirm, then switches to the Review tab, closes the drawer, and pre-loads the posting — selecting the new fixture if saved, else dropping title/company/body into the "paste a posting" box — and focuses the Tailor button so it's one click). Does NOT auto-run tailoring, so the user keeps control of engine/quality/length. **Verified:** end-to-end via real HTTP on an ephemeral `APPLICATIONBOT_DATA` — index 200, `POST /fixtures/add` saves + returns the friendly label and refreshed list, the token round-trips through `_fixture_path`→`load_job_description`, and `POST /tailor` (rules engine) tailors against the saved fixture (correct title/company); path-traversal token rejected; empty-JD app gives the actionable error; re-save is idempotent (file count unchanged); client JS passes `node --check`; 35 web/tailor/archive/track/autoloop tests pass (pre-existing `test_nav_recipes` collection error unrelated). **Button-consistency follow-up:** the drawer's action row was visually inconsistent — the `<button>` actions (Re-run/Retailor/Save to fixtures) fell through to the solid-accent base `button {}` style while the `<a>` actions (Open posting/View résumé) rendered as bare text links (the `.drawer-actions` rule only set width/margin/padding, no chrome). Gave `.drawer-actions button, .drawer-actions a` ONE shared secondary-button look (surface fill + `--line` border + `--accent-text`, matching the app's existing `.rerun`/`.runsbtn` pattern) with a consistent hover + disabled state, so all five actions are identical; verified via headless-Chromium screenshots of the open drawer in light **and** dark. | Accepted |
| 127 | 2026-07-22 | **Guarantee a full page by construction: instruct the model to OVERSHOOT one page, then let the existing measured trimmer bring it back to exactly one page.** Follow-up to decision 126: the fill-first prompt helped (user confirmed "a lot more vertical space") but résumés still under-fill, because the model estimates page geometry from character counts and can't see the rendered PDF — so it comes in *under* a page and the deterministic trimmer never engages. User's fix: "generate slightly over a page to begin, then slowly remove one line at a time until the rendered pdf is only one page." **Key realization — the trim half already exists:** [`pdf.fit_to_pages`](applicationbot/pdf.py) already renders the real PDF, counts pages, and pops one bullet (≈one line) at a time from the least-relevant entry (`_trim_once`, activities→projects→experience, last entry first), stopping the instant it hits ≤ max_pages — i.e. it lands *maximally full* given whatever it's handed. The only missing half was **overshoot**: nothing guaranteed the draft entering the trimmer exceeded a page. **Change (prompt-only, both Claude engines):** reframed `LengthBudget.prompt()` ([length.py](applicationbot/length.py)) from "FILL about N page(s)" to "aim to slightly **OVERFILL** N page(s) — produce a bit MORE strong, relevant content than fits, never less … the app measures the rendered PDF and removes the least-relevant lines one at a time until it fits exactly N page(s), so overshooting guarantees a full page while undershooting leaves it half-empty and cannot be fixed afterward … include the FULL allowance (4 exp / 3 proj / 2 act, each with the full 4 bullets) … when in doubt, include the extra entry or bullet — the app will trim it." Matched `SYSTEM_PROMPT` ([backends.py](applicationbot/backends.py)) directive to "OVERSHOOT THE PAGE, never undershoot it … extra strong content is free but a half-empty page cannot be fixed after the fact." **Why this is the foolproof shape:** the full allowance (4/3/2 entries × 4 bullets + skills/education/summary) reliably exceeds one page, so the trimmer always engages and lands right at the page boundary; the trim is measured on the real render, not estimated. **No code/behavior change to the trimmer or the caps:** `enforce`'s count caps and `fit_to_pages`'s hard 1-page guarantee are unchanged — the model just aims above them instead of below. Truthfulness guardrails ("never pad with filler / never invent") unchanged, so the overshoot is real base-résumé content the model was previously declining to surface. Rules engine unaffected. **Known limits (noted for follow-up):** overshoot is still produced by the model, so a genuinely thin candidate (few pool entries) can't be forced full — that's correct (no padding); and `_trim_once` drops a *whole entry* once all entries hit `_MIN_BULLETS=2`, a coarse step that can undershoot on the rare draft that's still 2 pages after bullet-trimming. If prompt-overshoot proves insufficient in practice, the deterministic escalation is a measured grow pass (re-add trimmed content / a bounded re-tailor when the render comes in under a page) rather than more prompt weight. **Verified:** rendered `LengthBudget().prompt()` shows the overshoot-first text with correct caps; `backends`/`length` import clean. Effect to be eyeballed on the user's next live retailor. | Accepted |
| 126 | 2026-07-22 | **Tailored résumés now fill the page: reframed the length rules from a ceiling-to-duck-under into a target-to-fill.** User observed tailored résumés leaving the bottom half of the page empty. Root cause was in the *rules*, not the layout: `catalogue.select_relevant` already hands the model ~2× the budget in candidates (up to 8 experience / 6 projects / 4 activities for a 1-page target), so the model was never content-starved — but every length instruction was phrased as an upper bound with active anti-fill language. `length.py`'s `prompt()` said *"Include **at most** N entries … Drop the least-relevant entries and bullets to fit … **Prefer fewer, stronger bullets over filling every slot**"*, and `backends.py`'s `SYSTEM_PROMPT` bullet section said *"STRONGLY PREFER single-line bullets"* with no vertical-fill counterweight. So the model played safe, dropped entries/bullets preemptively, and under-filled. `pdf.fit_to_pages` only ever **trims** when over a page — nothing ever pushes content **up** to fill an under-full page — so the prompt is the only lever that grows content, and it was pointed the wrong way. **Change (prompt-only, both engines):** rewrote `LengthBudget.prompt()` to say the résumé should **FILL** ~N page(s) ("a half-empty page with a blank bottom looks weak"), reframed "at most" → "up to", and added "**DEFAULT TO USING those slots** — include the full number of entries, and give each entry the full number of bullets, whenever you have genuinely relevant, truthful material … Go below a cap ONLY when you truly lack relevant content … never pad with filler or repeat yourself." Added one matching vertical-fill directive to `SYSTEM_PROMPT` ("FILL THE PAGE VERTICALLY … a résumé that stops halfway down the page reads as a thin candidate … Reach that fill with true, substantive bullets, never with filler or repetition"). **Truthfulness guardrails unchanged:** the existing "never invent / never pad with meaningless filler / truthfulness over length" rules stay, so the fill directive only mobilizes real base-résumé content the model was already declining to use. The caps (`max_experience`/`max_projects`/`max_activities`/`max_bullets_per_entry`) and the `enforce`/`fit_to_pages` hard-cap are unchanged — this only changes how aggressively the model fills *up to* them. Applies to both the Claude Code and Anthropic-API backends (shared `_user_message`/`SYSTEM_PROMPT`); the rules engine is unaffected (it selects/reorders, doesn't reword to length). **Verified:** `LengthBudget().prompt()` renders the new fill-first text with correct caps (4 exp / 3 proj / 2 act / 4 bullets). | Accepted |
| 125 | 2026-07-22 | **"Review before you sign off" — expand each goal-loop Ready card into a preview of exactly what will be submitted, and let the user watch the autofill first.** The goal-loop's Apply ▶ button (decision 069) fired a generic `confirm()` with no way to see the tailored résumé, the field answers, or the posting before committing an irreversible submit. The user asked to preview an application before signing off, and to "watch the autofill to make sure everything works." **Key realization — the data already existed:** the headless dry-run each `prepare_one` runs already writes the fill outcome (`report.json`: every filled `(label→value)` + the unanswered `skipped` fields) and the JD (`posting.md`) to the per-application archive (decision 043); the tailored PDF is already served by `/track/resume?id=` (decision 029). So the preview is mostly *exposure*, not new capture. **Three parts:** **(1) Preview (no storage change):** new read-only `GET /track/review?id=` joins the tracker row (company/role/location/remote/pay/portal/url/fit) with the archived `report.json` (filled answers + genuinely-unanswered fields) and `posting.md` (JD body) — the `[answer bank]` learning-summary line is filtered out of `skipped` in the view layer (it's not an unanswered field, and decision-018 still writes it to `report.skipped`, unchanged — Guideline #7), and file-upload answer values are shown as the bare filename, not the absolute path. Frontend: a **"Review ▾" action expands the ready card in place** (user's pick over a modal/page) into a panel — posting details, a lazy **"View tailored résumé ↗"** button (never rendered inline, per the user), the full answer table, an unanswered-fields callout (red), a collapsible JD, and the **Apply ▶ moved *inside* the reviewed panel** so a submit is always one step past seeing the answers. **(2) Per-application screenshot (small storage change):** the dry-run screenshot previously overwrote one shared `apply_review.png`; now `run_apply` writes it to the posting's archive dir as `filled.png` when `meta` identifies the posting (an explicit CLI `--screenshot` still wins; CLI runs with no meta keep the old default), served by new `GET /track/screenshot?id=` (`no-store`, same rationale as the résumé) and surfaced as a lazy **"View filled form ↗"** button. Same `archive.dir_for(company, role, source_url)` key on both the capture and read sides, so they always match. **(3) "Watch it fill" (browser-contention wiring):** a visible dry-run (`headed=True, pause=True, gate=None` — never submits) that lets the user watch the autofill. Because a single browser slot is shared, the user chose to **route it through the loop's serialized queue** rather than block on Stop-the-loop: new `_LOOP_WATCHES` queue + `queue_watch()` (enqueue while the loop runs; fall back to the existing visible `start_reapply(arm=False)` when idle) drained by the loop thread via **two new optional injected params on the pure `autoloop.auto_apply_loop`** — `take_watch_requests`/`watch_one`, drained at the same points as submits through a shared `_serve_requests()` (defaults make them inert, so pre-goal behaviour and all 22 existing autoloop tests are unchanged). `_LOOP_WATCH_HOLD` is set by Stop so a watch never blocks shutdown; the re-fill also refreshes that app's `filled.png` + `report.json`, so the panel reflects what was just watched. `POST /loop/watch` wires the button. **Coverage — the review gates EVERY real-submit button, not just the loop's:** the goal-loop Ready cards were the first host, but the parked/blocked "applications waiting on you" panel (`parkedCard`) has the other armed submit (`.pk-submit` "Submit for real ▶", `/parked/reapply` arm=true). Its resumable cards now get the same **Review ▾** toggle, with the dry-run re-fill + armed submit moved *inside* the reviewed panel. To keep each flow's own endpoint + messaging, `renderReview`/`toggleReview` gained an optional `signoff(r, title)→[button]` builder: `loopSignoff` (Watch `/loop/watch` + Apply `/loop/apply`) is the default; `parkedSignoff` (Re-apply-dry-run + Submit-for-real, both `/parked/reapply`) is passed by parked cards. So no armed submit in the UI is reachable without first expanding the preview. (The Track tab's only actions are dry-run re-runs — no armed submit — so it needs no gate.) Two view-layer polish fixes: the archive's `"(no posting text captured)"` placeholder is treated as no-JD, and the empty-fields note is context-neutral. **Safety unchanged:** Apply/Submit-for-real are still the only real submits (decision 058 one-shot armed gate); Review/Watch/Re-apply-dry-run are strictly dry-run. **Verified:** `/track/review` + `/track/screenshot` driven against the live server (real app #28 *Ramp — Software Engineer, Onboarding*: 7 answers, JD body, résumé 200, screenshot 200/`image/png`/`no-store`, actionable 404 when absent, `has_screenshot` toggles); the expanded panel rendered in headless Chromium (both artifact buttons, filtered unanswered field, filename-only résumé value, Watch+Apply sign-off row); watch wiring unit-checked without a browser (`auto_apply_loop` drains watches via `watch_one`; `queue_watch` enqueues idempotently while running; `_loop_watch` calls `run_apply` with `headed=True, pause=True, gate=None` and the watch hold). 22/22 `test_autoloop*` green (pre-existing `test_nav_recipes` `_bot_wall_evidence` collection error unrelated). **Still open:** the real happy-path capture of `filled.png` populates on the next live loop preparation/watch (verified plumbing with a fixture PNG, not a live external fill — dev never submits, but a full external dry-run is heavy/flaky to drive here). | Accepted |
...
**Date:** 2026-07-03
**Status:** Superseded in part by [[150]] (2026-07-29) — the `CLAUDE.md` session ritual is
removed and agents must not use or mention the bus; the CLI, docs, and hooks stay on disk
as dormant tooling.
...
> **Correction (2026-07-30, decision 157):** the `apply.py` half of this decision — `_bot_wall_evidence`,
> `_distil_nav`, `run_agent_nav`, `nav_agentic_enabled`, `ApplyReport.bot_wall`, and the `smartrecruiters`
> branch of `detect_ats` — was **never committed** (`git log -S` finds it in no revision), so everything
> below about the wall guard and the learn-once/replay loop described code that was not in the repo. Only
> `nav_recipes.py`, `parking.py`'s `bot_wall` kind, the fixtures, and the tests survived. Restored in
> decision 157; the tests that had been failing since are what specified it.
